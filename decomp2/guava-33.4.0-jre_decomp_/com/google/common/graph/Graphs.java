...
public final class Graphs extends GraphsBridgeMethods {
   ...
   private static boolean subgraphHasCycle(Graph graph, Map visitedNodes, Object startNode) {
      Deque<NodeAndRemainingSuccessors<N>> stack = new ArrayDeque();
      stack.addLast(new NodeAndRemainingSuccessors(startNode));

      while(!stack.isEmpty()) {
         NodeAndRemainingSuccessors<N> top = (NodeAndRemainingSuccessors)stack.removeLast();
         NodeAndRemainingSuccessors<N> prev = (NodeAndRemainingSuccessors)stack.peekLast();
         stack.addLast(top);
         N node = (N)top.node;
         N previousNode = (N)(prev == null ? null : prev.node);
         ...
         if (!top.remainingSuccessors.isEmpty()) {
            N nextNode = (N)top.remainingSuccessors.remove();
   ...
   public static ImmutableGraph transitiveClosure(Graph graph) {
   ...
   public static ImmutableSet reachableNodes(Graph graph, Object node) {
   ...
   private static final class NodeAndRemainingSuccessors {
      final Object node;
      @CheckForNull
      Queue remainingSuccessors;

      NodeAndRemainingSuccessors(Object node) {
         this.node = node;
      }
   }
