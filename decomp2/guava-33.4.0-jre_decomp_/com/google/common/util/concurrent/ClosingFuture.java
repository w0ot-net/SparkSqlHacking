...
public final class ClosingFuture {
   ...
   public static ClosingFuture submit(final ClosingCallable callable, Executor executor) {
   ...
   public static ClosingFuture submitAsync(final AsyncClosingCallable callable, Executor executor) {
      Preconditions.checkNotNull(callable);
      final CloseableList closeables = new CloseableList();
   ...
   private ClosingFuture(ListenableFuture future, CloseableList closeables) {
   ...
   public FluentFuture finishToFuture() {
      if (this.compareAndUpdateState(ClosingFuture.State.OPEN, ClosingFuture.State.WILL_CLOSE)) {
         ...
         switch (((State)this.state.get()).ordinal()) {
            case 0:
