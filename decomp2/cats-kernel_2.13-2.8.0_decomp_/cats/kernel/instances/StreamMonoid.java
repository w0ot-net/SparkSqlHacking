...
public class StreamMonoid implements Monoid {
   ...
   public Stream empty() {
      return .MODULE$.Stream().empty();
   }

   public Stream combine(final Stream x, final Stream y) {
      return (Stream)x.$plus$plus(y);
   }

   public Stream combineN(final Stream x, final int n) {
      return (Stream)StaticMethods$.MODULE$.combineNIterable(.MODULE$.Stream().newBuilder(), x, n);
   }

   public Stream combineAll(final IterableOnce xs) {
      return (Stream)StaticMethods$.MODULE$.combineAllIterable(.MODULE$.Stream().newBuilder(), xs);
   }
