...
public abstract class AbstractMultiValuedMap implements MultiValuedMap {
   ...
   public boolean containsMapping(Object key, Object value) {
      Collection<V> coll = (Collection)this.getMap().get(key);
   ...
   public MapIterator mapIterator() {
      return (MapIterator)(this.size() == 0 ? EmptyMapIterator.emptyMapIterator() : new MultiValuedMapIterator());
   }
   ...
   private class KeysMultiSet extends AbstractMultiSet {
      ...
      public int getCount(Object object) {
         int count = 0;
         Collection<V> col = (Collection)AbstractMultiValuedMap.this.getMap().get(object);
         if (col != null) {
            count = col.size();
         }
      ...
      private final class MapEntryTransformer implements Transformer {
         ...
         public MultiSet.Entry transform(final Map.Entry mapEntry) {
         ...
         return new LazyIteratorChain() {
            ...
            protected Iterator nextIterator(int count) {
               if (!this.keyIterator.hasNext()) {
                  ...
                  final K key = (K)this.keyIterator.next();
                  Transformer<V, Map.Entry<K, V>> entryTransformer = new Transformer() {
                     public Map.Entry transform(Object input) {
   ...
   private class AsMap extends AbstractMap {
      ...
      public Collection get(Object key) {
         Collection<V> collection = (Collection)this.decoratedMap.get(key);
      ...
      public Collection remove(Object key) {
         Collection<V> collection = (Collection)this.decoratedMap.remove(key);
         if (collection == null) {
            return null;
         } else {
