...
public class PubSecKeyOptionsConverter {
   ...
   static void fromJson(Iterable json, PubSecKeyOptions obj) {
      for(Map.Entry member : json) {
         switch ((String)member.getKey()) {
            ...
            case "buffer":
               if (member.getValue() instanceof String) {
                  obj.setBuffer(Buffer.buffer(BASE64_DECODER.decode((String)member.getValue())));
               }
               break;
            case "certificate":
   ...
   static void toJson(PubSecKeyOptions obj, Map json) {
