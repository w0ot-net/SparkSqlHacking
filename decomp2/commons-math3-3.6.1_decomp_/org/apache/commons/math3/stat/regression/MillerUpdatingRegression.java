...
public class MillerUpdatingRegression implements UpdatingMultipleLinearRegression {
   ...
   public MillerUpdatingRegression(int numberOfVariables, boolean includeConstant, double errorTolerance) throws ModelSpecificationException {
      this.nobs = 0L;
      this.sserr = (double)0.0F;
      this.rss_set = false;
      this.tol_set = false;
      this.sumy = (double)0.0F;
      this.sumsqy = (double)0.0F;
   ...
   public void addObservation(double[] x, double y) throws ModelSpecificationException {
      if ((this.hasIntercept || x.length == this.nvars) && (!this.hasIntercept || x.length + 1 == this.nvars)) {
         if (!this.hasIntercept) {
            this.include(MathArrays.copyOf(x, x.length), (double)1.0F, y);
         } else {
            double[] tmp = new double[x.length + 1];
            System.arraycopy(x, 0, tmp, 1, x.length);
            tmp[0] = (double)1.0F;
            this.include(tmp, (double)1.0F, y);
         }
   ...
   public void clear() {
      Arrays.fill(this.d, (double)0.0F);
      Arrays.fill(this.rhs, (double)0.0F);
      Arrays.fill(this.r, (double)0.0F);
      Arrays.fill(this.tol, (double)0.0F);
      Arrays.fill(this.rss, (double)0.0F);
      Arrays.fill(this.work_tolset, (double)0.0F);
      Arrays.fill(this.work_sing, (double)0.0F);
      Arrays.fill(this.x_sing, (double)0.0F);
         ...
         if (this.work_sing[col] < temp) {
            ...
            if (col >= this.nvars - 1) {
               ...
               for(int _xi = col + 1; _xi < this.nvars; ++_pi) {
                  ...
                  this.r[_pi] = (double)0.0F;
                  ++_xi;
               }

               double y = this.rhs[col];
               double weight = this.d[col];
               this.d[col] = (double)0.0F;
               this.rhs[col] = (double)0.0F;
   ...
   public double[] getPartialCorrelations(int in) {
      ...
      if (in >= -1 && in < this.nvars) {
         ...
         if (this.d[in] > (double)0.0F) {
            rms[in + rms_off] = (double)1.0F / FastMath.sqrt(this.d[in]);
         }
         ...
         for(int col = in + 1; col < this.nvars; ++col) {
            ...
            if (sumxx > (double)0.0F) {
               rms[col + rms_off] = (double)1.0F / FastMath.sqrt(sumxx);
            } else {
               rms[col + rms_off] = (double)0.0F;
            }
         }

         double sumyy = this.sserr;

         for(int row = in; row < this.nvars; ++row) {
            sumyy += this.d[row] * this.rhs[row] * this.rhs[row];
         }

         if (sumyy > (double)0.0F) {
            sumyy = (double)1.0F / FastMath.sqrt(sumyy);
         }
         ...
         for(int col1 = in; col1 < this.nvars; ++col1) {
            ...
            for(int row = in; row < col1; ++row) {
               ...
               for(int col2 = col1 + 1; col2 < this.nvars; ++col2) {
                  work[col2 + wrk_off] += this.d[row] * this.r[pos1] * this.r[pos2];
                  ++pos2;
               }
         ...
         for(int idx = 0; idx < count; ++idx) {
            ...
            if (d1 > this.epsilon || d2 > this.epsilon) {
               ...
               if (!(d1 < this.epsilon) && !(FastMath.abs(X) < this.epsilon)) {
                  if (d2 < this.epsilon) {
                     this.d[m] = d1 * X * X;
                     this.r[m1] = (double)1.0F / X;

                     for(int _i = m1 + 1; _i < m1 + this.nvars - m - 1; ++_i) {
                        double[] var10000 = this.r;
                        var10000[_i] /= X;
                     }

                     double[] var39 = this.rhs;
                     var39[m] /= X;
                     bSkipTo40 = true;
                  }
               } else {
