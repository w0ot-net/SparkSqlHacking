...
final class SaddlePointExpansion {
   ...
   static double getStirlingError(double z) {
      double ret;
      if (z < (double)15.0F) {
         double z2 = (double)2.0F * z;
         if (FastMath.floor(z2) == z2) {
            ret = EXACT_STIRLING_ERRORS[(int)z2];
         } else {
            ret = Gamma.logGamma(z + (double)1.0F) - (z + (double)0.5F) * FastMath.log(z) + z - HALF_LOG_2_PI;
         }
      } else {
         double z2 = z * z;
   ...
   static double logBinomialProbability(int x, int n, double p, double q) {
      ...
      if (x == 0) {
         ...
         double f = (Math.PI * 2D) * (double)x * (double)(n - x) / (double)n;
         ret = (double)-0.5F * FastMath.log(f) + ret;
      }
