...
public class ContinuousOutputModel implements StepHandler, Serializable {
   ...
   public void setInterpolatedTime(double time) {
      ...
      double tMin = (double)0.5F * (sMin.getPreviousTime() + sMin.getCurrentTime());
      int iMax = this.steps.size() - 1;
      StepInterpolator sMax = (StepInterpolator)this.steps.get(iMax);
      double tMax = (double)0.5F * (sMax.getPreviousTime() + sMax.getCurrentTime());
      if (this.locatePoint(time, sMin) <= 0) {
         ...
         while(iMax - iMin > 5) {
            ...
            if (location < 0) {
               iMax = this.index;
               tMax = (double)0.5F * (si.getPreviousTime() + si.getCurrentTime());
            } else {
               if (location <= 0) {
               ...
               iMin = this.index;
               tMin = (double)0.5F * (si.getPreviousTime() + si.getCurrentTime());
            }

            int iMed = (iMin + iMax) / 2;
            StepInterpolator sMed = (StepInterpolator)this.steps.get(iMed);
            double tMed = (double)0.5F * (sMed.getPreviousTime() + sMed.getCurrentTime());
