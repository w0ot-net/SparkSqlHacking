...
public abstract class MultistepFieldIntegrator extends AdaptiveStepsizeFieldIntegrator {
   protected RealFieldElement[] scaled;
   ...
   protected MultistepFieldIntegrator(Field field, String name, int nSteps, int order, double minStep, double maxStep, double scalAbsoluteTolerance, double scalRelativeTolerance) throws NumberIsTooSmallException {
      ...
      if (nSteps < 2) {
         ...
         this.nSteps = nSteps;
         this.exp = (double)-1.0F / (double)order;
         this.setSafety(0.9);
         this.setMinReduction(0.2);
         this.setMaxGrowth(FastMath.pow((double)2.0F, -this.exp));
      }
   }

   protected MultistepFieldIntegrator(Field field, String name, int nSteps, int order, double minStep, double maxStep, double[] vecAbsoluteTolerance, double[] vecRelativeTolerance) {
   ...
   protected void start(FieldExpandableODE equations, FieldODEState initialState, RealFieldElement t) throws DimensionMismatchException, NumberIsTooSmallException, MaxCountExceededException, NoBracketingException {
      ...
      } catch (InitializationCompletedMarkerException var5) {
         this.getEvaluationsCounter().increment(this.starter.getEvaluations());
         this.starter.clearStepHandlers();
      }
   }

   protected abstract Array2DRowFieldMatrix initializeHighOrderDerivatives(RealFieldElement var1, RealFieldElement[] var2, RealFieldElement[][] var3, RealFieldElement[][] var4);
   ...
   protected void rescale(RealFieldElement newStepSize) {
      T ratio = (T)((RealFieldElement)newStepSize.divide(this.getStepSize()));

      for(int i = 0; i < this.scaled.length; ++i) {
         this.scaled[i] = (RealFieldElement)this.scaled[i].multiply(ratio);
      }

      T[][] nData = (T[][])((RealFieldElement[][])this.nordsieck.getDataRef());
      T power = ratio;

      for(int i = 0; i < nData.length; ++i) {
         power = (T)((RealFieldElement)power.multiply(ratio));
         T[] nDataI = (T[])nData[i];
   ...
   protected RealFieldElement computeStepGrowShrinkFactor(RealFieldElement error) {
   ...
   private class FieldNordsieckInitializer implements FieldStepHandler {
      ...
      private final RealFieldElement[][] yDot;

      FieldNordsieckInitializer(FieldEquationsMapper mapper, int nbStartPoints) {
         this.mapper = mapper;
         this.count = 0;
