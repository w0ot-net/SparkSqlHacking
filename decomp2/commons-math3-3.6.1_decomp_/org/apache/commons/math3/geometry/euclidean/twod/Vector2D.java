...
public class Vector2D implements Vector {
   ...
   public static double angle(Vector2D v1, Vector2D v2) throws MathArithmeticException {
      ...
      if (normProduct == (double)0.0F) {
         ...
         if (!(dot < -threshold) && !(dot > threshold)) {
            ...
            return dot >= (double)0.0F ? FastMath.asin(n / normProduct) : Math.PI - FastMath.asin(n / normProduct);
         }
      }
   }
   ...
   public int hashCode() {
      return this.isNaN() ? 542 : 122 * (76 * MathUtils.hash(this.x) + MathUtils.hash(this.y));
   }
