...
public class Line implements Hyperplane, Embedding {
   ...
   public void reset(Vector2D p1, Vector2D p2) {
      ...
      if (d == (double)0.0F) {
         ...
         this.cos = (double)1.0F;
         this.sin = (double)0.0F;
   ...
   public Vector2D toSpace(Point point) {
   ...
   public Vector2D intersection(Line other) {
      ...
      return FastMath.abs(d) < this.tolerance ? null : new Vector2D(MathArrays.linearCombination(this.cos, other.originOffset, -other.cos, this.originOffset) / d, MathArrays.linearCombination(this.sin, other.originOffset, -other.sin, this.originOffset) / d);
   }
   ...
   public double getOffset(Line line) {
      return this.originOffset + (MathArrays.linearCombination(this.cos, line.cos, this.sin, line.sin) > (double)0.0F ? -line.originOffset : line.originOffset);
   }
   ...
   public double getOffset(Point point) {
      Vector2D p2 = (Vector2D)point;
      return MathArrays.linearCombination(this.sin, p2.getX(), -this.cos, p2.getY(), (double)1.0F, this.originOffset);
   }

   public boolean sameOrientationAs(Hyperplane other) {
      Line otherL = (Line)other;
      return MathArrays.linearCombination(this.sin, otherL.sin, this.cos, otherL.cos) >= (double)0.0F;
   }
   ...
   /** @deprecated */
   @Deprecated
   public static Transform getTransform(AffineTransform transform) throws MathIllegalArgumentException {
      double[] m = new double[6];
      transform.getMatrix(m);
      return new LineTransform(m[0], m[1], m[2], m[3], m[4], m[5]);
   }

   public static Transform getTransform(double cXX, double cYX, double cXY, double cYY, double cX1, double cY1) throws MathIllegalArgumentException {
   ...
   private static class LineTransform implements Transform {
      ...
      public Vector2D apply(Point point) {
         Vector2D p2D = (Vector2D)point;
         double x = p2D.getX();
         double y = p2D.getY();
         return new Vector2D(MathArrays.linearCombination(this.cXX, x, this.cXY, y, this.cX1, (double)1.0F), MathArrays.linearCombination(this.cYX, x, this.cYY, y, this.cY1, (double)1.0F));
      }

      public Line apply(Hyperplane hyperplane) {
         ...
         double inv = (double)1.0F / FastMath.sqrt(rSin * rSin + rCos * rCos);
      ...
      public SubHyperplane apply(SubHyperplane sub, Hyperplane original, Hyperplane transformed) {
         ...
         Vector1D newLoc = transformedLine.toSubSpace((Vector)this.apply((Point)originalLine.toSpace((Vector)op.getLocation())));
