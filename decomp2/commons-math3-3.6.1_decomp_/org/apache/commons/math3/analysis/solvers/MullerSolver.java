...
public class MullerSolver extends AbstractUnivariateSolver {
      ...
      if (FastMath.abs(fMin) < functionValueAccuracy) {
         ...
         if (FastMath.abs(fMax) < functionValueAccuracy) {
            ...
            if (FastMath.abs(fInitial) < functionValueAccuracy) {
               ...
               return this.isBracketing(min, initial) ? this.solve(min, initial, fMin, fInitial) : this.solve(initial, max, fInitial, fMax);
            }
         }
      }
   }
   ...
   private double solve(double min, double max, double fMin, double fMax) throws TooManyEvaluationsException {
      ...
      while(true) {
         ...
         double delta = c1 * c1 - (double)4.0F * y1 * d012;
         double xplus = x1 + (double)-2.0F * y1 / (c1 + FastMath.sqrt(delta));
         double xminus = x1 + (double)-2.0F * y1 / (c1 - FastMath.sqrt(delta));
         ...
         if (!bisect) {
            ...
            double xm = (double)0.5F * (x0 + x2);
            double ym = this.computeObjectiveValue(xm);
            if (FastMath.signum(y0) + FastMath.signum(ym) == (double)0.0F) {
               x2 = xm;
               y2 = ym;
            } else {
               x0 = xm;
