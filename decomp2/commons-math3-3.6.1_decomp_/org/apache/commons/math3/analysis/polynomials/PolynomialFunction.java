...
public class PolynomialFunction implements UnivariateDifferentiableFunction, DifferentiableUnivariateFunction, Serializable {
   ...
   public String toString() {
      ...
      if (this.coefficients[0] == (double)0.0F) {
      ...
      for(int i = 1; i < this.coefficients.length; ++i) {
         if (this.coefficients[i] != (double)0.0F) {
            if (s.length() > 0) {
               if (this.coefficients[i] < (double)0.0F) {
                  s.append(" - ");
               } else {
            ...
            } else if (this.coefficients[i] < (double)0.0F) {
               s.append("-");
            }

            double absAi = FastMath.abs(this.coefficients[i]);
            if (absAi - (double)1.0F != (double)0.0F) {
   ...
   private static String toString(double coeff) {
      ...
      return c.endsWith(".0") ? c.substring(0, c.length() - 2) : c;
   }
