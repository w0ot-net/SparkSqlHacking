...
public class Rule {
      ...
      if (!boxes) {
         ...
         if (startsWithBox && endsWithBox) {
            ...
            if (!boxContent.contains("[")) {
               ...
               if (startsWith && endsWith) {
                  return (input) -> input.length() == 1 && contains(boxContent, input.charAt(0)) == shouldMatch;
               }

               if (startsWith) {
                  return (input) -> input.length() > 0 && contains(boxContent, input.charAt(0)) == shouldMatch;
               }

               if (endsWith) {
                  return (input) -> input.length() > 0 && contains(boxContent, input.charAt(input.length() - 1)) == shouldMatch;
               }
            }
         }
      }
   ...
   public interface PhonemeExpr {
      ...
      default int size() {
         return (int)Math.min(this.getPhonemes().spliterator().getExactSizeIfKnown(), 2147483647L);
      }
   }

   public static final class PhonemeList implements PhonemeExpr {
      private final List phonemeList;
