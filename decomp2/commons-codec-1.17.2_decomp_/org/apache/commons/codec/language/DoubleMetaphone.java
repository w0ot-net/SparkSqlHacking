...
public class DoubleMetaphone implements StringEncoder {
   ...
   private String cleanInput(String input) {
      if (input == null) {
         ...
         input = input.trim();
         return input.isEmpty() ? null : input.toUpperCase(Locale.ENGLISH);
      }
   }
   ...
   private int handleG(String value, DoubleMetaphoneResult result, int index, boolean slavoGermanic) {
      if (this.charAt(value, index + 1) == 'H') {
         ...
         if ((contains(value, index + 1, 2, "ER") || this.charAt(value, index + 1) == 'Y') && !contains(value, 0, 6, "DANGER", "RANGER", "MANGER") && !contains(value, index - 1, 1, "E", "I") && !contains(value, index - 1, 3, "RGY", "OGY")) {
            result.append('K', 'J');
            index += 2;
   ...
   private int handleW(String value, DoubleMetaphoneResult result, int index) {
      if (contains(value, index, 2, "WR")) {
         ...
         if ((index != value.length() - 1 || !this.isVowel(this.charAt(value, index - 1))) && !contains(value, index - 1, 5, "EWSKI", "EWSKY", "OWSKI", "OWSKY") && !contains(value, 0, 3, "SCH")) {
