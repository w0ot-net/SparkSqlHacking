...
public abstract class BaseNCodec implements BinaryEncoder, BinaryDecoder {
   ...
   public byte[] encode(byte[] pArray) {
      return BinaryCodec.isEmpty(pArray) ? pArray : this.encode(pArray, 0, pArray.length);
   }
   ...
   public abstract static class AbstractBuilder implements Supplier {
      ...
      AbstractBuilder(byte[] defaultEncodeTable) {
         this.decodingPolicy = BaseNCodec.DECODING_POLICY_DEFAULT;
         this.lineSeparator = BaseNCodec.CHUNK_SEPARATOR;
         this.padding = 61;
         this.defaultEncodeTable = defaultEncodeTable;
         this.encodeTable = defaultEncodeTable;
      }

      AbstractBuilder asThis() {
         return this;
      }
      ...
      public AbstractBuilder setDecodingPolicy(CodecPolicy decodingPolicy) {
         this.decodingPolicy = decodingPolicy != null ? decodingPolicy : BaseNCodec.DECODING_POLICY_DEFAULT;
         return this.asThis();
      }

      public AbstractBuilder setEncodeTable(byte... encodeTable) {
         this.encodeTable = encodeTable != null ? encodeTable : this.defaultEncodeTable;
         return this.asThis();
      }

      public AbstractBuilder setLineLength(int lineLength) {
         this.lineLength = Math.max(0, lineLength);
         return this.asThis();
      }

      public AbstractBuilder setLineSeparator(byte... lineSeparator) {
         this.lineSeparator = lineSeparator != null ? lineSeparator : BaseNCodec.CHUNK_SEPARATOR;
         return this.asThis();
      }

      public AbstractBuilder setPadding(byte padding) {
