...
public final class InfoCmp {
   ...
   public static Map getCapabilitiesByName() {
      ...
      try {
         ...
         try {
            ...
            try {
               br.lines().map(String::trim).filter((s) -> !s.startsWith("#")).filter((s) -> !s.isEmpty()).forEach((s) -> {
                  String[] names = s.split(", ");
                  Capability cap = (Capability)Enum.valueOf(Capability.class, names[0]);
   ...
   public static void parseInfoCmp(String capabilities, Set bools, Map ints, Map strings) {
      ...
      for(int i = 1; i < lines.length; ++i) {
         ...
         while(m.find()) {
            ...
            if (cap.contains("#")) {
               ...
               Capability c = (Capability)capsByName.get(key);
               if (c != null) {
                  ints.put(c, iVal);
               }
               ...
               Capability c = (Capability)capsByName.get(cap);
               if (c != null) {
                  bools.add(c);
               }
            }
   ...
   static {
      for(String s : Arrays.asList("dumb", "dumb-color", "ansi", "xterm", "xterm-256color", "windows", "windows-256color", "windows-conemu", "windows-vtp", "screen", "screen-256color", "rxvt-unicode", "rxvt-unicode-256color", "rxvt-basic", "rxvt")) {
         setDefaultInfoCmp(s, (Supplier)(() -> loadDefaultInfoCmp(s)));
      }
