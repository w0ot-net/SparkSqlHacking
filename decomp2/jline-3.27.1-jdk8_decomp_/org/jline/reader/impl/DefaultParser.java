...
public class DefaultParser implements Parser {
   ...
   public static enum Bracket {
      ...
      // $FF: synthetic method
      private static Bracket[] $values() {
         return new Bracket[]{ROUND, CURLY, SQUARE, ANGLE};
      }
   }
      ...
      public void check(CharSequence buffer, int pos) {
         if (DefaultParser.this.openingBrackets != null && pos >= 0) {
            ...
            if (bid >= 0) {
               ...
               if (bid >= 0) {
                  if (!this.nested.isEmpty() && bid == (Integer)this.nested.get(this.nested.size() - 1)) {
            ...
            if (this.cursor > pos) {
               ...
               if (this.nested.size() > 0) {
                  this.nextClosingBracket = String.valueOf(DefaultParser.this.closingBrackets[(Integer)this.nested.get(this.nested.size() - 1)]);
               }
            }
      ...
      public String getMissingClosingBrackets() {
         if (!this.isClosingBracketMissing()) {
            ...
            for(int i = this.nested.size() - 1; i > -1; --i) {
               out.append(DefaultParser.this.closingBrackets[(Integer)this.nested.get(i)]);
            }
   ...
   public class ArgumentList implements ParsedLine, CompletingParsedLine {
      ...
      /** @deprecated */
      @Deprecated
      public ArgumentList(final String line, final List words, final int wordIndex, final int wordCursor, final int cursor) {
         this(line, words, wordIndex, wordCursor, cursor, (String)null, wordCursor, ((String)words.get(wordIndex)).length());
      }

      public ArgumentList(final String line, final List words, final int wordIndex, final int wordCursor, final int cursor, final String openingQuote, final int rawWordCursor, final int rawWordLength) {
         this.line = line;
      ...
      public String word() {
         return this.wordIndex >= 0 && this.wordIndex < this.words.size() ? (String)this.words.get(this.wordIndex) : "";
      }
      ...
      public CharSequence escape(CharSequence candidate, boolean complete) {
         ...
         if (DefaultParser.this.escapeChars != null) {
            if (DefaultParser.this.escapeChars.length > 0) {
               ...
               if (this.openingQuote != null) {
                  needToBeEscaped = (ixx) -> DefaultParser.this.isRawEscapeChar(sb.charAt(ixx)) || String.valueOf(sb.charAt(ixx)).equals(this.openingQuote);
               } else if (middleQuotes) {
                  needToBeEscaped = (ixx) -> DefaultParser.this.isRawEscapeChar(sb.charAt(ixx));
               } else {
                  needToBeEscaped = (ixx) -> DefaultParser.this.isDelimiterChar(sb, ixx) || DefaultParser.this.isRawEscapeChar(sb.charAt(ixx)) || DefaultParser.this.isRawQuoteChar(sb.charAt(ixx));
               }
