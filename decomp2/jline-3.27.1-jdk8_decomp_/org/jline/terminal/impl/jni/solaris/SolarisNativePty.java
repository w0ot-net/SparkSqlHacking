...
public class SolarisNativePty extends JniNativePty {
   ...
   public static SolarisNativePty current(TerminalProvider provider, SystemStream systemStream) throws IOException {
      try {
         switch (systemStream) {
            case Output:
               return new SolarisNativePty(provider, systemStream, -1, (FileDescriptor)null, 0, FileDescriptor.in, 1, FileDescriptor.out, ttyname(1));
            case Error:
               return new SolarisNativePty(provider, systemStream, -1, (FileDescriptor)null, 0, FileDescriptor.in, 2, FileDescriptor.err, ttyname(2));
            default:
   ...
   public static SolarisNativePty open(TerminalProvider provider, Attributes attr, Size size) throws IOException {
      ...
      return new SolarisNativePty(provider, (SystemStream)null, master[0], newDescriptor(master[0]), slave[0], newDescriptor(slave[0]), name);
   }
