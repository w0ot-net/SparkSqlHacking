...
public abstract class CodeGen extends Visitor implements Opcode, TokenId {
   ...
   private void atSwitchStmnt(Stmnt st) throws CompileError {
      ...
      for(ASTList list = body; list != null; list = list.tail()) {
         ...
         if (op == 310) {
            ...
            long caseLabel;
            if (isString) {
               caseLabel = (long)this.computeStringLabel(label.head(), tmpVar, gotoDefaults);
            } else {
               caseLabel = (long)this.computeLabel(label.head());
            }

            pairs[ipairs++] = (caseLabel << 32) + ((long)(curPos - opcodePc) & -1L);
         }
   ...
   private int computeStringLabel(ASTree expr, int tmpVar, List gotoDefaults) throws CompileError {
      this.doTypeCheck(expr);
      expr = TypeChecker.stripPlusExpr(expr);
   ...
   private void atPlusPlus(int token, ASTree oprand, Expr expr, boolean doDup) throws CompileError {
      ...
      if (oprand instanceof Variable) {
         ...
         if (t == 312) {
            ...
            this.bytecode.addDconst((double)1.0F);
