...
public class MemberResolver implements TokenId {
   ...
   private static Map invalidNamesMap = new WeakHashMap();
   ...
   private Method lookupMethod(CtClass clazz, String methodName, int[] argTypes, int[] argDims, String[] argClassNames, boolean onlyExact) throws CompileError {
      ...
      try {
         if (!isIntf) {
            ...
            if (pclazz != null) {
               ...
               if (r != null && (maybe == null || maybe.notmatch > r.notmatch)) {
                  maybe = r;
               }
            }
         }
      ...
      try {
         ...
         for(CtClass intf : ifs) {
            Method r = this.lookupMethod(intf, methodName, argTypes, argDims, argClassNames, onlyExact);
            if (r != null && (maybe == null || maybe.notmatch > r.notmatch)) {
               maybe = r;
            }
         }
   ...
   private Map getInvalidNames() {
      ...
      if (ht == null) {
         synchronized(MemberResolver.class) {
            Reference<Map<String, String>> ref = (Reference)invalidNamesMap.get(this.classPool);
            if (ref != null) {
               ht = (Map)ref.get();
            }
