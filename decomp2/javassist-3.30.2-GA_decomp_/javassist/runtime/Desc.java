...
public class Desc {
   ...
   private static final ThreadLocal USE_CONTEXT_CLASS_LOADER_LOCALLY = new ThreadLocal() {
   ...
   private static Class getClassObject(String name) throws ClassNotFoundException {
      return !useContextClassLoader && !(Boolean)USE_CONTEXT_CLASS_LOADER_LOCALLY.get() ? Class.forName(name) : Class.forName(name, true, Thread.currentThread().getContextClassLoader());
   }
