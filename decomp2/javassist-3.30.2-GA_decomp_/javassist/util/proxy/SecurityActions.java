...
class SecurityActions extends SecurityManager {
   ...
   static Method[] getDeclaredMethods(final Class clazz) {
      return System.getSecurityManager() == null ? clazz.getDeclaredMethods() : (Method[])AccessController.doPrivileged(new PrivilegedAction() {
         public Method[] run() {
   ...
   static Constructor[] getDeclaredConstructors(final Class clazz) {
      return System.getSecurityManager() == null ? clazz.getDeclaredConstructors() : (Constructor[])AccessController.doPrivileged(new PrivilegedAction() {
   ...
   static MethodHandle getMethodHandle(final Class clazz, final String name, final Class[] params) throws NoSuchMethodException {
      try {
         return (MethodHandle)AccessController.doPrivileged(new PrivilegedExceptionAction() {
   ...
   static TheUnsafe getSunMiscUnsafeAnonymously() throws ClassNotFoundException {
      try {
         return (TheUnsafe)AccessController.doPrivileged(new PrivilegedExceptionAction() {
            public TheUnsafe run() throws ClassNotFoundException, NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException {
               ...
               TheUnsafe usf = var10002.new TheUnsafe(unsafe, theUnsafe.get((Object)null));
         ...
         } else if (!(var1.getCause() instanceof IllegalAccessException) && !(var1.getCause() instanceof SecurityException)) {
