...
public class EdgePartitionBuilder$mcJ$sp extends EdgePartitionBuilder {
   public final PrimitiveVector edges$mcJ$sp;

   public void add(final long src, final long dst, final long d) {
      this.add$mcJ$sp(src, dst, d);
   }

   public void add$mcJ$sp(final long src, final long dst, final long d) {
      this.edges$mcJ$sp.$plus$eq(new Edge$mcJ$sp(src, dst, d));
   }
   ...
   public EdgePartition toEdgePartition$mcJ$sp() {
      Edge[] edgeArray = (Edge[])this.edges$mcJ$sp.trim().array();
      ...
      long[] data = (long[])this.org$apache$spark$graphx$impl$EdgePartitionBuilder$$evidence$1.newArray(edgeArray.length);
      ...
      return new EdgePartition$mcJ$sp(localSrcIds, localDstIds, data, index, global2local, local2global.trim$mcJ$sp().array$mcJ$sp(), vertexAttrs, scala.None..MODULE$, this.org$apache$spark$graphx$impl$EdgePartitionBuilder$$evidence$1, this.org$apache$spark$graphx$impl$EdgePartitionBuilder$$evidence$2);
   }

   public EdgePartitionBuilder$mcJ$sp(final int size, final ClassTag evidence$1, final ClassTag evidence$2) {
      super(size, evidence$1, evidence$2);
      this.edges$mcJ$sp = new PrimitiveVector(size, .MODULE$.apply(Edge.class));
   }
