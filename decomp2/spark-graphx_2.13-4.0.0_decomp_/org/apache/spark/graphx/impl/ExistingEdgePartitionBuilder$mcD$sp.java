...
public class ExistingEdgePartitionBuilder$mcD$sp extends ExistingEdgePartitionBuilder {
   public final PrimitiveVector edges$mcD$sp;

   public void add(final long src, final long dst, final int localSrc, final int localDst, final double d) {
      this.add$mcD$sp(src, dst, localSrc, localDst, d);
   }

   public void add$mcD$sp(final long src, final long dst, final int localSrc, final int localDst, final double d) {
      this.edges$mcD$sp.$plus$eq(new EdgeWithLocalIds$mcD$sp(src, dst, localSrc, localDst, d));
   }

   public EdgePartition toEdgePartition() {
      return this.toEdgePartition$mcD$sp();
   }

   public EdgePartition toEdgePartition$mcD$sp() {
      EdgeWithLocalIds[] edgeArray = (EdgeWithLocalIds[])this.edges$mcD$sp.trim().array();
      ...
      double[] data = (double[])this.org$apache$spark$graphx$impl$ExistingEdgePartitionBuilder$$evidence$3.newArray(edgeArray.length);
      ...
      if (edgeArray.length > 0) {
         ...
         for(int i = 0; i < edgeArray.length; ++i) {
            ...
            data[i] = edgeArray[i].attr$mcD$sp();
