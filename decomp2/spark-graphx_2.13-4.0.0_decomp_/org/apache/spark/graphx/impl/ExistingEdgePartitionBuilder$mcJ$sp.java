...
public class ExistingEdgePartitionBuilder$mcJ$sp extends ExistingEdgePartitionBuilder {
   public final PrimitiveVector edges$mcJ$sp;

   public void add(final long src, final long dst, final int localSrc, final int localDst, final long d) {
      this.add$mcJ$sp(src, dst, localSrc, localDst, d);
   }

   public void add$mcJ$sp(final long src, final long dst, final int localSrc, final int localDst, final long d) {
      this.edges$mcJ$sp.$plus$eq(new EdgeWithLocalIds$mcJ$sp(src, dst, localSrc, localDst, d));
   }

   public EdgePartition toEdgePartition() {
      return this.toEdgePartition$mcJ$sp();
   }

   public EdgePartition toEdgePartition$mcJ$sp() {
      EdgeWithLocalIds[] edgeArray = (EdgeWithLocalIds[])this.edges$mcJ$sp.trim().array();
      ...
      long[] data = (long[])this.org$apache$spark$graphx$impl$ExistingEdgePartitionBuilder$$evidence$3.newArray(edgeArray.length);
      ...
      if (edgeArray.length > 0) {
         ...
         for(int i = 0; i < edgeArray.length; ++i) {
            ...
            data[i] = edgeArray[i].attr$mcJ$sp();
