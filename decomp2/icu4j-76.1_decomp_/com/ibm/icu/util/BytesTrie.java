...
public final class BytesTrie implements Cloneable, Iterable {
   ...
   public Result current() {
      ...
      if (pos < 0) {
         ...
         int node;
         return this.remainingMatchLength_ < 0 && (node = this.bytes_[pos] & 255) >= 32 ? valueResults_[node & 1] : BytesTrie.Result.NO_VALUE;
      }
   }
            ...
            while(sIndex != sLimit) {
               ...
               if (length < 0) {
                  ...
                  while(true) {
                     ...
                     if (node < 16) {
                        Result result = this.branchNext(pos, node, inByte & 255);
   ...
   static {
      valueResults_ = new Result[]{BytesTrie.Result.INTERMEDIATE_VALUE, BytesTrie.Result.FINAL_VALUE};
   }
   ...
   public static final class Iterator implements java.util.Iterator {
      ...
      public Entry next() {
         ...
         if (pos < 0) {
            ...
            long top = (Long)this.stack_.remove(this.stack_.size() - 1);
            int length = (int)top;
            pos = (int)(top >> 32);
            this.entry_.truncateString(length & '\uffff');
            length >>>= 16;
