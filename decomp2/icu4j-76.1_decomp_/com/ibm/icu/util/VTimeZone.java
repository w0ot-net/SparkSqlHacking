...
public class VTimeZone extends BasicTimeZone {
   ...
   public boolean hasSameRules(TimeZone other) {
      if (this == other) {
         ...
         return other instanceof VTimeZone ? this.tz.hasSameRules(((VTimeZone)other).tz) : this.tz.hasSameRules(other);
      }
   }
      ...
      if (!hasTransitions) {
         ...
         if (stdCount > 0) {
            if (finalStdRule == null) {
               ...
               writeZonePropsByDOW(w, false, stdName, stdFromOffset, stdToOffset, stdMonth, stdWeekInMonth, stdDayOfWeek, stdStartTime, stdUntilTime);
               Date nextStart = finalStdRule.getNextStart(stdUntilTime, stdFromOffset - stdFromDSTSavings, stdFromDSTSavings, false);
   ...
   private static long parseDateTimeString(String str, int offset) {
      ...
      if (!isValid) {
         ...
         time += (long)(hour * 3600000 + min * '\uea60' + sec * 1000);
         if (!isUTC) {
            time -= (long)offset;
         }
