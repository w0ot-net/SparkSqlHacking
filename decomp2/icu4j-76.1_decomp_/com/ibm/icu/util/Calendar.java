...
public abstract class Calendar implements Serializable, Cloneable, Comparable {
   ...
   static final int[][][] MONTH_PRECEDENCE = new int[][][]{{{2}, {23}}};
   ...
   protected int internalGetMonth() {
      return this.resolveFields(MONTH_PRECEDENCE) == 2 ? this.internalGet(2) : this.internalGet(23);
   }

   /** @deprecated */
   @Deprecated
   protected int internalGetMonth(int defaultValue) {
      return this.resolveFields(MONTH_PRECEDENCE) == 2 ? this.internalGet(2, defaultValue) : this.internalGet(23, defaultValue);
   }
   ...
   private static PatternData getPatternData(ULocale locale, String calType) {
      ...
      if (useDTPG) {
         for(DateTimePatternGenerator dtpg = DateTimePatternGenerator.getInstanceNoStdPat(locale); i < TIME_SKELETONS.length; ++i) {
            dateTimePatterns[i] = dtpg.getBestPattern(TIME_SKELETONS[i]);
         }
      }

      for(; i < patternsSize; ++i) {
      ...
      if (dtPatternsRb != null) {
         ...
         if (patternsSize >= 4) {
            for(int var15 = 0; var15 < 4; ++var15) {
               ICUResourceBundle concatenationPatternRb = (ICUResourceBundle)dtPatternsRb.get(var15);
               if (concatenationPatternRb.getType() == 0) {
                  atTimePatterns[var15] = concatenationPatternRb.getString();
               }
            }
         }
      }
   ...
   protected void validateField(int field) {
      switch (field) {
         case 5:
            ...
            this.validateField(field, 1, this.handleGetMonthLength(y, this.internalGetMonth()));
            break;
         case 6:
            int y = this.handleGetExtendedYear();
   ...
   protected void computeTime() {
      ...
      if (this.stamp[21] >= 2 && this.newestStamp(9, 14, 0) <= this.stamp[21]) {
         millisInDay = (long)this.internalGet(21);
      } else {
         ...
         if (hour > 548) {
            ...
            millisInDay = (long)this.computeMillisInDay();
         }
      }
   ...
   protected long computeMillisInDayLong() {
      ...
      millisInDay *= 1000L;
      millisInDay += (long)this.internalGet(14);
      return millisInDay;
   }
   ...
   protected int computeJulianDay() {
      if (this.stamp[20] >= 2) {
         ...
         bestStamp = this.newestStamp(23, 23, bestStamp);
   ...
   static class PatternData {
      ...
      private static PatternData make(Calendar cal, ULocale loc) {
         ...
         boolean hasHourCycleKeywords = loc.getKeywordValue("rg") != null || loc.getKeywordValue("hours") != null;
         ...
         if (patternData == null) {
            try {
               ...
               patternData = new PatternData(Calendar.DEFAULT_PATTERNS, (String[])null, Calendar.DEFAULT_ATTIME_PATTERNS);
            }
