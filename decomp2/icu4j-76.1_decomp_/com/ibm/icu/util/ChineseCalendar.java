...
public class ChineseCalendar extends Calendar {
   ...
   private void offsetMonth(int newMoon, int dom, int delta) {
      newMoon += (int)(29.530588853 * ((double)delta - (double)0.5F));
            ...
            if (amount != 0) {
               ...
               if (this.hasLeapMonthBetweenWinterSolstices) {
                  if (this.get(22) == 1) {
                     ...
                     int moon1 = moon - (int)(29.530588853 * ((double)m - (double)0.5F));
               ...
               int n = this.hasLeapMonthBetweenWinterSolstices ? 13 : 12;
               int newM = (m + amount) % n;
               if (newM < 0) {
                  newM += n;
               }
   ...
   private int majorSolarTerm(int days) {
      int term = ((int)Math.floor((double)6.0F * (new CalendarAstronomer(this.daysToMillis(days))).getSunLongitude() / Math.PI) + 2) % 12;
      if (term < 1) {
         term += 12;
      }
   ...
   private void computeChineseFields(int days, int gyear, int gmonth, boolean setAllFields) {
      ...
      int ordinalMonth = this.synodicMonthsBetween(theNewYear, thisMoon);
      if (ordinalMonth < 0) {
         ordinalMonth += 12;
      }
   ...
   public String getTemporalMonthCode() {
      ...
      return is_leap != 0 ? gTemporalLeapMonthCodes[this.get(2)] : super.getTemporalMonthCode();
   }
   ...
   protected int internalGetMonth(int defaultValue) {
      return this.resolveFields(MONTH_PRECEDENCE) == 2 ? this.internalGet(2, defaultValue) : this.internalGetMonth();
   }
}
