...
public final class CollationBuilder extends CollationRuleParser.Sink {
   ...
   private int insertTailoredNodeAfter(int index, int strength) {
      ...
      int nextIndex;
      for(long node = this.nodes.elementAti(index); (nextIndex = nextIndexFromNode(node)) != 0; index = nextIndex) {
         node = this.nodes.elementAti(nextIndex);
         if (strengthFromNode(node) <= strength) {
            break;
         }
      ...
      long var6 = 8L | nodeFromStrength(strength);
      return this.insertNodeBetween(index, nextIndex, var6);
   }
   ...
   private int addOnlyClosure(CharSequence nfdPrefix, CharSequence nfdString, long[] newCEs, int newCEsLength, int ce32) {
      ...
      if (nfdPrefix.length() == 0) {
         ...
         while(true) {
            ...
            if (!this.ignoreString(str) && !str.contentEquals(nfdString)) {
         ...
         while(true) {
            ...
            if (!this.ignorePrefix(prefix)) {
               ...
               while(true) {
                  ...
                  if (!this.ignoreString(str) && (!samePrefix || !str.contentEquals(nfdString))) {
                     if (loop++ > kClosureLoopLimit) {
                        throw new ICUInputTooLongException("Too many closure");
                     }
                  ...
                  if (!isTailoredNode(node)) {
                     ...
                     if (!tIsTailored) {
                        ...
                        if (t == 0) {
                           ...
                           tLimit = (int)this.rootElements.getFirstTertiaryCE() & 16191;
                        ...
                        assert tLimit == 16384 || (tLimit & -16192) == 0;
