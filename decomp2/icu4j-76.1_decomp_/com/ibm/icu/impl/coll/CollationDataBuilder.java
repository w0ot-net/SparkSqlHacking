...
final class CollationDataBuilder {
      ...
      if (!Collation.isSpecialCE32(ce32)) {
         ...
         switch (Collation.tagFromCE32(ce32)) {
            ...
            case 8:
               ...
               for(CharsTrie.Iterator prefixes = CharsTrie.iterator(this.base.contexts, trieIndex + 2, 0); prefixes.hasNext(); cond = this.getConditionalCE32(index)) {
      ...
      if (!this.isMutable()) {
         ...
         if (this.getJamoCE32s(jamoCE32s)) {
            ...
            for(int i = 0; i < 19; ++i) {
               ...
               int limit = limit + 588;
               this.trie.setRange(limit, limit - 1, ce32, true);
               limit = limit;
            }
         } else {
            for(int c = 44032; c < 55204; c = limit) {
   ...
   protected int buildContext(ConditionalCE32 head) {
      ...
      ConditionalCE32 cond;
      for(cond = head; $assertionsDisabled || cond == head || cond.hasContext(); cond = this.getConditionalCE32(cond.next)) {
         ...
         if (!Collation.isSpecialCE32(ce32)) {
            ...
            if (tag == 5) {
               ...
               for(int i = 0; i < length; ++i) {
                  ...
                  long ce;
                  if (!Collation.isSpecialCE32(ce32) && (ce = this.modifier.modifyCE32(ce32)) != 4311744768L) {
                     if (!isModified) {
                        for(int j = 0; j < i; ++j) {
               ...
               for(ce32 = CollationDataBuilder.makeBuilderContextCE32(destIndex); cond.next >= 0; prevDestCond.next = destIndex) {
                  cond = this.src.getConditionalCE32(cond.next);
                  prevDestCond = this.dest.getConditionalCE32(destIndex);
