   ...
   public Number doParse(String text, ParsePosition parsePosition, double baseValue, double upperBound, boolean lenientParse, int nonNumericalExecutedRuleMask, int recursionCount) {
      ...
      if (this.withZeros) {
         ...
         while(workText.length() > 0 && workPos.getIndex() != 0) {
            workPos.setIndex(0);
            this.ruleSet.parse(workText, workPos, (double)1.0F, nonNumericalExecutedRuleMask, recursionCount).intValue();
      ...
      Number result = super.doParse(text, parsePosition, this.withZeros ? (double)1.0F : baseValue, upperBound, false, nonNumericalExecutedRuleMask, recursionCount);
      if (this.withZeros) {
