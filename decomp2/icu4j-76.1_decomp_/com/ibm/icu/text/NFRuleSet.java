      ...
      if (this.isFractionRuleSet) {
         ...
         if (Double.isInfinite(number)) {
            ...
            if (number != Math.floor(number)) {
               if (number < (double)1.0F && this.nonNumericalRules[2] != null) {
      ...
      if (recursionCount >= 64) {
         ...
         for(int nonNumericalRuleIdx = 0; nonNumericalRuleIdx < this.nonNumericalRules.length; ++nonNumericalRuleIdx) {
            ...
            if (nonNumericalRule != null && (nonNumericalExecutedRuleMask >> nonNumericalRuleIdx & 1) == 0) {
               ...
               Number tempResult = nonNumericalRule.doParse(text, parsePosition, false, upperBound, nonNumericalExecutedRuleMask, recursionCount + 1);
         ...
         for(int i = this.rules.length - 1; i >= 0 && highWaterMark.getIndex() < text.length(); --i) {
            if (this.isFractionRuleSet || !((double)this.rules[i].getBaseValue() >= upperBound)) {
               Number tempResult = this.rules[i].doParse(text, parsePosition, this.isFractionRuleSet, upperBound, nonNumericalExecutedRuleMask, recursionCount + 1);
               if (parsePosition.getIndex() > highWaterMark.getIndex()) {
                  result = tempResult;
                  highWaterMark.setIndex(parsePosition.getIndex());
               }
