...
public class RuleBasedNumberFormat extends NumberFormat {
   ...
   public RuleBasedNumberFormat(ULocale locale, int format) {
      ...
      this.capitalizationBrkIter = null;
      this.locale = locale;
      ICUResourceBundle bundle = (ICUResourceBundle)UResourceBundle.getBundleInstance("com/ibm/icu/impl/data/icudata/rbnf", locale);
   ...
   public String[] getRuleSetDisplayNames(ULocale loc) {
      ...
      if (names != null) {
         return (String[])((String;)names).clone();
      } else {
   ...
   public Number parse(String text, ParsePosition parsePosition) {
      ...
      for(int i = this.ruleSets.length - 1; i >= 0; --i) {
         if (this.ruleSets[i].isPublic() && this.ruleSets[i].isParseable()) {
            tempResult = this.ruleSets[i].parse(workingText, workingPos, Double.MAX_VALUE, 0, 0);
   ...
   private void initCapitalizationContextInfo(ULocale theLocale) {
      ICUResourceBundle rb = (ICUResourceBundle)UResourceBundle.getBundleInstance("com/ibm/icu/impl/data/icudata", theLocale);
