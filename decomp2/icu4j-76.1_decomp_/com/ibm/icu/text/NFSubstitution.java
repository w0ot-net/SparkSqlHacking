...
abstract class NFSubstitution {
   ...
   public void doSubstitution(long number, StringBuilder toInsertInto, int position, int recursionCount) {
      if (this.ruleSet != null) {
         ...
         toInsertInto.insert(position + this.pos, this.numberFormat.format(this.transformNumber((double)number)));
      } else {
         long numberToFormat = this.transformNumber(number);
   ...
   public Number doParse(String text, ParsePosition parsePosition, double baseValue, double upperBound, boolean lenientParse, int nonNumericalExecutedRuleMask, int recursionCount) {
      ...
      Number tempResult;
      if (this.ruleSet != null) {
         tempResult = this.ruleSet.parse(text, parsePosition, upperBound, nonNumericalExecutedRuleMask, recursionCount);
