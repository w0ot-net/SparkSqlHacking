...
public final class CanonicalIterator {
   ...
   public void setSource(String newSource) {
      ...
      if (newSource.length() == 0) {
         ...
         for(int var6 = 0; var6 < this.pieces.length; ++var6) {
            if (PROGRESS) {
               System.out.println("SEGMENT");
            }

            this.pieces[var6] = this.getEquivalents((String)segmentList.get(var6));
         }
   ...
   /** @deprecated */
   @Deprecated
   private static void permute(String source, boolean skipZeros, Set output, int depth) {
      if (depth > 8) {
         throw new UnsupportedOperationException("Stack too deep:" + depth);
      } else if (source.length() <= 2 && UTF16.countCodePoint(source) <= 1) {
         output.add(source);
      } else {
         ...
         for(int i = 0; i < source.length(); i += UTF16.getCharCount(cp)) {
            ...
            if (!skipZeros || i == 0 || UCharacter.getCombiningClass(cp) != 0) {
               subpermute.clear();
               permute(source.substring(0, i) + source.substring(i + UTF16.getCharCount(cp)), skipZeros, subpermute, depth + 1);
