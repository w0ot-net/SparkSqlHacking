...
final class NFRule {
   ...
   public void doFormat(double number, StringBuilder toInsertInto, int pos, int recursionCount) {
      ...
      if (this.rulePatternFormat == null) {
         ...
         double pluralVal;
         if ((double)0.0F <= number && number < (double)1.0F) {
   ...
   public Number doParse(String text, ParsePosition parsePosition, boolean isFractionRule, double upperBound, int nonNumericalExecutedRuleMask, int recursionCount) {
      ...
      if (pp.getIndex() == 0 && sub1Pos != 0) {
         ...
         do {
            pp.setIndex(0);
            double partialResult = this.matchToDelimiter(workText, start, tempBaseValue, this.ruleText.substring(sub1Pos, sub2Pos), this.rulePatternFormat, pp, this.sub1, upperBound, nonNumericalExecutedRuleMask, recursionCount).doubleValue();
            if (pp.getIndex() != 0 || this.sub1 == null) {
               start = pp.getIndex();
               String workText2 = workText.substring(pp.getIndex());
               ParsePosition pp2 = new ParsePosition(0);
               partialResult = this.matchToDelimiter(workText2, 0, partialResult, this.ruleText.substring(sub2Pos), this.rulePatternFormat, pp2, this.sub2, upperBound, nonNumericalExecutedRuleMask, recursionCount).doubleValue();
         ...
         if (isFractionRule && highWaterMark > 0 && this.sub1 == null) {
            result = (double)1.0F / result;
         }

         return (Number)(result == (double)((long)result) ? (long)result : result);
      }
   }
   ...
   private Number matchToDelimiter(String text, int startPos, double baseVal, String delimiter, PluralFormat pluralFormatDelimiter, ParsePosition pp, NFSubstitution sub, double upperBound, int nonNumericalExecutedRuleMask, int recursionCount) {
      if (!this.allIgnorable(delimiter)) {
         ...
         for(int dLen = temp[1]; dPos >= 0; dLen = temp[1]) {
            ...
            if (subText.length() > 0) {
               Number tempResult = sub.doParse(subText, tempPP, baseVal, upperBound, this.formatter.lenientParseEnabled(), nonNumericalExecutedRuleMask, recursionCount);
         ...
         ParsePosition tempPP = new ParsePosition(0);
         Number result = ZERO;
         Number tempResult = sub.doParse(text, tempPP, baseVal, upperBound, this.formatter.lenientParseEnabled(), nonNumericalExecutedRuleMask, recursionCount);
   ...
   private int prefixLength(String str, String prefix) {
      if (prefix.length() == 0) {
         ...
         if (scanner != null) {
            return str.startsWith(prefix) ? prefix.length() : scanner.prefixLength(str, prefix);
         } else {
   ...
   private int[] findText(String str, String key, PluralFormat pluralFormatKey, int startingAt) {
      ...
      if (pluralFormatKey != null) {
         ...
         return pos[0] >= 0 ? pos : scanner.findText(str, key, startingAt);
      } else {
