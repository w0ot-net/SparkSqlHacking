...
public final class AlphabeticIndex implements Iterable {
   ...
   private List initLabels() {
      ...
      for(String item : this.initialLabels) {
         ...
         if (this.collatorPrimaryOnly.compare(item, firstScriptBoundary) >= 0 && this.collatorPrimaryOnly.compare(item, overflowBoundary) < 0 && (!checkDistinct || this.collatorPrimaryOnly.compare(item, this.separated(item)) != 0)) {
   ...
   private BucketList createBucketList() {
      ...
      for(String current : indexCharacters) {
         ...
         Bucket<V> bucket = new Bucket(fixLabel(current), current, AlphabeticIndex.Bucket.LabelType.NORMAL);
         ...
         if (!current.startsWith("\ufdd0") && hasMultiplePrimaryWeights(this.collatorPrimaryOnly, variableTop, current) && !current.endsWith("\uffff")) {
            ...
            while(true) {
               Bucket<V> singleBucket = (Bucket)bucketList.get(i);
      ...
      if (bucketList.size() == 1) {
         ...
         if (!hasInvisibleBuckets) {
            ...
            while(true) {
               ...
               Bucket<V> bucket = (Bucket)bucketList.get(i);
               if (bucket.displayBucket == null) {
   ...
   private static class BucketList implements Iterable {
      ...
      private int getBucketIndex(CharSequence name, Collator collatorPrimaryOnly) {
         ...
         while(start + 1 < limit) {
            int i = (start + limit) / 2;
            Bucket<V> bucket = (Bucket)this.bucketList.get(i);
