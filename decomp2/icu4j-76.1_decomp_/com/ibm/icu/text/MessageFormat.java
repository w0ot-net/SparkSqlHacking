...
public class MessageFormat extends UFormat {
         ...
         while(true) {
            ...
            if (type != MessagePattern.Part.Type.REPLACE_NUMBER && type != MessagePattern.Part.Type.SKIP_SYNTAX) {
               if (type == MessagePattern.Part.Type.ARG_START) {
                  ...
                  int var16 = this.msgPattern.getPart(i).getLimit();
                  MessagePattern.appendReducedApostrophes(msgString, index, var16, sb);
                  prevIndex = var16;
               }
            } else {
               if (sb == null) {
   ...
   private static double parseChoiceArgument(MessagePattern pattern, int partIndex, String source, ParsePosition pos) {
      ...
      int msgLimit;
      for(double tempNumber = (double)0.0F; pattern.getPartType(partIndex) != MessagePattern.Part.Type.ARG_LIMIT; partIndex = msgLimit + 1) {
   ...
   private int findOtherSubMessage(int param1) {
   ...
   private void format(Object arguments, AppendableWrapper result, FieldPosition fp) {
      if (arguments != null && !(arguments instanceof Map)) {
         this.format(arguments, (Map)null, result, fp);
      } else {
   ...
   Format dateTimeFormatForPatternOrSkeleton(String style) {
      ...
      return (Format)(style.regionMatches(i, "::", 0, 2) ? DateFormat.getInstanceForSkeleton(style.substring(i + 2), this.ulocale) : new SimpleDateFormat(style, this.ulocale));
   }
