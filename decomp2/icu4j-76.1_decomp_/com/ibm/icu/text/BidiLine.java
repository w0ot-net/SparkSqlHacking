...
final class BidiLine {
      ...
      if (bidi.runCount < 0) {
         if (bidi.direction != 2) {
            ...
            if (runCount == 1 && limit == length) {
               ...
               do {
                  ...
                  do {
                     ++var20;
                  } while(var20 < limit && levels[var20] == level);
               ...
               for(int var21 = 0; var21 < runCount; ++var21) {
                  runs[var21].level = levels[runs[var21].start];
                  limit = runs[var21].limit += limit;
               }
         ...
         if (bidi.insertPoints.size > 0) {
            for(int ip = 0; ip < bidi.insertPoints.size; ++ip) {
               ...
               BidiRun var10000 = bidi.runs[runIndex];
   ...
   static int[] prepareReorder(byte[] levels, byte[] pMinLevel, byte[] pMaxLevel) {
      if (levels != null && levels.length > 0) {
         ...
         for(int var8 = levels.length; var8 > 0; indexMap[var8] = var8) {
            --var8;
         }
   ...
   static int[] getVisualMap(Bidi bidi) {
      ...
      if (bidi.insertPoints.size > 0) {
         ...
         for(int var31 = runCount - 1; var31 >= 0 && markFound > 0; --var31) {
            int insertRemove = runs[var31].insertRemove;
            if ((insertRemove & 10) > 0) {
               --k;
               indexMap[k] = -1;
               --markFound;
            ...
            visualStart = var31 > 0 ? runs[var31 - 1].limit : 0;

            for(int j = runs[var31].limit - 1; j >= visualStart && markFound > 0; --j) {
               --k;
