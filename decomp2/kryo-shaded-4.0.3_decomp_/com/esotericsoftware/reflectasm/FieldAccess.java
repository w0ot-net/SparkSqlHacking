...
public abstract class FieldAccess {
   ...
   public static FieldAccess get(Class type) {
      ...
      for(Class nextClass = type; nextClass != Object.class; nextClass = nextClass.getSuperclass()) {
         ...
         for(int n = declaredFields.length; i < n; ++i) {
      ...
      if (!fields.isEmpty()) {
         ...
         for(int n = labels.length; i < n; ++i) {
            if (!labels[i].equals(labelForInvalidTypes)) {
               ...
               mv.visitVarInsn(25, 1);
               mv.visitTypeInsn(192, classNameInternal);
               mv.visitFieldInsn(180, classNameInternal, ((Field)fields.get(i)).getName(), "Ljava/lang/String;");
               mv.visitInsn(176);
            }
         }
      ...
      if (!fields.isEmpty()) {
         ...
         for(int n = labels.length; i < n; ++i) {
            if (!labels[i].equals(labelForInvalidTypes)) {
               ...
               mv.visitFieldInsn(181, classNameInternal, ((Field)fields.get(i)).getName(), typeNameInternal);
               mv.visitInsn(177);
            }
         }
