...
public class SocksPortUnificationServerHandler extends ByteToMessageDecoder {
      ...
      if (in.writerIndex() != readerIndex) {
         ...
         switch (version) {
            case SOCKS4a:
               ...
               p.addAfter(ctx.name(), (String)null, Socks4ServerEncoder.INSTANCE);
               p.addAfter(ctx.name(), (String)null, new Socks4ServerDecoder());
               break;
            case SOCKS5:
               logKnownVersion(ctx, version);
               p.addAfter(ctx.name(), (String)null, this.socks5encoder);
               p.addAfter(ctx.name(), (String)null, new Socks5InitialRequestDecoder());
               break;
            default:
   ...
   private static void logUnknownVersion(ChannelHandlerContext ctx, byte versionVal) {
      if (logger.isDebugEnabled()) {
         logger.debug("{} Unknown protocol version: {}", ctx.channel(), versionVal & 255);
      }
