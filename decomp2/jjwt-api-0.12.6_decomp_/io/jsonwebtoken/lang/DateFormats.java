...
public final class DateFormats {
   ...
   public static String formatIso8601(Date date, boolean includeMillis) {
      return includeMillis ? ((DateFormat)ISO_8601_MILLIS.get()).format(date) : ((DateFormat)ISO_8601.get()).format(date);
   }

   public static Date parseIso8601Date(String s) throws ParseException {
      Assert.notNull(s, "String argument cannot be null.");
      return s.lastIndexOf(46) > -1 ? ((DateFormat)ISO_8601_MILLIS.get()).parse(s) : ((DateFormat)ISO_8601.get()).parse(s);
   }
}
