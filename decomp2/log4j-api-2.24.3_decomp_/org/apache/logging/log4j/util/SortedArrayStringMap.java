...
public class SortedArrayStringMap implements IndexedStringMap {
   ...
   private static final TriConsumer PUT_ALL = (key, value, contextData) -> contextData.putValue(key, value);
   ...
   public SortedArrayStringMap(final Map map) {
      this.keys = EMPTY;
      this.values = EMPTY;
      this.resize(ceilingNextPowerOfTwo(map.size()));

      for(Map.Entry entry : map.entrySet()) {
         this.putValue(Objects.toString(entry.getKey(), (String)null), entry.getValue());
      }
   ...
   private void writeObject(final ObjectOutputStream s) throws IOException {
      ...
      if (this.size > 0) {
         for(int i = 0; i < this.size; ++i) {
            ...
            SerializationUtil.writeWrappedObject(this.values[i] instanceof Serializable ? (Serializable)this.values[i] : null, s);
         }
      }
      ...
      if (capacity < 0) {
         ...
         if (mappings < 0) {
            ...
            for(int i = 0; i < mappings; ++i) {
               ...
               this.values[i] = SerializationUtil.readWrappedObject(s);
            }
