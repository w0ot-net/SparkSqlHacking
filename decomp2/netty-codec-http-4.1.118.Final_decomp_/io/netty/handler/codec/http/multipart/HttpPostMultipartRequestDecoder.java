...
public class HttpPostMultipartRequestDecoder implements InterfaceHttpPostRequestDecoder {
   ...
   private InterfaceHttpData findMultipartDisposition() {
      ...
      while(!this.skipOneLine()) {
         ...
         } catch (HttpPostRequestDecoder.NotEnoughDataDecoderException var22) {
            this.undecodedChunk.readerIndex(readerIndex);
            return null;
         }
   ...
   private static boolean loadDataMultipartOptimized(ByteBuf undecodedChunk, String delimiter, HttpData httpData) {
      if (!undecodedChunk.isReadable()) {
         ...
         if (posDelimiter < 0) {
            ...
            if (posDelimiter < 0 && httpData.definedLength() == httpData.length() + (long)readableBytes - 1L && undecodedChunk.getByte(readableBytes + startReaderIndex - 1) == 13) {
               lastPosition = 0;
