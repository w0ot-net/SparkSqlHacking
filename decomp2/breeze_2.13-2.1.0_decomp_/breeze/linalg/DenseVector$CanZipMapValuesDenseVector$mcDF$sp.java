...
public class DenseVector$CanZipMapValuesDenseVector$mcDF$sp extends DenseVector.CanZipMapValuesDenseVector implements CanZipMapValues$mcDF$sp {
   ...
   public DenseVector map$mcDF$sp(final DenseVector from, final DenseVector from2, final Function2 fn) {
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         for(int i = 0; i < from.length(); ++i) {
            result.data$mcD$sp()[i] = BoxesRunTime.unboxToDouble(fn.apply(BoxesRunTime.boxToFloat(from.apply$mcF$sp(i)), BoxesRunTime.boxToFloat(from2.apply$mcF$sp(i))));
         }
   ...
   public DenseVector$CanZipMapValuesDenseVector$mcDF$sp(final ClassTag evidence$15) {
      super(evidence$15);
