...
public class SliceVector$mcIF$sp extends SliceVector implements Vector$mcF$sp {
   public final Tensor tensor$mcIF$sp;
   ...
   public float apply$mcF$sp(final int i) {
      return this.tensor().apply$mcIF$sp(BoxesRunTime.unboxToInt(this.slices().apply(i)));
   }
   ...
   public void update$mcF$sp(final int i, final float v) {
      this.tensor().update$mcIF$sp(BoxesRunTime.unboxToInt(this.slices().apply(i)), v);
   }
   ...
   public DenseVector copy$mcF$sp() {
      return (DenseVector)DenseVector$.MODULE$.apply((Seq)this.slices().map((JFunction1.mcFI.sp)(i) -> this.tensor().apply$mcIF$sp(i)), this.breeze$linalg$SliceVector$$evidence$1);
   }

   public SliceVector repr() {
      return this.repr$mcIF$sp();
   }
   ...
   public SliceVector$mcIF$sp(final Tensor tensor$mcIF$sp, final IndexedSeq slices, final ClassTag evidence$1) {
      super((Tensor)null, slices, evidence$1);
      this.tensor$mcIF$sp = tensor$mcIF$sp;
