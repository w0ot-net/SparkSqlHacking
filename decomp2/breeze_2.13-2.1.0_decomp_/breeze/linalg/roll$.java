...
public final class roll$ implements UFunc {
   public static final roll$ MODULE$ = new roll$();
   ...
   public UFunc.UImpl2 impl(final ClassTag evidence$1) {
      return new UFunc.UImpl2(evidence$1) {
         ...
         public DenseVector apply(final DenseVector v, final int n) {
            boolean cond$macro$1 = n >= 0;
            if (!cond$macro$1) {
               throw new IllegalArgumentException("requirement failed: n.>=(0)");
            } else {
               boolean cond$macro$2 = n < v.size();
               if (!cond$macro$2) {
                  throw new IllegalArgumentException("requirement failed: n.<(v.size)");
               } else {
                  DenseVector result = DenseVector$.MODULE$.apply(this.evidence$1$1.newArray(v.size()));
                  int endOfBeginning = v.size() - n;
                  int index$macro$4 = 0;

                  for(int limit$macro$6 = n; index$macro$4 < limit$macro$6; ++index$macro$4) {
                     result.update(index$macro$4, v.apply(endOfBeginning + index$macro$4));
                  }

                  int index$macro$9 = n;

                  for(int limit$macro$11 = v.size(); index$macro$9 < limit$macro$11; ++index$macro$9) {
                     result.update(index$macro$9, v.apply(index$macro$9 - n));
                  }
