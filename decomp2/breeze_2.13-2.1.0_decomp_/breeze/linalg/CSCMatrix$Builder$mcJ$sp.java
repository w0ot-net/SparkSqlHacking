...
public class CSCMatrix$Builder$mcJ$sp extends CSCMatrix.Builder {
   public final Semiring evidence$9$mcJ$sp;
   public final Zero evidence$10$mcJ$sp;
   public final ArrayBuilder vs$mcJ$sp;
   private final int initNnz;
   private final ClassTag evidence$8;

   public Semiring ring() {
      return this.ring$mcJ$sp();
   }

   public Semiring ring$mcJ$sp() {
      return (Semiring).MODULE$.implicitly(this.evidence$9$mcJ$sp);
   }

   public void add(final int r, final int c, final long v) {
      this.add$mcJ$sp(r, c, v);
   }

   public void add$mcJ$sp(final int r, final int c, final long v) {
      if (v != this.ring$mcJ$sp().zero$mcJ$sp()) {
         this.breeze$linalg$CSCMatrix$Builder$$numAdded_$eq(this.breeze$linalg$CSCMatrix$Builder$$numAdded() + 1);
         this.vs().$plus$eq(BoxesRunTime.boxToLong(v));
         this.breeze$linalg$CSCMatrix$Builder$$indices().$plus$eq(BoxesRunTime.boxToLong((long)c << 32 | (long)r & 4294967295L));
      }

   }

   public ArrayBuilder vs$mcJ$sp() {
      return this.vs$mcJ$sp;
   }
   ...
   public CSCMatrix result$mcJ$sp() {
      return this.result$mcJ$sp(false, false);
   }

   public CSCMatrix result(final boolean keysAlreadyUnique, final boolean keysAlreadySorted) {
      return this.result$mcJ$sp(keysAlreadyUnique, keysAlreadySorted);
   }
   ...
   public CSCMatrix result$mcJ$sp(final boolean keysAlreadyUnique, final boolean keysAlreadySorted) {
      ...
      if (nnz == 0) {
         return new CSCMatrix$mcJ$sp(vs, _rows, _cols, outCols, 0, (int[])scala.Array..MODULE$.apply(scala.collection.immutable.Nil..MODULE$, scala.reflect.ClassTag..MODULE$.Int()), this.evidence$10$mcJ$sp);
      } else {
         Sorting$.MODULE$.indirectSort$mJc$sp((long[])indices, vs, 0, nnz);
         int[] outRows = new int[nnz];
         long[] outData = (long[])this.breeze$linalg$CSCMatrix$Builder$$evidence$8.newArray(nnz);
         ...
         for(lastCol = this.breeze$linalg$CSCMatrix$Builder$$colFromIndex(indices[0]); i < nnz; ++i) {
            ...
            if (colsEqual && row == this.breeze$linalg$CSCMatrix$Builder$$rowFromIndex(indices[i - 1])) {
               ...
               outData[outDataIndex] = this.ring$mcJ$sp().$plus$mcJ$sp(outData[outDataIndex], vs[i]);
            } else {
               ++outDataIndex;
         ...
         if (keysAlreadyUnique && outDataIndex != nnz) {
            ...
            CSCMatrix out = new CSCMatrix$mcJ$sp(outData, _rows, _cols, outCols, outDataIndex, outRows, this.evidence$10$mcJ$sp);
   ...
   public CSCMatrix$Builder$mcJ$sp(final int rows, final int cols, final int initNnz, final ClassTag evidence$8, final Semiring evidence$9$mcJ$sp, final Zero evidence$10$mcJ$sp) {
      super(rows, cols, initNnz, evidence$8, evidence$9$mcJ$sp, evidence$10$mcJ$sp);
      this.evidence$9$mcJ$sp = evidence$9$mcJ$sp;
      this.evidence$10$mcJ$sp = evidence$10$mcJ$sp;
      this.initNnz = initNnz;
      this.evidence$8 = evidence$8;
      this.vs$mcJ$sp = ArrayBuilder$.MODULE$.make(this.breeze$linalg$CSCMatrix$Builder$$evidence$8);
