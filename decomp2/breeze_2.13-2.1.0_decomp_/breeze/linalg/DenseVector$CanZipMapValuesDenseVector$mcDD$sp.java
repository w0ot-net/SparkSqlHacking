...
public class DenseVector$CanZipMapValuesDenseVector$mcDD$sp extends DenseVector.CanZipMapValuesDenseVector implements CanZipMapValues$mcDD$sp {
   ...
   public DenseVector map$mcDD$sp(final DenseVector from, final DenseVector from2, final Function2 fn) {
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         for(int i = 0; i < from.length(); ++i) {
            result.data$mcD$sp()[i] = fn.apply$mcDDD$sp(from.apply$mcD$sp(i), from2.apply$mcD$sp(i));
         }
   ...
   public DenseVector$CanZipMapValuesDenseVector$mcDD$sp(final ClassTag evidence$15) {
      super(evidence$15);
