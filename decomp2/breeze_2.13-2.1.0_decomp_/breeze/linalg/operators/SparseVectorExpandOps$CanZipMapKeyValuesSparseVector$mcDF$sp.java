...
public class SparseVectorExpandOps$CanZipMapKeyValuesSparseVector$mcDF$sp extends SparseVectorExpandOps.CanZipMapKeyValuesSparseVector implements CanZipMapKeyValues$mcIDF$sp {
   ...
   public SparseVector map$mcDF$sp(final SparseVector from, final SparseVector from2, final Function3 fn) {
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         for(int limit$macro$6 = from.length(); index$macro$4 < limit$macro$6; ++index$macro$4) {
            result.update$mcD$sp(index$macro$4, BoxesRunTime.unboxToDouble(fn.apply(BoxesRunTime.boxToInteger(index$macro$4), BoxesRunTime.boxToFloat(from.apply$mcF$sp(index$macro$4)), BoxesRunTime.boxToFloat(from2.apply$mcF$sp(index$macro$4)))));
         }
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         for(off2 = 0; off1 < from.activeSize(); ++off1) {
            while(off2 < from2.activeSize() && from2.indexAt(off2) < from.indexAt(off1)) {
               ...
               vb.add$mcD$sp(index, BoxesRunTime.unboxToDouble(fn.apply(BoxesRunTime.boxToInteger(index), BoxesRunTime.boxToFloat(from.default$mcF$sp()), BoxesRunTime.boxToFloat(from2.valueAt$mcF$sp(off2)))));
               ++off2;
            }

            if (off2 < from2.activeSize() && from.indexAt(off1) == from2.indexAt(off2)) {
               int index = from2.indexAt(off2);
               vb.add$mcD$sp(index, BoxesRunTime.unboxToDouble(fn.apply(BoxesRunTime.boxToInteger(index), BoxesRunTime.boxToFloat(from.valueAt$mcF$sp(off1)), BoxesRunTime.boxToFloat(from2.valueAt$mcF$sp(off2)))));
               ++off2;
            } else {
               int index = from.indexAt(off1);
               vb.add$mcD$sp(index, BoxesRunTime.unboxToDouble(fn.apply(BoxesRunTime.boxToInteger(index), BoxesRunTime.boxToFloat(from.valueAt$mcF$sp(off1)), BoxesRunTime.boxToFloat(from2.default$mcF$sp()))));
            }
         }
   ...
   // $FF: synthetic method
   public SparseVectorExpandOps breeze$linalg$operators$SparseVectorExpandOps$CanZipMapKeyValuesSparseVector$mcDF$sp$$$outer() {
      return this.$outer;
   }

   public SparseVectorExpandOps$CanZipMapKeyValuesSparseVector$mcDF$sp(final SparseVectorExpandOps $outer, final ClassTag evidence$14, final Zero evidence$15$mcD$sp, final Semiring evidence$16$mcD$sp) {
