...
public class SparseVectorExpandOps$CanZipMapValuesSparseVector$mcIJ$sp extends SparseVectorExpandOps.CanZipMapValuesSparseVector implements CanZipMapValues$mcIJ$sp {
   ...
   public SparseVector map$mcIJ$sp(final SparseVector from, final SparseVector from2, final Function2 fn) {
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         int zz = fn.apply$mcIJJ$sp(from.default$mcJ$sp(), from2.default$mcJ$sp());
         ...
         if (zz != ((Zero).MODULE$.implicitly(this.evidence$9$mcI$sp)).zero$mcI$sp()) {
            ...
            for(off2 = 0; off1 < from.activeSize(); ++off1) {
               while(off2 < from2.activeSize() && from2.indexAt(off2) < from.indexAt(off1)) {
                  ...
                  vb.add$mcI$sp(index, fn.apply$mcIJJ$sp(from.default$mcJ$sp(), from2.valueAt$mcJ$sp(off2)));
                  ++off2;
               }

               if (off2 < from2.activeSize() && from.indexAt(off1) == from2.indexAt(off2)) {
                  int index = from2.indexAt(off2);
                  vb.add$mcI$sp(index, fn.apply$mcIJJ$sp(from.valueAt$mcJ$sp(off1), from2.valueAt$mcJ$sp(off2)));
                  ++off2;
               } else {
                  int index = from.indexAt(off1);
                  vb.add$mcI$sp(index, fn.apply$mcIJJ$sp(from.valueAt$mcJ$sp(off1), from2.default$mcJ$sp()));
               }
            }
   ...
   // $FF: synthetic method
   public SparseVectorExpandOps breeze$linalg$operators$SparseVectorExpandOps$CanZipMapValuesSparseVector$mcIJ$sp$$$outer() {
      return this.$outer;
   }

   public SparseVectorExpandOps$CanZipMapValuesSparseVector$mcIJ$sp(final SparseVectorExpandOps $outer, final ClassTag evidence$8, final Zero evidence$9$mcI$sp, final Semiring evidence$10$mcI$sp) {
