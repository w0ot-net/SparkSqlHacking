...
public class SparseVectorExpandOps$CanZipMapValuesSparseVector$mcIF$sp extends SparseVectorExpandOps.CanZipMapValuesSparseVector implements CanZipMapValues$mcIF$sp {
   ...
   public SparseVector map$mcIF$sp(final SparseVector from, final SparseVector from2, final Function2 fn) {
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         int zz = BoxesRunTime.unboxToInt(fn.apply(BoxesRunTime.boxToFloat(from.default$mcF$sp()), BoxesRunTime.boxToFloat(from2.default$mcF$sp())));
         ...
         if (zz != ((Zero).MODULE$.implicitly(this.evidence$9$mcI$sp)).zero$mcI$sp()) {
            ...
            for(off2 = 0; off1 < from.activeSize(); ++off1) {
               while(off2 < from2.activeSize() && from2.indexAt(off2) < from.indexAt(off1)) {
                  ...
                  vb.add$mcI$sp(index, BoxesRunTime.unboxToInt(fn.apply(BoxesRunTime.boxToFloat(from.default$mcF$sp()), BoxesRunTime.boxToFloat(from2.valueAt$mcF$sp(off2)))));
                  ++off2;
               }

               if (off2 < from2.activeSize() && from.indexAt(off1) == from2.indexAt(off2)) {
                  int index = from2.indexAt(off2);
                  vb.add$mcI$sp(index, BoxesRunTime.unboxToInt(fn.apply(BoxesRunTime.boxToFloat(from.valueAt$mcF$sp(off1)), BoxesRunTime.boxToFloat(from2.valueAt$mcF$sp(off2)))));
                  ++off2;
               } else {
                  int index = from.indexAt(off1);
                  vb.add$mcI$sp(index, BoxesRunTime.unboxToInt(fn.apply(BoxesRunTime.boxToFloat(from.valueAt$mcF$sp(off1)), BoxesRunTime.boxToFloat(from2.default$mcF$sp()))));
               }
            }
   ...
   // $FF: synthetic method
   public SparseVectorExpandOps breeze$linalg$operators$SparseVectorExpandOps$CanZipMapValuesSparseVector$mcIF$sp$$$outer() {
      return this.$outer;
   }

   public SparseVectorExpandOps$CanZipMapValuesSparseVector$mcIF$sp(final SparseVectorExpandOps $outer, final ClassTag evidence$8, final Zero evidence$9$mcI$sp, final Semiring evidence$10$mcI$sp) {
