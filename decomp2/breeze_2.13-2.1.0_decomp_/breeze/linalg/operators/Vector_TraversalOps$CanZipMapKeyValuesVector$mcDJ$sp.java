...
public class Vector_TraversalOps$CanZipMapKeyValuesVector$mcDJ$sp extends Vector_TraversalOps.CanZipMapKeyValuesVector implements CanZipMapKeyValues$mcIDJ$sp {
   ...
   public Vector map$mcDJ$sp(final Vector from, final Vector from2, final Function3 fn) {
      ...
      for(int i = 0; i < from.length(); ++i) {
         result.data$mcD$sp()[i] = BoxesRunTime.unboxToDouble(fn.apply(BoxesRunTime.boxToInteger(i), BoxesRunTime.boxToLong(from.apply$mcIJ$sp(i)), BoxesRunTime.boxToLong(from2.apply$mcIJ$sp(i))));
      }
   ...
   // $FF: synthetic method
   public Vector_TraversalOps breeze$linalg$operators$Vector_TraversalOps$CanZipMapKeyValuesVector$mcDJ$sp$$$outer() {
      return this.$outer;
   }

   public Vector_TraversalOps$CanZipMapKeyValuesVector$mcDJ$sp(final Vector_TraversalOps $outer, final ClassTag evidence$4) {
      super(evidence$4);
