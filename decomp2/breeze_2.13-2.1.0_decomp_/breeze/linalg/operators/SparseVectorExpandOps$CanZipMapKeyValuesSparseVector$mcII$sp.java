...
public class SparseVectorExpandOps$CanZipMapKeyValuesSparseVector$mcII$sp extends SparseVectorExpandOps.CanZipMapKeyValuesSparseVector implements CanZipMapKeyValues$mcIII$sp {
   ...
   public SparseVector map$mcII$sp(final SparseVector from, final SparseVector from2, final Function3 fn) {
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         for(int limit$macro$6 = from.length(); index$macro$4 < limit$macro$6; ++index$macro$4) {
            result.update$mcI$sp(index$macro$4, BoxesRunTime.unboxToInt(fn.apply(BoxesRunTime.boxToInteger(index$macro$4), BoxesRunTime.boxToInteger(from.apply$mcI$sp(index$macro$4)), BoxesRunTime.boxToInteger(from2.apply$mcI$sp(index$macro$4)))));
         }
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         for(off2 = 0; off1 < from.activeSize(); ++off1) {
            while(off2 < from2.activeSize() && from2.indexAt(off2) < from.indexAt(off1)) {
               ...
               vb.add$mcI$sp(index, BoxesRunTime.unboxToInt(fn.apply(BoxesRunTime.boxToInteger(index), BoxesRunTime.boxToInteger(from.default$mcI$sp()), BoxesRunTime.boxToInteger(from2.valueAt$mcI$sp(off2)))));
               ++off2;
            }

            if (off2 < from2.activeSize() && from.indexAt(off1) == from2.indexAt(off2)) {
               int index = from2.indexAt(off2);
               vb.add$mcI$sp(index, BoxesRunTime.unboxToInt(fn.apply(BoxesRunTime.boxToInteger(index), BoxesRunTime.boxToInteger(from.valueAt$mcI$sp(off1)), BoxesRunTime.boxToInteger(from2.valueAt$mcI$sp(off2)))));
               ++off2;
            } else {
               int index = from.indexAt(off1);
               vb.add$mcI$sp(index, BoxesRunTime.unboxToInt(fn.apply(BoxesRunTime.boxToInteger(index), BoxesRunTime.boxToInteger(from.valueAt$mcI$sp(off1)), BoxesRunTime.boxToInteger(from2.default$mcI$sp()))));
            }
         }
   ...
   // $FF: synthetic method
   public SparseVectorExpandOps breeze$linalg$operators$SparseVectorExpandOps$CanZipMapKeyValuesSparseVector$mcII$sp$$$outer() {
      return this.$outer;
   }

   public SparseVectorExpandOps$CanZipMapKeyValuesSparseVector$mcII$sp(final SparseVectorExpandOps $outer, final ClassTag evidence$14, final Zero evidence$15$mcI$sp, final Semiring evidence$16$mcI$sp) {
