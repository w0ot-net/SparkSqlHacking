...
public class SparseVectorExpandOps$CanZipMapValuesSparseVector$mcII$sp extends SparseVectorExpandOps.CanZipMapValuesSparseVector implements CanZipMapValues$mcII$sp {
   ...
   public SparseVector map$mcII$sp(final SparseVector from, final SparseVector from2, final Function2 fn) {
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         int zz = fn.apply$mcIII$sp(from.default$mcI$sp(), from2.default$mcI$sp());
         ...
         if (zz != ((Zero).MODULE$.implicitly(this.evidence$9$mcI$sp)).zero$mcI$sp()) {
            ...
            for(off2 = 0; off1 < from.activeSize(); ++off1) {
               while(off2 < from2.activeSize() && from2.indexAt(off2) < from.indexAt(off1)) {
                  ...
                  vb.add$mcI$sp(index, fn.apply$mcIII$sp(from.default$mcI$sp(), from2.valueAt$mcI$sp(off2)));
                  ++off2;
               }

               if (off2 < from2.activeSize() && from.indexAt(off1) == from2.indexAt(off2)) {
                  int index = from2.indexAt(off2);
                  vb.add$mcI$sp(index, fn.apply$mcIII$sp(from.valueAt$mcI$sp(off1), from2.valueAt$mcI$sp(off2)));
                  ++off2;
               } else {
                  int index = from.indexAt(off1);
                  vb.add$mcI$sp(index, fn.apply$mcIII$sp(from.valueAt$mcI$sp(off1), from2.default$mcI$sp()));
               }
            }
   ...
   // $FF: synthetic method
   public SparseVectorExpandOps breeze$linalg$operators$SparseVectorExpandOps$CanZipMapValuesSparseVector$mcII$sp$$$outer() {
      return this.$outer;
   }

   public SparseVectorExpandOps$CanZipMapValuesSparseVector$mcII$sp(final SparseVectorExpandOps $outer, final ClassTag evidence$8, final Zero evidence$9$mcI$sp, final Semiring evidence$10$mcI$sp) {
