...
public class Vector_TraversalOps$CanZipMapKeyValuesVector$mcIJ$sp extends Vector_TraversalOps.CanZipMapKeyValuesVector implements CanZipMapKeyValues$mcIIJ$sp {
   ...
   public Vector map$mcIJ$sp(final Vector from, final Vector from2, final Function3 fn) {
      ...
      for(int i = 0; i < from.length(); ++i) {
         result.data$mcI$sp()[i] = BoxesRunTime.unboxToInt(fn.apply(BoxesRunTime.boxToInteger(i), BoxesRunTime.boxToLong(from.apply$mcIJ$sp(i)), BoxesRunTime.boxToLong(from2.apply$mcIJ$sp(i))));
      }
   ...
   // $FF: synthetic method
   public Vector_TraversalOps breeze$linalg$operators$Vector_TraversalOps$CanZipMapKeyValuesVector$mcIJ$sp$$$outer() {
      return this.$outer;
   }

   public Vector_TraversalOps$CanZipMapKeyValuesVector$mcIJ$sp(final Vector_TraversalOps $outer, final ClassTag evidence$4) {
      super(evidence$4);
