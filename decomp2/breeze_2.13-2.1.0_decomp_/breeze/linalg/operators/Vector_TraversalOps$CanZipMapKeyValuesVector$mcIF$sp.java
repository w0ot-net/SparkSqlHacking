...
public class Vector_TraversalOps$CanZipMapKeyValuesVector$mcIF$sp extends Vector_TraversalOps.CanZipMapKeyValuesVector implements CanZipMapKeyValues$mcIIF$sp {
   ...
   public Vector map$mcIF$sp(final Vector from, final Vector from2, final Function3 fn) {
      ...
      for(int i = 0; i < from.length(); ++i) {
         result.data$mcI$sp()[i] = BoxesRunTime.unboxToInt(fn.apply(BoxesRunTime.boxToInteger(i), BoxesRunTime.boxToFloat(from.apply$mcIF$sp(i)), BoxesRunTime.boxToFloat(from2.apply$mcIF$sp(i))));
      }
   ...
   // $FF: synthetic method
   public Vector_TraversalOps breeze$linalg$operators$Vector_TraversalOps$CanZipMapKeyValuesVector$mcIF$sp$$$outer() {
      return this.$outer;
   }

   public Vector_TraversalOps$CanZipMapKeyValuesVector$mcIF$sp(final Vector_TraversalOps $outer, final ClassTag evidence$4) {
      super(evidence$4);
