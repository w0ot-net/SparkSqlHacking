...
public class Vector_TraversalOps$CanZipMapKeyValuesVector$mcID$sp extends Vector_TraversalOps.CanZipMapKeyValuesVector implements CanZipMapKeyValues$mcIID$sp {
   ...
   public Vector map$mcID$sp(final Vector from, final Vector from2, final Function3 fn) {
      ...
      for(int i = 0; i < from.length(); ++i) {
         result.data$mcI$sp()[i] = BoxesRunTime.unboxToInt(fn.apply(BoxesRunTime.boxToInteger(i), BoxesRunTime.boxToDouble(from.apply$mcID$sp(i)), BoxesRunTime.boxToDouble(from2.apply$mcID$sp(i))));
      }
   ...
   // $FF: synthetic method
   public Vector_TraversalOps breeze$linalg$operators$Vector_TraversalOps$CanZipMapKeyValuesVector$mcID$sp$$$outer() {
      return this.$outer;
   }

   public Vector_TraversalOps$CanZipMapKeyValuesVector$mcID$sp(final Vector_TraversalOps $outer, final ClassTag evidence$4) {
      super(evidence$4);
