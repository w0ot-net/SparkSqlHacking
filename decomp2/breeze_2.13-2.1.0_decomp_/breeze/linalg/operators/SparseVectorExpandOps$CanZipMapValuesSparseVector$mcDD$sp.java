...
public class SparseVectorExpandOps$CanZipMapValuesSparseVector$mcDD$sp extends SparseVectorExpandOps.CanZipMapValuesSparseVector implements CanZipMapValues$mcDD$sp {
   ...
   public SparseVector map$mcDD$sp(final SparseVector from, final SparseVector from2, final Function2 fn) {
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         double zz = fn.apply$mcDDD$sp(from.default$mcD$sp(), from2.default$mcD$sp());
         ...
         if (zz != ((Zero).MODULE$.implicitly(this.evidence$9$mcD$sp)).zero$mcD$sp()) {
            ...
            for(int i = 0; i < from.length(); ++i) {
               result.update$mcD$sp(i, fn.apply$mcDDD$sp(from.apply$mcD$sp(i), from2.apply$mcD$sp(i)));
            }
            ...
            for(off2 = 0; off1 < from.activeSize(); ++off1) {
               while(off2 < from2.activeSize() && from2.indexAt(off2) < from.indexAt(off1)) {
                  ...
                  vb.add$mcD$sp(index, fn.apply$mcDDD$sp(from.default$mcD$sp(), from2.valueAt$mcD$sp(off2)));
                  ++off2;
               }

               if (off2 < from2.activeSize() && from.indexAt(off1) == from2.indexAt(off2)) {
                  int index = from2.indexAt(off2);
                  vb.add$mcD$sp(index, fn.apply$mcDDD$sp(from.valueAt$mcD$sp(off1), from2.valueAt$mcD$sp(off2)));
                  ++off2;
               } else {
                  int index = from.indexAt(off1);
                  vb.add$mcD$sp(index, fn.apply$mcDDD$sp(from.valueAt$mcD$sp(off1), from2.default$mcD$sp()));
               }
            }
   ...
   // $FF: synthetic method
   public SparseVectorExpandOps breeze$linalg$operators$SparseVectorExpandOps$CanZipMapValuesSparseVector$mcDD$sp$$$outer() {
      return this.$outer;
   }

   public SparseVectorExpandOps$CanZipMapValuesSparseVector$mcDD$sp(final SparseVectorExpandOps $outer, final ClassTag evidence$8, final Zero evidence$9$mcD$sp, final Semiring evidence$10$mcD$sp) {
