...
public class Vector_TraversalOps$CanZipMapKeyValuesVector$mcDF$sp extends Vector_TraversalOps.CanZipMapKeyValuesVector implements CanZipMapKeyValues$mcIDF$sp {
   ...
   public Vector map$mcDF$sp(final Vector from, final Vector from2, final Function3 fn) {
      ...
      for(int i = 0; i < from.length(); ++i) {
         result.data$mcD$sp()[i] = BoxesRunTime.unboxToDouble(fn.apply(BoxesRunTime.boxToInteger(i), BoxesRunTime.boxToFloat(from.apply$mcIF$sp(i)), BoxesRunTime.boxToFloat(from2.apply$mcIF$sp(i))));
      }
   ...
   // $FF: synthetic method
   public Vector_TraversalOps breeze$linalg$operators$Vector_TraversalOps$CanZipMapKeyValuesVector$mcDF$sp$$$outer() {
      return this.$outer;
   }

   public Vector_TraversalOps$CanZipMapKeyValuesVector$mcDF$sp(final Vector_TraversalOps $outer, final ClassTag evidence$4) {
      super(evidence$4);
