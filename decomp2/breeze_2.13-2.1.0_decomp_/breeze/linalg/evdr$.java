...
public final class evdr$ implements UFunc {
   public static final evdr$ MODULE$ = new evdr$();
   ...
   public eigSym.EigSym breeze$linalg$evdr$$doEigSymDouble(final DenseMatrix M, final int s, final int nOversamples, final int nIter) {
      .MODULE$.require(s <= scala.runtime.RichInt..MODULE$.min$extension(.MODULE$.intWrapper(M.rows()), M.cols()), () -> "Number of columns in orthonormal matrix should be less than min(M.rows, M.cols)");
      .MODULE$.require(s >= 1, () -> "Sketch size should be greater than 1");
      int nRandom = s + nOversamples;
      DenseMatrix Q = this.randomizedStateFinder(M, nRandom, nIter);
      DenseMatrix b = (DenseMatrix)((ImmutableNumericOps)Q.t(HasOps$.MODULE$.canTranspose_DM())).$times(M.$times(Q, HasOps$.MODULE$.impl_OpMulMatrix_DMD_DMD_eq_DMD()), HasOps$.MODULE$.impl_OpMulMatrix_DMD_DMD_eq_DMD());
      eig.Eig var10 = (eig.Eig)eig$.MODULE$.apply(b, eig.Eig_DM_Impl$.MODULE$);
      if (var10 != null) {
         DenseVector w = (DenseVector)var10.eigenvalues();
         DenseMatrix v = (DenseMatrix)var10.eigenvectors();
         Tuple2 var5 = new Tuple2(w, v);
         DenseVector w = (DenseVector)var5._1();
         DenseMatrix v = (DenseMatrix)var5._2();
         DenseMatrix _u = (DenseMatrix)Q.$times(v, HasOps$.MODULE$.impl_OpMulMatrix_DMD_DMD_eq_DMD());
         DenseMatrix u = this.flipSigns(_u);
         return new eigSym.EigSym(w, u);
      } else {
         throw new MatchError(var10);
      }
   }

   private int doEigSymDouble$default$3() {
      return 10;
   }

   private int doEigSymDouble$default$4() {
      return 0;
   }
   ...
   private DenseMatrix flipSigns(final DenseMatrix u) {
      ...
      IndexedSeq max_abs_cols = scala.runtime.RichInt..MODULE$.until$extension(.MODULE$.intWrapper(0), u.cols()).map((JFunction1.mcII.sp)(c) -> BoxesRunTime.unboxToInt(argmax$.MODULE$.apply(abs_u.apply(scala.package..MODULE$.$colon$colon(), BoxesRunTime.boxToInteger(c), HasOps$.MODULE$.canSliceCol()), argmax$.MODULE$.reduce_Double(HasOps$.MODULE$.DV_canTraverseKeyValuePairs()))));
      IndexedSeq signs = (IndexedSeq)((IndexedSeqOps)max_abs_cols.zipWithIndex()).map((e) -> BoxesRunTime.boxToDouble($anonfun$flipSigns$2(u, e)));
      ((IterableOnceOps)signs.zipWithIndex()).foreach((s) -> (DenseVector)((NumericOps)u.apply(scala.package..MODULE$.$colon$colon(), BoxesRunTime.boxToInteger(s._2$mcI$sp()), HasOps$.MODULE$.canSliceCol())).$colon$times$eq(BoxesRunTime.boxToDouble(s._1$mcD$sp()), HasOps$.MODULE$.impl_Op_InPlace_DV_S_Double_OpMulScalar()));
      return u;
   }

   // $FF: synthetic method
   public static final double $anonfun$flipSigns$2(final DenseMatrix u$1, final Tuple2 e) {
