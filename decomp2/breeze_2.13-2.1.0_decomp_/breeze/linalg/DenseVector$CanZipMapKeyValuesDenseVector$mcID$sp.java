...
public class DenseVector$CanZipMapKeyValuesDenseVector$mcID$sp extends DenseVector.CanZipMapKeyValuesDenseVector implements CanZipMapKeyValues$mcIID$sp {
   ...
   public DenseVector map$mcID$sp(final DenseVector from, final DenseVector from2, final Function3 fn) {
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         for(int i = 0; i < from.length(); ++i) {
            result.data$mcI$sp()[i] = BoxesRunTime.unboxToInt(fn.apply(BoxesRunTime.boxToInteger(i), BoxesRunTime.boxToDouble(from.apply$mcD$sp(i)), BoxesRunTime.boxToDouble(from2.apply$mcD$sp(i))));
         }
   ...
   public DenseVector$CanZipMapKeyValuesDenseVector$mcID$sp(final ClassTag evidence$17) {
      super(evidence$17);
