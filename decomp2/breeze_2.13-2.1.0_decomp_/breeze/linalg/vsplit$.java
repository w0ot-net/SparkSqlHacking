...
public final class vsplit$ implements UFunc {
   ...
   public UFunc.UImpl2 implIntMat(final ClassTag evidence$7, final Zero zero) {
      return new UFunc.UImpl2(evidence$7, zero) {
         ...
         public IndexedSeq apply(final DenseMatrix v, final int n) {
            boolean cond$macro$1 = n >= 0;
            if (!cond$macro$1) {
               throw new IllegalArgumentException("requirement failed: n.>=(0)");
            } else {
               boolean cond$macro$2 = n <= v.cols();
               if (!cond$macro$2) {
                  throw new IllegalArgumentException("requirement failed: n.<=(v.cols)");
               } else {
                  int left$macro$3 = v.cols() % n;
                  int right$macro$4 = 0;
                  if (left$macro$3 != 0) {
                     throw new IllegalArgumentException((new StringBuilder(54)).append("requirement failed: ").append("v.cols.%(n) == 0 (").append(left$macro$3).append(" ").append("!=").append(" ").append(0).append(")").toString());
                  } else {
                     Builder result = breeze.collection.compat.package$.MODULE$.arraySeqBuilder(.MODULE$.apply(DenseMatrix.class));
                     int newRows = v.rows() / n;
                     int index$macro$16 = 0;

                     for(int limit$macro$18 = n; index$macro$16 < limit$macro$18; ++index$macro$16) {
                        int offsetInOriginalMatrix = index$macro$16 * newRows;
                        DenseMatrix chunk = DenseMatrix$.MODULE$.create(newRows, v.cols(), this.evidence$7$1.newArray(v.cols() * newRows), this.zero$3);
                        int index$macro$11 = 0;

                        for(int limit$macro$13 = newRows; index$macro$11 < limit$macro$13; ++index$macro$11) {
                           int index$macro$6 = 0;

                           for(int limit$macro$8 = v.cols(); index$macro$6 < limit$macro$8; ++index$macro$6) {
                              ((i, j) -> chunk.update(i, j, v.apply(i + offsetInOriginalMatrix, j))).apply$mcVII$sp(index$macro$11, index$macro$6);
                           }
                        }

                        result.$plus$eq(chunk);
                     }

                     return (IndexedSeq)result.result();
                  }
               }
            }
         }
