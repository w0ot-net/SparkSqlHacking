...
public final class logdet$ implements UFunc {
   public static final logdet$ MODULE$ = new logdet$();
   ...
   public UFunc.UImpl canDetUsingLU(final UFunc.UImpl luImpl) {
      return new UFunc.UImpl(luImpl) {
         ...
         public Tuple2 apply(final Object X) {
            Tuple2 var4 = (Tuple2)LU.primitive$.MODULE$.apply(X, this.luImpl$1);
            if (var4 != null) {
               DenseMatrix m = (DenseMatrix)var4._1();
               int[] ipiv = (int[])var4._2();
               if (m != null && ipiv != null) {
                  Tuple2 var2 = new Tuple2(m, ipiv);
                  DenseMatrix mx = (DenseMatrix)var2._1();
                  int[] ipivx = (int[])var2._2();
                  int numExchangedRows = .MODULE$.count$extension(scala.Predef..MODULE$.refArrayOps((Object[]).MODULE$.zipWithIndex$extension(scala.Predef..MODULE$.intArrayOps((int[]).MODULE$.map$extension(scala.Predef..MODULE$.intArrayOps(ipivx), (JFunction1.mcII.sp)(x$2) -> x$2 - 1, scala.reflect.ClassTag..MODULE$.Int())))), (piv) -> BoxesRunTime.boxToBoolean($anonfun$apply$2(piv)));
                  double sign = numExchangedRows % 2 == 1 ? (double)-1.0F : (double)1.0F;
                  double acc = (double)0.0F;
                  int index$macro$2 = 0;

                  for(int limit$macro$4 = mx.rows(); index$macro$2 < limit$macro$4; ++index$macro$2) {
                     double mii = mx.apply$mcD$sp(index$macro$2, index$macro$2);
                     if (mii == (double)0.0F) {
                        return new Tuple2.mcDD.sp((double)0.0F, Double.NEGATIVE_INFINITY);
                     }

                     acc += scala.math.package..MODULE$.log(scala.math.package..MODULE$.abs(mii));
                     sign *= scala.math.package..MODULE$.signum(mii);
                  }

                  return new Tuple2.mcDD.sp(sign, acc);
               }
            }
         ...
         // $FF: synthetic method
         public static final boolean $anonfun$apply$2(final Tuple2 piv) {
            return piv._1$mcI$sp() != piv._2$mcI$sp();
         }

         public {
            this.luImpl$1 = luImpl$1;
         }
