...
public final class reverse$ implements UFunc {
   ...
   public UFunc.UImpl dvReverse(final ClassTag evidence$1) {
      return new UFunc.UImpl() {
         ...
         public DenseVector apply(final DenseVector dv) {
            return ((DenseVector)dv.apply(.MODULE$.to$extension(scala.Predef..MODULE$.intWrapper(dv.length() - 1), 0).by(-1), HasOps$.MODULE$.canSlice_DV_Range_eq_DV())).copy();
         }
      };
   }

   public UFunc.UImpl svReverse(final ClassTag evidence$2, final Zero evidence$3) {
      return new UFunc.UImpl(evidence$3) {
         ...
         public SparseVector apply(final SparseVector sv) {
            int[] nIndex = new int[sv.activeSize()];

            for(int i = 0; i < sv.activeSize(); ++i) {
               nIndex[i] = sv.length() - 1 - sv.index()[sv.activeSize() - 1 - i];
            }

            return new SparseVector(nIndex, scala.collection.ArrayOps..MODULE$.reverse$extension(scala.Predef..MODULE$.genericArrayOps(scala.collection.ArrayOps..MODULE$.take$extension(scala.Predef..MODULE$.genericArrayOps(sv.data()), sv.activeSize()))), sv.activeSize(), sv.length(), this.evidence$3$1);
         }
