...
public class DenseVector$CanZipMapValuesDenseVector$mcID$sp extends DenseVector.CanZipMapValuesDenseVector implements CanZipMapValues$mcID$sp {
   ...
   public DenseVector map$mcID$sp(final DenseVector from, final DenseVector from2, final Function2 fn) {
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         for(int i = 0; i < from.length(); ++i) {
            result.data$mcI$sp()[i] = fn.apply$mcIDD$sp(from.apply$mcD$sp(i), from2.apply$mcD$sp(i));
         }
   ...
   public DenseVector$CanZipMapValuesDenseVector$mcID$sp(final ClassTag evidence$15) {
      super(evidence$15);
