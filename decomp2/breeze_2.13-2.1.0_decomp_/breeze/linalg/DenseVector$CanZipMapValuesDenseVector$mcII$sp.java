...
public class DenseVector$CanZipMapValuesDenseVector$mcII$sp extends DenseVector.CanZipMapValuesDenseVector implements CanZipMapValues$mcII$sp {
   ...
   public DenseVector map$mcII$sp(final DenseVector from, final DenseVector from2, final Function2 fn) {
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         for(int i = 0; i < from.length(); ++i) {
            result.data$mcI$sp()[i] = fn.apply$mcIII$sp(from.apply$mcI$sp(i), from2.apply$mcI$sp(i));
         }
   ...
   public DenseVector$CanZipMapValuesDenseVector$mcII$sp(final ClassTag evidence$15) {
      super(evidence$15);
