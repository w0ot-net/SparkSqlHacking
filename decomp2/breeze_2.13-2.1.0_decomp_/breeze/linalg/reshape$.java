...
public final class reshape$ implements UFunc {
   ...
   public UFunc.UImpl3 svReshape(final ClassTag evidence$1, final Semiring evidence$2, final Zero evidence$3) {
      return new UFunc.UImpl3(evidence$1, evidence$2, evidence$3) {
         ...
         public CSCMatrix apply(final SparseVector sv, final int rows, final int cols) {
            int i = 0;

            CSCMatrix.Builder cscB;
            for(cscB = new CSCMatrix.Builder(rows, cols, sv.activeSize(), this.evidence$1$1, this.evidence$2$1, this.evidence$3$1); i < sv.activeSize(); ++i) {
               cscB.add(sv.index()[i] / cols, sv.index()[i] % cols, .MODULE$.array_apply(sv.data(), i));
            }

            return cscB.result(true, false);
         }
   ...
   public UFunc.UImpl3 dvReshape(final ClassTag evidence$4, final Semiring evidence$5, final Zero evidence$6) {
      return new UFunc.UImpl3(evidence$4) {
         ...
         public DenseMatrix apply(final DenseVector v, final int rows, final int cols) {
            scala.Predef..MODULE$.require(v.length() == rows * cols, () -> "Vector length must equal rows * cols to reshape.");
            return new DenseMatrix(rows, cols, v.toArray(this.evidence$4$1));
         }
   ...
   public UFunc.UImpl3 dmReshape(final ClassTag evidence$7, final Semiring evidence$8, final Zero evidence$9) {
      return new UFunc.UImpl3(evidence$7) {
         ...
         public DenseMatrix apply(final DenseMatrix dm, final int rows, final int cols) {
            scala.Predef..MODULE$.require(dm.rows() * dm.cols() == rows * cols, () -> scala.collection.StringOps..MODULE$.format$extension(scala.Predef..MODULE$.augmentString("Cannot reshape a (%d,%d) matrix to a (%d,%d) matrix!"), .MODULE$.genericWrapArray(new Object[]{BoxesRunTime.boxToInteger(dm.rows()), BoxesRunTime.boxToInteger(dm.cols()), BoxesRunTime.boxToInteger(rows), BoxesRunTime.boxToInteger(cols)})));
            DenseMatrix nDM = new DenseMatrix(dm.rows(), dm.cols(), this.evidence$7$1.newArray(dm.activeSize()));
            nDM.$colon$eq(dm, HasOps$.MODULE$.impl_OpMulSet_InPlace_DM_DM());
            return new DenseMatrix(rows, cols, nDM.data(), dm.offset(), dm.isTranspose() ? cols : rows, dm.isTranspose());
         }
   ...
   public UFunc.UImpl3 cscReshape(final ClassTag evidence$10, final Semiring evidence$11, final Zero evidence$12) {
      return new UFunc.UImpl3(evidence$10, evidence$12) {
         ...
         public CSCMatrix apply(final CSCMatrix csc, final int rows, final int cols) {
            scala.Predef..MODULE$.require(csc.rows() * csc.cols() == rows * cols, () -> "Size of matrix must match new dimensions (i.e. m.rows * m.cols == rows * cols");
            Object nData = this.evidence$10$1.newArray(csc.activeSize());
            System.arraycopy(csc.data(), 0, nData, 0, csc.activeSize());
            int[] rIndex = new int[csc.activeSize()];
            int[] cPtrs = new int[cols + 1];
            cPtrs[0] = 0;
            int nColInd = 1;
            int lastCol = 0;

            for(int c = 0; c < csc.cols(); ++c) {
               for(int ip = csc.colPtrs()[c]; ip < csc.colPtrs()[c + 1]; ++ip) {
                  int r = csc.rowIndices()[ip];
                  int dld = c * csc.rows() + r;
                  rIndex[ip] = dld % rows;
                  int nCol = dld / rows;
                  if (nCol > lastCol) {
                     while(nColInd <= nCol) {
            ...
            while(nColInd < cPtrs.length) {
               cPtrs[nColInd] = .MODULE$.array_length(nData);
               ++nColInd;
            }

            return new CSCMatrix(nData, rows, cols, cPtrs, csc.activeSize(), rIndex, this.evidence$12$1);
         }
