...
public final class mpow$ implements UFunc {
   public static final mpow$ MODULE$ = new mpow$();
   ...
   public DenseMatrix breeze$linalg$mpow$$powBySquaring(final DenseMatrix m, final int exp) {
      while(true) {
         DenseMatrix var10000;
         if (exp == 0) {
            var10000 = DenseMatrix$.MODULE$.eye$mDc$sp(m.rows(), .MODULE$.Double(), Zero$.MODULE$.DoubleZero(), Semiring$.MODULE$.semiringD());
         } else {
            if (exp < 0) {
               var10000 = (DenseMatrix)inv$.MODULE$.apply(m, inv$.MODULE$.canInvUsingLU_Double(LU$primitive$LU_DM_Impl_Double$.MODULE$));
               exp = -exp;
               m = var10000;
               continue;
            }

            DenseMatrix res_odds = DenseMatrix$.MODULE$.eye$mDc$sp(m.rows(), .MODULE$.Double(), Zero$.MODULE$.DoubleZero(), Semiring$.MODULE$.semiringD());
            DenseMatrix res = m;
            int n = exp;

            while(n > 1) {
               if (n % 2 == 0) {
                  res = (DenseMatrix)res.$times(res, HasOps$.MODULE$.impl_OpMulMatrix_DMD_DMD_eq_DMD());
                  n /= 2;
               } else {
                  res_odds = (DenseMatrix)res.$times(res_odds, HasOps$.MODULE$.impl_OpMulMatrix_DMD_DMD_eq_DMD());
                  res = (DenseMatrix)res.$times(res, HasOps$.MODULE$.impl_OpMulMatrix_DMD_DMD_eq_DMD());
                  n = (n - 1) / 2;
               }
            }

            var10000 = (DenseMatrix)res.$times(res_odds, HasOps$.MODULE$.impl_OpMulMatrix_DMD_DMD_eq_DMD());
         }
