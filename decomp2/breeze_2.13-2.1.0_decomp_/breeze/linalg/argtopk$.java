...
public final class argtopk$ implements UFunc, LowPriorityArgTopK {
   public static final argtopk$ MODULE$ = new argtopk$();

   static {
      UFunc.$init$(MODULE$);
      LowPriorityArgTopK.$init$(MODULE$);
   }

   public UFunc.UImpl2 argtopkWithQT(final .less.colon.less qt, final Ordering ord) {
      return LowPriorityArgTopK.argtopkWithQT$(this, qt, ord);
   }
   ...
   public UFunc.UImpl2 argtopkDenseVector(final Ordering evidence$1) {
      return new UFunc.UImpl2(evidence$1) {
         ...
         public IndexedSeq apply(final DenseVector v, final int k) {
            Ordering orderingInt = scala.package..MODULE$.Ordering().apply(this.evidence$1$1).on((x) -> $anonfun$apply$1(v, BoxesRunTime.unboxToInt(x))).reverse();
            int[] ints = ArrayUtil$.MODULE$.range(0, v.length(), ArrayUtil$.MODULE$.range$default$3());
            if (k != 0) {
               quickSelect$.MODULE$.implFromOrdering(scala..less.colon.less..MODULE$.refl(), orderingInt).apply$mcI$sp(scala.Predef..MODULE$.wrapIntArray(ints), k - 1);
            }

            return scala.Predef..MODULE$.copyArrayToImmutableIndexedSeq(scala.collection.ArrayOps..MODULE$.take$extension(scala.Predef..MODULE$.intArrayOps(ints), k));
         }

         // $FF: synthetic method
         public static final Object $anonfun$apply$1(final DenseVector v$1, final int x) {
            return v$1.apply(x);
         }
