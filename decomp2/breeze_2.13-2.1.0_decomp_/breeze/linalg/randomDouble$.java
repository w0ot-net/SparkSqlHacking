...
import breeze.storage.Zero$mcD$sp;
import java.lang.invoke.SerializedLambda;
import scala.reflect.ClassTag;
import scala.reflect.package.;
import scala.runtime.java8.JFunction1;

public final class randomDouble$ implements RandomGeneratorUFunc {
   public static final randomDouble$ MODULE$ = new randomDouble$();
   private static final ClassTag _classTag;
   private static final Zero _zero;

   static {
      UFunc.$init$(MODULE$);
      RandomGeneratorUFunc.$init$(MODULE$);
      _classTag = .MODULE$.classTag(scala.reflect.ClassTag..MODULE$.Double());
      _zero = new Zero$mcD$sp((double)0.0F);
   }

   public Object apply(final RandBasis basis) {
      return RandomGeneratorUFunc.apply$(this, basis);
   }

   public UFunc.UImpl implRandomT_1D(final RandBasis basis) {
      return RandomGeneratorUFunc.implRandomT_1D$(this, basis);
   }

   public UFunc.UImpl2 implRandomT_1DRange(final RandBasis basis) {
      return RandomGeneratorUFunc.implRandomT_1DRange$(this, basis);
   }

   public UFunc.UImpl implRandomT_2D(final RandBasis basis) {
      return RandomGeneratorUFunc.implRandomT_2D$(this, basis);
   }

   public UFunc.UImpl2 implRandomT_2DRange(final RandBasis basis) {
      return RandomGeneratorUFunc.implRandomT_2DRange$(this, basis);
   }
   ...
   public Rand gen(final RandBasis basis) {
      return basis.uniform();
   }

   public Rand genRange(final double low, final double high, final RandBasis basis) {
      boolean cond$macro$1 = high >= low;
      if (!cond$macro$1) {
         throw new IllegalArgumentException((new StringBuilder(34)).append("requirement failed: ").append((new StringBuilder(45)).append("High term must be greater than low term. (").append(low).append(", ").append(high).append(")").toString()).append(": ").append("high.>=(low)").toString());
      } else {
         double range = high - low;
         return basis.uniform().map$mcD$sp((JFunction1.mcDD.sp)(x$1) -> x$1 * range + low);
      }
   }

   public ClassTag _classTag() {
      return _classTag;
   }
