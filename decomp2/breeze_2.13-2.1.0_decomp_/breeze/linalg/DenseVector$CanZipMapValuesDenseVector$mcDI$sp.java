...
public class DenseVector$CanZipMapValuesDenseVector$mcDI$sp extends DenseVector.CanZipMapValuesDenseVector implements CanZipMapValues$mcDI$sp {
   ...
   public DenseVector map$mcDI$sp(final DenseVector from, final DenseVector from2, final Function2 fn) {
      ...
      if (left$macro$1 != right$macro$2) {
         ...
         for(int i = 0; i < from.length(); ++i) {
            result.data$mcD$sp()[i] = fn.apply$mcDII$sp(from.apply$mcI$sp(i), from2.apply$mcI$sp(i));
         }
   ...
   public DenseVector$CanZipMapValuesDenseVector$mcDI$sp(final ClassTag evidence$15) {
      super(evidence$15);
