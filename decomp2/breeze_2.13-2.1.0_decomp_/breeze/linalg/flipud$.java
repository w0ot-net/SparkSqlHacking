...
public final class flipud$ implements UFunc {
   public static final flipud$ MODULE$ = new flipud$();
   ...
   public UFunc.UImpl implDM() {
      return new UFunc.UImpl() {
         ...
         public DenseMatrix apply(final DenseMatrix v) {
            return v.isTranspose() ? ((DenseMatrix)v.apply(.MODULE$.to$extension(scala.Predef..MODULE$.intWrapper(v.cols() - 1), 0).by(-1), scala.package..MODULE$.$colon$colon(), HasOps$.MODULE$.canSliceRows())).copy() : ((DenseMatrix)((ImmutableNumericOps)fliplr$.MODULE$.apply(v.t(HasOps$.MODULE$.canTranspose_DM()), fliplr$.MODULE$.implDM())).t(HasOps$.MODULE$.canTranspose_DM())).copy();
         }
      };
   }

   public UFunc.UImpl implDV() {
      return new UFunc.UImpl() {
         public double apply$mcDD$sp(final double v) {
            return UFunc.UImpl.apply$mcDD$sp$(this, v);
         }
         ...
         public DenseVector apply(final DenseVector v) {
            return ((DenseVector)v.apply(.MODULE$.to$extension(scala.Predef..MODULE$.intWrapper(v.length() - 1), 0).by(-1), HasOps$.MODULE$.canSlice_DV_Range_eq_DV())).copy();
         }
      };
   }
