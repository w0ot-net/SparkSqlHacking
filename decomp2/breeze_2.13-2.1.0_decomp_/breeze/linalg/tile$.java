...
public final class tile$ implements UFunc {
   ...
   public UFunc.UImpl2 tile_DV_Impl2(final ClassTag ct, final Zero z, final UFunc.InPlaceImpl2 set) {
      return new UFunc.UImpl2(ct, z, set) {
         ...
         public DenseVector apply(final DenseVector x, final int n) {
            DenseVector out = DenseVector$.MODULE$.zeros(x.length() * n, this.ct$1, this.z$1);

            for(int i = 0; i < n; ++i) {
               ((NumericOps)out.apply(.MODULE$.until$extension(scala.Predef..MODULE$.intWrapper(i * x.length()), (i + 1) * x.length()), HasOps$.MODULE$.canSlice_DV_Range_eq_DV())).$colon$eq(x, this.set$1);
            }
   ...
   public UFunc.UImpl3 tile_DV_Impl3(final ClassTag evidence$1, final Zero evidence$2, final UFunc.UImpl2 impl2, final UFunc.InPlaceImpl2 set) {
      return new UFunc.UImpl3(impl2, evidence$1, evidence$2, set) {
         ...
         public DenseMatrix apply(final DenseVector x, final int m, final int n) {
            DenseVector col = (DenseVector)this.impl2$1.apply(x, BoxesRunTime.boxToInteger(m));
            DenseMatrix out = DenseMatrix$.MODULE$.zeros(x.length() * m, n, this.evidence$1$1, this.evidence$2$1);

            for(int i = 0; i < n; ++i) {
               ((NumericOps)out.apply(scala.package..MODULE$.$colon$colon(), BoxesRunTime.boxToInteger(i), HasOps$.MODULE$.canSliceCol())).$colon$eq(col, this.set$2);
            }
   ...
   public UFunc.UImpl2 tile_DM_Impl2(final ClassTag evidence$3, final Zero evidence$4, final UFunc.InPlaceImpl2 set) {
      return new UFunc.UImpl2(evidence$3, evidence$4, set) {
         ...
         public DenseMatrix apply(final DenseMatrix x, final int m) {
            DenseMatrix out = DenseMatrix$.MODULE$.zeros(x.rows() * m, x.cols(), this.evidence$3$1, this.evidence$4$1);

            for(int i = 0; i < x.cols(); ++i) {
               DenseVector column = (DenseVector)x.apply(scala.package..MODULE$.$colon$colon(), BoxesRunTime.boxToInteger(i), HasOps$.MODULE$.canSliceCol());

               for(int j = 0; j < m; ++j) {
                  ((NumericOps)out.apply(.MODULE$.until$extension(scala.Predef..MODULE$.intWrapper(j * x.rows()), (j + 1) * x.rows()), BoxesRunTime.boxToInteger(i), HasOps$.MODULE$.canSlicePartOfCol())).$colon$eq(column, this.set$3);
               }
            }
   ...
   public UFunc.UImpl3 tile_DM_Impl3(final ClassTag evidence$5, final Zero evidence$6, final UFunc.UImpl2 impl2, final UFunc.InPlaceImpl2 set) {
      return new UFunc.UImpl3(impl2, evidence$5, evidence$6, set) {
         ...
         public DenseMatrix apply(final DenseMatrix x, final int m, final int n) {
            DenseMatrix columnTiledMatrix = (DenseMatrix)this.impl2$2.apply(x, BoxesRunTime.boxToInteger(m));
            if (n == 1) {
               return columnTiledMatrix;
            } else {
               DenseMatrix out = DenseMatrix$.MODULE$.zeros(x.rows() * m, x.cols() * n, this.evidence$5$1, this.evidence$6$1);

               for(int i = 0; i < n; ++i) {
                  ((NumericOps)out.apply(scala.package..MODULE$.$colon$colon(), .MODULE$.until$extension(scala.Predef..MODULE$.intWrapper(i * x.cols()), (i + 1) * x.cols()), HasOps$.MODULE$.canSliceCols())).$colon$eq(columnTiledMatrix, this.set$4);
               }
