...
public final class unique$ implements UFunc {
   ...
   public UFunc.UImpl impl() {
      return new UFunc.UImpl() {
         ...
         public DenseVector apply(final DenseVector v) {
            ClassTag ct = ReflectionUtil$.MODULE$.elemClassTagFromArray(v.data());
            DenseVector var10000;
            if (v.size() == 0) {
               var10000 = DenseVector$.MODULE$.apply(ct.newArray(0));
            } else {
               Object data = v.toArray(ct);
               ArrayUtil$.MODULE$.sort(data);
               int elementCount = 1;
               Object lastElement = .MODULE$.array_apply(data, 0);
               int index$macro$2 = 0;

               for(int limit$macro$4 = .MODULE$.array_length(data); index$macro$2 < limit$macro$4; ++index$macro$2) {
                  Object di = .MODULE$.array_apply(data, index$macro$2);
                  if (!BoxesRunTime.equals(di, lastElement)) {
                     ++elementCount;
                     lastElement = di;
                  }
               }

               Object result = ct.newArray(elementCount);
               .MODULE$.array_update(result, 0, .MODULE$.array_apply(data, 0));
               lastElement = .MODULE$.array_apply(data, 0);
               int idx = 1;
               int index$macro$7 = 0;

               for(int limit$macro$9 = .MODULE$.array_length(data); index$macro$7 < limit$macro$9; ++index$macro$7) {
                  Object di = .MODULE$.array_apply(data, index$macro$7);
                  if (!BoxesRunTime.equals(di, lastElement)) {
                     .MODULE$.array_update(result, idx, di);
                     lastElement = di;
                     ++idx;
                  }
               }

               var10000 = DenseVector$.MODULE$.apply(result);
            }
