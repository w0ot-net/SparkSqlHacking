...
public final class corrcoeff$ implements UFunc {
   ...
   public UFunc.UImpl matrixCorrelation(final UFunc.UImpl covarianceCalculator) {
      return new UFunc.UImpl(covarianceCalculator) {
         ...
         public DenseMatrix apply(final Object data) {
            DenseMatrix covariance = (DenseMatrix)this.covarianceCalculator$1.apply(data);
            double[] d = new double[covariance.rows()];
            int index$macro$2 = 0;

            for(int limit$macro$4 = covariance.rows(); index$macro$2 < limit$macro$4; ++index$macro$2) {
               d[index$macro$2] = .MODULE$.sqrt(covariance.apply$mcD$sp(index$macro$2, index$macro$2));
            }

            int index$macro$12 = 0;

            for(int limit$macro$14 = covariance.rows(); index$macro$12 < limit$macro$14; ++index$macro$12) {
               int index$macro$7 = 0;

               for(int limit$macro$9 = covariance.rows(); index$macro$7 < limit$macro$9; ++index$macro$7) {
                  if (index$macro$12 != index$macro$7) {
                     covariance.update$mcD$sp(index$macro$12, index$macro$7, covariance.apply$mcD$sp(index$macro$12, index$macro$7) / (d[index$macro$12] * d[index$macro$7]));
                  } else {
                     covariance.update$mcD$sp(index$macro$12, index$macro$7, (double)1.0F);
                  }
               }
            }
         ...
         public {
            this.covarianceCalculator$1 = covarianceCalculator$1;
         }
      };
   }
