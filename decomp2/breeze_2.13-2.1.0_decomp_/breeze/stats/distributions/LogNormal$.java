...
public final class LogNormal$ implements ExponentialFamily, ContinuousDistributionUFuncProvider, Serializable {
   ...
   public Gaussian.SufficientStatistic sufficientStatisticFor(final double t) {
      return new Gaussian.SufficientStatistic((double)1.0F, .MODULE$.log(t), (double)0.0F);
   }
   ...
   public LogNormal distribution(final Tuple2 p, final RandBasis rand) {
      return new LogNormal(p._1$mcD$sp(), .MODULE$.sqrt(p._2$mcD$sp()), rand);
   }
   ...
   public LogNormal apply(final double mu, final double sigma, final RandBasis rand) {
      return new LogNormal(mu, sigma, rand);
   }

   public Option unapply(final LogNormal x$0) {
