...
public final class Bernoulli$ implements HasConjugatePrior, Serializable {
   ...
   public Polya predictive(final Tuple2 parameter, final RandBasis basis) {
      return new Polya(Counter$.MODULE$.apply((Seq).MODULE$.wrapRefArray((Object[])(new Tuple2[]{scala.Predef.ArrowAssoc..MODULE$.$minus$greater$extension(scala.Predef..MODULE$.ArrowAssoc(BoxesRunTime.boxToBoolean(true)), BoxesRunTime.boxToDouble(parameter._1$mcD$sp())), scala.Predef.ArrowAssoc..MODULE$.$minus$greater$extension(scala.Predef..MODULE$.ArrowAssoc(BoxesRunTime.boxToBoolean(false)), BoxesRunTime.boxToDouble(parameter._2$mcD$sp()))})), Zero$.MODULE$.DoubleZero(), Semiring$.MODULE$.semiringD()), Counter$.MODULE$.space(Field.fieldDouble$.MODULE$), basis);
   }

   public Tuple2 posterior(final Tuple2 prior, final IterableOnce evidence) {
      return (Tuple2)scala.collection.IterableOnceExtensionMethods..MODULE$.foldLeft$extension(scala.collection.IterableOnce..MODULE$.iterableOnceExtensionMethods(evidence), prior, (acc, ev) -> $anonfun$posterior$1(acc, BoxesRunTime.unboxToBoolean(ev)));
   }

   public Bernoulli.SufficientStatistic emptySufficientStatistic() {
      return new Bernoulli.SufficientStatistic((double)0.0F, (double)0.0F);
   }

   public Bernoulli.SufficientStatistic sufficientStatisticFor(final boolean t) {
      return new Bernoulli.SufficientStatistic(BoxesRunTime.unboxToDouble(package.I$.MODULE$.apply(BoxesRunTime.boxToBoolean(t), package$I$iBoolImpl$.MODULE$)), (double)1.0F);
   }

   public double mle(final Bernoulli.SufficientStatistic stats) {
      return stats.numYes() / stats.n();
   }

   public Bernoulli distribution(final double p, final RandBasis rand) {
      return new Bernoulli(p, rand);
   }

   public DiffFunction likelihoodFunction(final Bernoulli.SufficientStatistic stats) {
      return new DiffFunction(stats) {
         ...
         public Tuple2 calculate(final double p) {
            double obj = this.yes() * scala.math.package..MODULE$.log(p) + this.no() * scala.math.package..MODULE$.log1p(-p);
            double grad = this.yes() / p - this.no() / ((double)1 - p);
         ...
         public {
            ...
            if (stats$1 != null) {
               double yes = stats$1.numYes();
               double num = stats$1.n();
               Tuple2.mcDD.sp var2 = new Tuple2.mcDD.sp(yes, num);
               this.x$1 = var2;
               this.yes = this.x$1._1$mcD$sp();
               this.num = this.x$1._2$mcD$sp();
               this.no = this.num() - this.yes();
            } else {
   ...
   public Bernoulli apply(final double p, final RandBasis rand) {
      return new Bernoulli(p, rand);
   }

   public Option unapply(final Bernoulli x$0) {
   ...
   // $FF: synthetic method
   public static final Tuple2 $anonfun$posterior$1(final Tuple2 acc, final boolean ev) {
      Tuple2 var10000;
      if (ev) {
         var10000 = acc.copy$mDDc$sp(acc._1$mcD$sp() + (double)1, BoxesRunTime.unboxToDouble(acc.copy$default$2()));
      } else {
         double x$1 = acc._2$mcD$sp() + (double)1;
         double x$2 = BoxesRunTime.unboxToDouble(acc.copy$default$1());
         var10000 = acc.copy$mDDc$sp(x$2, x$1);
      }
