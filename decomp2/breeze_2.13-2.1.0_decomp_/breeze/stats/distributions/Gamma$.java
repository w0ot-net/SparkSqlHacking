...
import breeze.numerics.package$trigamma$trigammaImplDouble$;
...
public final class Gamma$ implements ExponentialFamily, ContinuousDistributionUFuncProvider, Serializable {
   ...
   public Gamma.SufficientStatistic emptySufficientStatistic() {
      return new Gamma.SufficientStatistic((double)0.0F, (double)0.0F, (double)0.0F);
   }

   public Gamma.SufficientStatistic sufficientStatisticFor(final double t) {
      return new Gamma.SufficientStatistic((double)1.0F, .MODULE$.log(t), t);
   }

   public Tuple2 mle(final Gamma.SufficientStatistic ss) {
      double s = .MODULE$.log(ss.mean()) - ss.meanOfLogs();
      scala.Predef..MODULE$.assert(s > (double)0, (JFunction0.mcD.sp)() -> s);
      double k_approx = this.approx_k(s);
      scala.Predef..MODULE$.assert(k_approx > (double)0, (JFunction0.mcD.sp)() -> k_approx);
      double k = this.Nwt_Rph_iter_for_k(k_approx, s, this.Nwt_Rph_iter_for_k$default$3());
      double theta = ss.mean() / k;
      return new Tuple2.mcDD.sp(k, theta);
   }

   public double approx_k(final double s) {
      return ((double)3 - s + .MODULE$.sqrt(.MODULE$.pow(s - (double)3, (double)2.0F) + (double)24 * s)) / ((double)12 * s);
   }
   ...
   private double Nwt_Rph_iter_for_k(final double k, final double s, final int iter) {
      while(true) {
         double k_new = k - (.MODULE$.log(k) - package.digamma$.MODULE$.apply$mDDc$sp(k, package$digamma$digammaImplDouble$.MODULE$) - s) / ((double)1.0F / k - package.trigamma$.MODULE$.apply$mDDc$sp(k, package$trigamma$trigammaImplDouble$.MODULE$));
         if (package$.MODULE$.closeTo(k, k_new, package$.MODULE$.closeTo$default$3()) || iter >= this.MaxIter()) {
            return k_new;
         }
   ...
   private int Nwt_Rph_iter_for_k$default$3() {
      return 0;
   }

   public Gamma distribution(final Tuple2 p, final RandBasis rand) {
      return new Gamma(p._1$mcD$sp(), p._2$mcD$sp(), rand);
   }

   public DiffFunction likelihoodFunction(final Gamma.SufficientStatistic stats) {
      return new DiffFunction(stats) {
         ...
         public Tuple2 calculate(final Tuple2 x) {
            if (x != null) {
               ...
               double obj = -this.n() * ((a - (double)1) * this.meanOfLogs() - package.lgamma$.MODULE$.apply$mDDc$sp(a, package$lgamma$lgammaImplDouble$.MODULE$) - a * package.log$.MODULE$.apply$mDDc$sp(b, package$log$logDoubleImpl$.MODULE$) - this.mean() / b);
               double gradA = -this.n() * (this.meanOfLogs() - package.digamma$.MODULE$.apply$mDDc$sp(a, package$digamma$digammaImplDouble$.MODULE$) - package.log$.MODULE$.apply$mDDc$sp(b, package$log$logDoubleImpl$.MODULE$));
               double gradB = -this.n() * (-a / b + this.mean() / b / b);
               return new Tuple2(BoxesRunTime.boxToDouble(obj), new Tuple2.mcDD.sp(gradA, gradB));
            } else {
         ...
         public {
            ...
            if (stats$1 != null) {
               double n = stats$1.n();
               double meanOfLogs = stats$1.meanOfLogs();
               double mean = stats$1.mean();
               Tuple3 var2 = new Tuple3(BoxesRunTime.boxToDouble(n), BoxesRunTime.boxToDouble(meanOfLogs), BoxesRunTime.boxToDouble(mean));
               this.x$1 = var2;
               this.n = BoxesRunTime.unboxToDouble(this.x$1._1());
               this.meanOfLogs = BoxesRunTime.unboxToDouble(this.x$1._2());
               this.mean = BoxesRunTime.unboxToDouble(this.x$1._3());
            } else {
               throw new MatchError(stats$1);
            }
         }
      };
   }

   public Gamma apply(final double shape, final double scale, final RandBasis rand) {
      return new Gamma(shape, scale, rand);
   }

   public Option unapply(final Gamma x$0) {
      return (Option)(x$0 == null ? scala.None..MODULE$ : new Some(new Tuple2.mcDD.sp(x$0.shape(), x$0.scale())));
   }
