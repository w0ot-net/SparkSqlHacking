...
public final class Gaussian$ implements ExponentialFamily, ContinuousDistributionUFuncProvider, Serializable {
   public static final Gaussian$ MODULE$ = new Gaussian$();
   private static final double breeze$stats$distributions$Gaussian$$sqrt2;
   ...
   static {
      ...
      breeze$stats$distributions$Gaussian$$sqrt2 = .MODULE$.sqrt((double)2.0F);
      emptySufficientStatistic = new Gaussian.SufficientStatistic((double)0.0F, (double)0.0F, (double)0.0F);
   }
   ...
   public double breeze$stats$distributions$Gaussian$$sqrt2() {
      return breeze$stats$distributions$Gaussian$$sqrt2;
   }
   ...
   public Gaussian.SufficientStatistic sufficientStatisticFor(final double t) {
      return new Gaussian.SufficientStatistic((double)1.0F, t, (double)0.0F);
   }

   public Tuple2 mle(final Gaussian.SufficientStatistic stats) {
      return new Tuple2.mcDD.sp(stats.mean(), stats.variance());
   }

   public Gaussian distribution(final Tuple2 p, final RandBasis rand) {
      return new Gaussian(p._1$mcD$sp(), .MODULE$.sqrt(p._2$mcD$sp()), rand);
   }

   public DiffFunction likelihoodFunction(final Gaussian.SufficientStatistic stats) {
      return new DiffFunction(stats) {
         private final double normPiece;
         private final Gaussian.SufficientStatistic stats$1;
         ...
         private double normPiece() {
            return this.normPiece;
         }

         public Tuple2 calculate(final Tuple2 x) {
            if (x != null) {
               double mu = x._1$mcD$sp();
               double sigma2 = x._2$mcD$sp();
               Tuple2.mcDD.sp var3 = new Tuple2.mcDD.sp(mu, sigma2);
               double mux = ((Tuple2)var3)._1$mcD$sp();
               double sigma2 = ((Tuple2)var3)._2$mcD$sp();
               Gaussian.SufficientStatistic var15 = this.stats$1;
               if (var15 != null) {
                  double n = var15.n();
                  double mean = var15.mean();
                  Tuple2.mcDD.sp var2 = new Tuple2.mcDD.sp(n, mean);
                  double nx = ((Tuple2)var2)._1$mcD$sp();
                  double mean = ((Tuple2)var2)._2$mcD$sp();
                  double variance = this.stats$1.variance();
                  Tuple2 var10000;
                  if (sigma2 <= (double)0) {
                     var10000 = new Tuple2(BoxesRunTime.boxToDouble(Double.POSITIVE_INFINITY), new Tuple2.mcDD.sp(Double.NaN, Double.NaN));
                  } else {
                     double objective = nx * ((variance + mean * mean) / sigma2 / (double)2 - mean * mux / sigma2 + mux * mux / sigma2 / (double)2 + (double)0.5F * (.MODULE$.log(sigma2) + this.normPiece()));
                     double gradientMu = nx * (-mean / sigma2 + mux / sigma2);
                     double gradientSig = nx * (-(variance + mean * mean) / sigma2 / sigma2 / (double)2 + mean * mux / sigma2 / sigma2 - mux * mux / sigma2 / sigma2 / (double)2 + (double)0.5F / sigma2);
                     var10000 = new Tuple2(BoxesRunTime.boxToDouble(objective), new Tuple2.mcDD.sp(gradientMu, gradientSig));
                  }
         ...
         public {
            ...
            this.normPiece = .MODULE$.log((Math.PI * 2D));
         }
      };
   }

   public Gaussian apply(final double mu, final double sigma, final RandBasis rand) {
      return new Gaussian(mu, sigma, rand);
   }

   public Option unapply(final Gaussian x$0) {
      return (Option)(x$0 == null ? scala.None..MODULE$ : new Some(new Tuple2.mcDD.sp(x$0.mu(), x$0.sigma())));
   }
