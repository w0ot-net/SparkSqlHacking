...
public final class ChiSquared$ implements ExponentialFamily, ContinuousDistributionUFuncProvider, Serializable {
   public static final ChiSquared$ MODULE$ = new ChiSquared$();
   ...
   public DiffFunction likelihoodFunction(final Gamma.SufficientStatistic stats) {
      DiffFunction inner = Gamma$.MODULE$.likelihoodFunction(stats);
      return new DiffFunction(inner) {
         ...
         public Tuple2 calculate(final double x) {
            Tuple2 var5 = this.inner$1.calculate(new Tuple2.mcDD.sp(x / (double)2.0F, (double)2.0F));
            if (var5 != null) {
               double obj = var5._1$mcD$sp();
               Tuple2 ggrad = (Tuple2)var5._2();
               Tuple2 var3 = new Tuple2(BoxesRunTime.boxToDouble(obj), ggrad);
               double objx = var3._1$mcD$sp();
               Tuple2 ggrad = (Tuple2)var3._2();
               return .MODULE$.$minus$greater$extension(scala.Predef..MODULE$.ArrowAssoc(BoxesRunTime.boxToDouble(objx)), BoxesRunTime.boxToDouble(ggrad._1$mcD$sp()));
            } else {
   ...
   public double mle(final Gamma.SufficientStatistic ss) {
      return ss.mean();
   }

   public ChiSquared distribution(final double p, final RandBasis rand) {
      return new ChiSquared(p, rand);
   }

   public ChiSquared apply(final double k, final RandBasis rand) {
      return new ChiSquared(k, rand);
   }

   public Option unapply(final ChiSquared x$0) {
      return (Option)(x$0 == null ? scala.None..MODULE$ : new Some(BoxesRunTime.boxToDouble(x$0.k())));
   }

   private Object writeReplace() {
      return new ModuleSerializationProxy(ChiSquared$.class);
   }
