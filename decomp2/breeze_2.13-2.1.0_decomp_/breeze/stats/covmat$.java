...
public final class covmat$ implements UFunc {
   public static final covmat$ MODULE$ = new covmat$();
   private static final UFunc.UImpl matrixCovariance;
   private static final UFunc.UImpl sequenceCovariance;

   static {
      UFunc.$init$(MODULE$);
      matrixCovariance = new UFunc.UImpl() {
         ...
         public DenseMatrix apply(final DenseMatrix data) {
            return package$.MODULE$.cov(data, package$.MODULE$.cov$default$2());
         }
      };
      sequenceCovariance = new UFunc.UImpl() {
         public double apply$mcDD$sp(final double v) {
            return UFunc.UImpl.apply$mcDD$sp$(this, v);
         }
         ...
         public DenseMatrix apply(final Seq data) {
            return (DenseMatrix)data.headOption().map((firstRow) -> {
               DenseMatrix result = new DenseMatrix$mcD$sp(firstRow.size(), firstRow.size(), .MODULE$.Double());
               int dataSize = firstRow.size();
               ObjectRef mean = ObjectRef.create(firstRow.copy$mcD$sp());
               LongRef numRows = LongRef.create(1L);
               ((IterableOnceOps)data.tail()).foreach((x) -> {
                  $anonfun$apply$2(numRows, mean, firstRow, x);
                  return BoxedUnit.UNIT;
               });
               double numRowsD = (double)numRows.elem;
               mean.elem = (DenseVector)((DenseVector)mean.elem).$div(BoxesRunTime.boxToDouble(numRowsD), HasOps$.MODULE$.impl_Op_DV_S_eq_DV_Double_OpDiv());
               data.foreach((x) -> {
                  $anonfun$apply$3(dataSize, mean, result, numRowsD, x);
                  return BoxedUnit.UNIT;
               });
               return result;
            }).getOrElse(() -> new DenseMatrix$mcD$sp(0, 0, .MODULE$.Double()));
         }

         // $FF: synthetic method
         public static final void $anonfun$apply$2(final LongRef numRows$1, final ObjectRef mean$1, final DenseVector firstRow$1, final DenseVector x) {
            ++numRows$1.elem;
            if (((DenseVector)mean$1.elem).size() != x.size()) {
               throw new IllegalArgumentException("Attempting to compute covariance of dataset where elements have different sizes");
            } else {
               int index$macro$2 = 0;

               for(int limit$macro$4 = firstRow$1.size(); index$macro$2 < limit$macro$4; ++index$macro$2) {
                  ((DenseVector)mean$1.elem).update$mcD$sp(index$macro$2, ((DenseVector)mean$1.elem).apply$mcD$sp(index$macro$2) + x.apply$mcD$sp(index$macro$2));
               }
         ...
         // $FF: synthetic method
         public static final void $anonfun$apply$3(final int dataSize$1, final ObjectRef mean$1, final DenseMatrix result$1, final double numRowsD$1, final DenseVector x) {
            int index$macro$12 = 0;

            for(int limit$macro$14 = dataSize$1; index$macro$12 < limit$macro$14; ++index$macro$12) {
               double a = x.apply$mcD$sp(index$macro$12) - ((DenseVector)mean$1.elem).apply$mcD$sp(index$macro$12);
               int index$macro$7 = 0;

               for(int limit$macro$9 = dataSize$1; index$macro$7 < limit$macro$9; ++index$macro$7) {
                  double b = x.apply$mcD$sp(index$macro$7) - ((DenseVector)mean$1.elem).apply$mcD$sp(index$macro$7);
                  result$1.update$mcD$sp(index$macro$12, index$macro$7, result$1.apply$mcD$sp(index$macro$12, index$macro$7) + a * b / (numRowsD$1 - (double)1));
               }
            }
   ...
   public UFunc.UImpl matrixCovariance() {
      return matrixCovariance;
   }

   public UFunc.UImpl sequenceCovariance() {
