...
public final class JwtRsaSsaPssPublicKey extends JwtSignaturePublicKey {
   ...
   private JwtRsaSsaPssPublicKey(JwtRsaSsaPssParameters parameters, BigInteger modulus, Optional idRequirement, Optional kid) {
   ...
   public static class Builder {
      ...
      private Optional computeKid() throws GeneralSecurityException {
         if (((JwtRsaSsaPssParameters)this.parameters.get()).getKidStrategy().equals(JwtRsaSsaPssParameters.KidStrategy.BASE64_ENCODED_KEY_ID)) {
         ...
         } else if (((JwtRsaSsaPssParameters)this.parameters.get()).getKidStrategy().equals(JwtRsaSsaPssParameters.KidStrategy.CUSTOM)) {
            if (!this.customKid.isPresent()) {
               throw new GeneralSecurityException("customKid needs to be set for KidStrategy CUSTOM");
            } else {
               return this.customKid;
            }
         } else if (((JwtRsaSsaPssParameters)this.parameters.get()).getKidStrategy().equals(JwtRsaSsaPssParameters.KidStrategy.IGNORED)) {
            if (this.customKid.isPresent()) {
      ...
      public JwtRsaSsaPssPublicKey build() throws GeneralSecurityException {
         if (!this.parameters.isPresent()) {
            ...
            int paramModulusSize = ((JwtRsaSsaPssParameters)this.parameters.get()).getModulusSizeBits();
            ...
            } else if (((JwtRsaSsaPssParameters)this.parameters.get()).hasIdRequirement() && !this.idRequirement.isPresent()) {
               throw new GeneralSecurityException("Cannot create key without ID requirement with parameters with ID requirement");
            } else if (!((JwtRsaSsaPssParameters)this.parameters.get()).hasIdRequirement() && this.idRequirement.isPresent()) {
               throw new GeneralSecurityException("Cannot create key with ID requirement with parameters without ID requirement");
            } else {
               return new JwtRsaSsaPssPublicKey((JwtRsaSsaPssParameters)this.parameters.get(), (BigInteger)this.modulus.get(), this.idRequirement, this.computeKid());
            }
         }
      }
   }
