...
public class AddressResolverOptionsConverter {
   ...
   static void fromJson(Iterable json, AddressResolverOptions obj) {
      for(Map.Entry member : json) {
         switch ((String)member.getKey()) {
            ...
            case "hostsValue":
               if (member.getValue() instanceof String) {
                  obj.setHostsValue(Buffer.buffer(BASE64_DECODER.decode((String)member.getValue())));
               }
               break;
            case "maxQueries":
               if (member.getValue() instanceof Number) {
   ...
   static void toJson(AddressResolverOptions obj, Map json) {
