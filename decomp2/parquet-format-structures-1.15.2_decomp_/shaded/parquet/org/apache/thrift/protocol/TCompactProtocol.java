...
public class TCompactProtocol extends TProtocol {
   ...
   public void writeUuid(UUID uuid) throws TException {
      this.fixedLongToBytes(uuid.getLeastSignificantBits(), this.temp, 0);
      this.fixedLongToBytes(uuid.getMostSignificantBits(), this.temp, 8);
   ...
   public UUID readUuid() throws TException {
      ...
      long mostSigBits = this.bytesToLong(this.temp, 8);
      long leastSigBits = this.bytesToLong(this.temp, 0);
   ...
   private long bytesToLong(byte[] bytes, int offset) {
      return ((long)bytes[offset + 7] & 255L) << 56 | ((long)bytes[offset + 6] & 255L) << 48 | ((long)bytes[offset + 5] & 255L) << 40 | ((long)bytes[offset + 4] & 255L) << 32 | ((long)bytes[offset + 3] & 255L) << 24 | ((long)bytes[offset + 2] & 255L) << 16 | ((long)bytes[offset + 1] & 255L) << 8 | (long)bytes[offset + 0] & 255L;
   }
   ...
   static {
      ...
      ttypeToCompactType = new byte[18];
