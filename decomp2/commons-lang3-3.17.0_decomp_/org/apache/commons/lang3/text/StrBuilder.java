...
public class StrBuilder implements CharSequence, Appendable, Serializable, Builder {
   ...
   public StrBuilder append(CharSequence seq) {
      if (seq == null) {
         return this.appendNull();
      } else if (seq instanceof StrBuilder) {
   ...
   public StrBuilder ensureCapacity(int capacity) {
      if (capacity > this.buffer.length) {
         this.buffer = (char[])ArrayUtils.arraycopy(this.buffer, 0, 0, this.size, (Supplier)(() -> new char[capacity * 2]));
      }
   ...
   public StrBuilder minimizeCapacity() {
      if (this.buffer.length > this.length()) {
         this.buffer = (char[])ArrayUtils.arraycopy(this.buffer, 0, 0, this.size, (Supplier)(() -> new char[this.length()]));
      }
   ...
   public char[] toCharArray() {
      return this.size == 0 ? ArrayUtils.EMPTY_CHAR_ARRAY : (char[])ArrayUtils.arraycopy(this.buffer, 0, 0, this.size, (Function)((x$0) -> new char[x$0]));
   }

   public char[] toCharArray(int startIndex, int endIndex) {
      endIndex = this.validateRange(startIndex, endIndex);
      int len = endIndex - startIndex;
      return len == 0 ? ArrayUtils.EMPTY_CHAR_ARRAY : (char[])ArrayUtils.arraycopy(this.buffer, startIndex, 0, len, (Function)((x$0) -> new char[x$0]));
   }
