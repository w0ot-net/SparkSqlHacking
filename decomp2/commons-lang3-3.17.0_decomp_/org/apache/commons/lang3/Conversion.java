   ...
   public static char binaryBeMsb0ToHexDigit(boolean[] src, int srcPos) {
      if (Integer.compareUnsigned(srcPos, src.length) >= 0) {
         ...
         if (3 <= pos && src[pos - 3]) {
            if (src[pos - 2]) {
               if (src[pos - 1]) {
                  return (char)(src[pos] ? 'f' : 'e');
               } else {
                  return (char)(src[pos] ? 'd' : 'c');
               }
            } else if (src[pos - 1]) {
               return (char)(src[pos] ? 'b' : 'a');
            } else {
               return (char)(src[pos] ? '9' : '8');
            }
         } else if (2 <= pos && src[pos - 2]) {
            if (src[pos - 1]) {
               return (char)(src[pos] ? '7' : '6');
            } else {
               return (char)(src[pos] ? '5' : '4');
            }
         } else if (1 <= pos && src[pos - 1]) {
            return (char)(src[pos] ? '3' : '2');
         } else {
            return (char)(src[pos] ? '1' : '0');
         }
      }
   }
