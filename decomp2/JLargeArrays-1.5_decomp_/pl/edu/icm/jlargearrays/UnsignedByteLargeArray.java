...
public class UnsignedByteLargeArray extends LargeArray {
   ...
   public UnsignedByteLargeArray clone() {
      if (this.isConstant) {
         return new UnsignedByteLargeArray(this.length, (short)this.getByte(0L));
      } else {
   ...
   public final int getInt(long i) {
      if (this.ptr != 0L) {
         return 255 & LargeArrayUtils.UNSAFE.getByte(this.ptr + i);
      } else {
         return this.isConstant ? 255 & this.data[0] : 255 & this.data[(int)i];
      }
   }

   public final long getLong(long i) {
      if (this.ptr != 0L) {
         return (long)(255 & LargeArrayUtils.UNSAFE.getByte(this.ptr + i));
      } else {
         return this.isConstant ? (long)(255 & this.data[0]) : (long)(255 & this.data[(int)i]);
      }
   }

   public final float getFloat(long i) {
      if (this.ptr != 0L) {
         return (float)(255 & LargeArrayUtils.UNSAFE.getByte(this.ptr + i));
      } else {
         return this.isConstant ? (float)(255 & this.data[0]) : (float)(255 & this.data[(int)i]);
      }
   }

   public final double getDouble(long i) {
      if (this.ptr != 0L) {
         return (double)(255 & LargeArrayUtils.UNSAFE.getByte(this.ptr + i));
      } else {
         return this.isConstant ? (double)(255 & this.data[0]) : (double)(255 & this.data[(int)i]);
      }
   }
         ...
         if (endPos >= 0L && endPos <= this.length && endPos >= startPos) {
            if (step < 1L) {
               ...
               if (len > 1073741824L) {
                  ...
                  if (this.ptr != 0L) {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = (short)(255 & LargeArrayUtils.UNSAFE.getByte(this.ptr + i));
                     }
                  } else if (this.isConstant) {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = (short)(255 & this.data[0]);
                     }
                  } else {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = (short)(255 & this.data[(int)i]);
                     }
                  }
         ...
         if (endPos >= 0L && endPos <= this.length && endPos >= startPos) {
            if (step < 1L) {
               ...
               if (len > 1073741824L) {
                  ...
                  if (this.ptr != 0L) {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = 255 & LargeArrayUtils.UNSAFE.getByte(this.ptr + i);
                     }
                  } else if (this.isConstant) {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = 255 & this.data[0];
                     }
                  } else {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = 255 & this.data[(int)i];
                     }
                  }
         ...
         if (endPos >= 0L && endPos <= this.length && endPos >= startPos) {
            if (step < 1L) {
               ...
               if (len > 1073741824L) {
                  ...
                  if (this.ptr != 0L) {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = (long)(255 & LargeArrayUtils.UNSAFE.getByte(this.ptr + i));
                     }
                  } else if (this.isConstant) {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = (long)(255 & this.data[0]);
                     }
                  } else {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = (long)(255 & this.data[(int)i]);
                     }
                  }
         ...
         if (endPos >= 0L && endPos <= this.length && endPos >= startPos) {
            if (step < 1L) {
               ...
               if (len > 1073741824L) {
                  ...
                  if (this.ptr != 0L) {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = (float)(255 & LargeArrayUtils.UNSAFE.getByte(this.ptr + i));
                     }
                  } else if (this.isConstant) {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = (float)(255 & this.data[0]);
                     }
                  } else {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = (float)(255 & this.data[(int)i]);
                     }
                  }
         ...
         if (endPos >= 0L && endPos <= this.length && endPos >= startPos) {
            if (step < 1L) {
               ...
               if (len > 1073741824L) {
                  ...
                  if (this.ptr != 0L) {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = (double)(255 & LargeArrayUtils.UNSAFE.getByte(this.ptr + i));
                     }
                  } else if (this.isConstant) {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = (double)(255 & this.data[0]);
                     }
                  } else {
                     for(long i = startPos; i < endPos; i += step) {
                        out[idx++] = (double)(255 & this.data[(int)i]);
                     }
                  }
   ...
   public final void setShort(long i, short value) {
      if (this.ptr != 0L) {
         LargeArrayUtils.UNSAFE.putByte(this.ptr + i, (byte)(255 & value));
      } else {
         if (this.isConstant) {
            throw new IllegalAccessError("Constant arrays cannot be modified.");
         }

         this.data[(int)i] = (byte)(255 & value);
      }
   ...
   public final void setLong(long i, long value) {
      if (this.ptr != 0L) {
         LargeArrayUtils.UNSAFE.putByte(this.ptr + i, (byte)((int)(255L & value)));
      } else {
         if (this.isConstant) {
            throw new IllegalAccessError("Constant arrays cannot be modified.");
         }

         this.data[(int)i] = (byte)((int)(255L & value));
      }
   ...
   public final void setFloat(long i, float value) {
      if (this.ptr != 0L) {
         LargeArrayUtils.UNSAFE.putByte(this.ptr + i, (byte)(255 & (int)value));
      } else {
         if (this.isConstant) {
            throw new IllegalAccessError("Constant arrays cannot be modified.");
         }

         this.data[(int)i] = (byte)(255 & (int)value);
      }
   ...
   public final void setDouble(long i, double value) {
      if (this.ptr != 0L) {
         LargeArrayUtils.UNSAFE.putByte(this.ptr + i, (byte)((int)(255L & (long)value)));
      } else {
         if (this.isConstant) {
            throw new IllegalAccessError("Constant arrays cannot be modified.");
         }

         this.data[(int)i] = (byte)((int)(255L & (long)value));
      }
