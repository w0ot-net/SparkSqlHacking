...
class ProvRevocationChecker extends PKIXRevocationChecker implements PKIXCertRevocationChecker {
   ...
   public ProvRevocationChecker(JcaJceHelper var1) {
      this.helper = var1;
      this.crlChecker = new ProvCrlRevocationChecker(var1);
      this.ocspChecker = new ProvOcspRevocationChecker(this, var1);
   }
   ...
   public void initialize(PKIXCertRevocationCheckerParameters var1) {
      ...
      this.crlChecker.initialize(var1);
      this.ocspChecker.initialize(var1);
   }
   ...
   public void init(boolean var1) throws CertPathValidatorException {
      ...
      this.crlChecker.init(var1);
      this.ocspChecker.init(var1);
   }
   ...
   public void check(Certificate var1, Collection var2) throws CertPathValidatorException {
      X509Certificate var3 = (X509Certificate)var1;
      if (!this.hasOption(Option.ONLY_END_ENTITY) || var3.getBasicConstraints() == -1) {
         if (this.hasOption(Option.PREFER_CRLS)) {
            try {
               this.crlChecker.check(var1);
            } catch (RecoverableCertPathValidatorException var5) {
               if (this.hasOption(Option.NO_FALLBACK)) {
                  throw var5;
               }

               this.ocspChecker.check(var1);
            }
         } else {
            try {
               this.ocspChecker.check(var1);
            } catch (RecoverableCertPathValidatorException var6) {
               if (this.hasOption(Option.NO_FALLBACK)) {
                  throw var6;
               }
   ...
   private boolean hasOption(PKIXRevocationChecker.Option var1) {
      return this.getOptions().contains(var1);
   }
