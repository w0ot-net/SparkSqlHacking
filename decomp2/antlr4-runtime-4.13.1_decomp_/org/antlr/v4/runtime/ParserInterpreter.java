...
public class ParserInterpreter extends Parser {
   ...
   /** @deprecated */
   @Deprecated
   public ParserInterpreter(String grammarFileName, Collection tokenNames, Collection ruleNames, ATN atn, TokenStream input) {
      this(grammarFileName, VocabularyImpl.fromTokenNames((String[])tokenNames.toArray(new String[0])), ruleNames, atn, input);
   }

   public ParserInterpreter(String grammarFileName, Vocabulary vocabulary, Collection ruleNames, ATN atn, TokenStream input) {
      super(input);
   ...
   protected void recover(RecognitionException e) {
      ...
      if (this._input.index() == i) {
         if (e instanceof InputMismatchException) {
            ...
            Token tok = e.getOffendingToken();
            Token errToken = this.getTokenFactory().create(new Pair(tok.getTokenSource(), tok.getTokenSource().getInputStream()), 0, tok.getText(), 0, -1, -1, tok.getLine(), tok.getCharPositionInLine());
            this._ctx.addErrorNode(this.createErrorNode(this._ctx, errToken));
         }
      }
