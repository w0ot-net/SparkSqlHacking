...
public class ParseTreePatternMatcher {
   ...
   public ParseTreePattern compile(String pattern, int patternRuleIndex) {
      ...
      try {
         ...
         var12 = parserInterp.parse(patternRuleIndex);
      ...
      if (tokens.LA(1) != -1) {
         ...
         return new ParseTreePattern(this, pattern, patternRuleIndex, var12);
      }
   }
   ...
   protected ParseTree matchImpl(ParseTree tree, ParseTree patternTree, MultiMap labels) {
      if (tree == null) {
   ...
   public List split(String pattern) {
      ...
      if (starts.size() > stops.size()) {
         ...
         for(int i = 0; i < ntags; ++i) {
            String tag = pattern.substring((Integer)starts.get(i) + this.start.length(), (Integer)stops.get(i));
         ...
         if (ntags > 0) {
            int afterLastTag = (Integer)stops.get(ntags - 1) + this.stop.length();
            if (afterLastTag < n) {
