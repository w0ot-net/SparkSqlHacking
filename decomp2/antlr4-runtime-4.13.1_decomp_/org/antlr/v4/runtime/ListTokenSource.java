...
public class ListTokenSource implements TokenSource {
   ...
   public ListTokenSource(List tokens) {
      this(tokens, (String)null);
   }

   public ListTokenSource(List tokens, String sourceName) {
      ...
      if (this.i >= this.tokens.size()) {
         if (this.eofToken == null) {
            ...
            if (this.tokens.size() > 0) {
               int previousStop = ((Token)this.tokens.get(this.tokens.size() - 1)).getStopIndex();
               if (previousStop != -1) {
            ...
            this.eofToken = this._factory.create(new Pair(this, this.getInputStream()), -1, "EOF", 0, start, stop, this.getLine(), this.getCharPositionInLine());
         }
