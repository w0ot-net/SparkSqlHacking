...
final class DnsMessageUtil {
   ...
   static StringBuilder appendRecordClass(StringBuilder buf, int dnsClass) {
      String name;
      switch (dnsClass = dnsClass & 65535) {
         case 1:
            name = "IN";
            break;
         case 2:
   ...
   private static void appendResponseHeader(StringBuilder buf, DnsResponse msg) {
      buf.append(StringUtil.simpleClassName(msg)).append('(');
      appendAddresses(buf, msg).append("id: ").append(msg.id()).append(", ").append(msg.opCode()).append(", ").append(msg.code()).append(',');
   ...
   interface DnsQueryFactory {
      DnsQuery newQuery(int var1, DnsOpCode var2);
   }
}
