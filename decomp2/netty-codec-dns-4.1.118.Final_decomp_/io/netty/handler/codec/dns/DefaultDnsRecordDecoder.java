...
public class DefaultDnsRecordDecoder implements DnsRecordDecoder {
   ...
   public final DnsRecord decodeRecord(ByteBuf in) throws Exception {
   ...
   protected DnsRecord decodeRecord(String name, DnsRecordType type, int dnsClass, long timeToLive, ByteBuf in, int offset, int length) throws Exception {
      if (type == DnsRecordType.PTR) {
         ...
         return type != DnsRecordType.CNAME && type != DnsRecordType.NS ? new DefaultDnsRawRecord(name, type, dnsClass, timeToLive, in.retainedDuplicate().setIndex(offset, offset + length)) : new DefaultDnsRawRecord(name, type, dnsClass, timeToLive, DnsCodecUtil.decompressDomainName(in.duplicate().setIndex(offset, offset + length)));
      }
   }
