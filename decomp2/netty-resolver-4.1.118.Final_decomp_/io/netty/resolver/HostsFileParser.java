...
public final class HostsFileParser {
   ...
   private static HostsFileEntries hostsFileEntries(HostsFileEntriesProvider provider) {
      return provider == HostsFileEntriesProvider.EMPTY ? HostsFileEntries.EMPTY : new HostsFileEntries(toMapWithSingleValue(provider.ipv4Entries()), toMapWithSingleValue(provider.ipv6Entries()));
   }

   private static Map toMapWithSingleValue(Map fromMapWithListValue) {
      Map<String, InetAddress> result = new HashMap(fromMapWithListValue.size());

      for(Map.Entry entry : fromMapWithListValue.entrySet()) {
