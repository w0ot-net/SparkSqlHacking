...
class ReversePurgeLongHashMap {
   ...
   void keepOnlyPositiveCounts() {
      int firstProbe;
      for(firstProbe = this.keys.length - 1; this.states[firstProbe] > 0; --firstProbe) {
      }
   ...
   void adjustAllValuesBy(long adjustAmount) {
      long[] var4;
      for(int i = this.keys.length; i-- > 0; var4[i] += adjustAmount) {
         var4 = this.values;
      }
   ...
   public String toString() {
      ...
      StringBuilder sb = new StringBuilder();
      sb.append("ReversePurgeLongHashMap:").append(org.apache.datasketches.common.Util.LS);
      sb.append(String.format("  %12s:%11s%20s %s", "Index", "States", "Values", "Keys")).append(org.apache.datasketches.common.Util.LS);

      for(int i = 0; i < this.keys.length; ++i) {
         if (this.states[i] > 0) {
            sb.append(String.format("  %12d:%11d%20d %d", i, this.states[i], this.values[i], this.keys[i])).append(org.apache.datasketches.common.Util.LS);
         }
      }
   ...
   private int hashProbe(long key) {
      ...
      int probe;
      for(probe = (int)Util.hash(key) & arrayMask; this.states[probe] > 0 && this.keys[probe] != key; probe = probe + 1 & arrayMask) {
      }
   ...
   static class Iterator {
      ...
      Iterator(long[] keys, long[] values, short[] states, int numActive) {
         this.keys_ = keys;
         this.values_ = values;
         this.states_ = states;
         this.numActive_ = numActive;
         this.stride_ = (int)((double)keys.length * 0.6180339887498949) | 1;
