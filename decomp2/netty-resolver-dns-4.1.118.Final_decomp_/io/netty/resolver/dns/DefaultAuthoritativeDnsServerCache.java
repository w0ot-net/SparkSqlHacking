...
public class DefaultAuthoritativeDnsServerCache implements AuthoritativeDnsServerCache {
   ...
   public DefaultAuthoritativeDnsServerCache() {
      this(0, Cache.MAX_SUPPORTED_TTL_SECS, (Comparator)null);
   }

   public DefaultAuthoritativeDnsServerCache(int minTtl, int maxTtl, Comparator comparator) {
      this.resolveCache = new Cache() {
         ...
         protected boolean equals(InetSocketAddress entry, InetSocketAddress otherEntry) {
            return PlatformDependent.javaVersion() >= 7 ? entry.getHostString().equalsIgnoreCase(otherEntry.getHostString()) : entry.getHostName().equalsIgnoreCase(otherEntry.getHostName());
         }
   ...
   public DnsServerAddressStream get(String hostname) {
      ...
      return addresses != null && !addresses.isEmpty() ? new SequentialDnsServerAddressStream(addresses, 0) : null;
   }
