...
class Frame {
   ...
   static int getAbstractTypeFromApiFormat(SymbolTable symbolTable, Object type) {
      if (type instanceof Integer) {
         ...
         return (label.flags & 4) != 0 ? 12582912 | symbolTable.addUninitializedType("", label.bytecodeOffset) : 16777216 | symbolTable.addForwardUninitializedType("", label);
      }
   }
   ...
   private static int getAbstractTypeFromDescriptor(SymbolTable symbolTable, String buffer, int offset) {
      switch (buffer.charAt(offset)) {
         ...
         case 'Y':
         default:
            throw new IllegalArgumentException(stringConcat$1(buffer.substring(offset)));
         case 'F':
            return 4194306;
         case 'J':
            return 4194308;
   ...
   private int getLocal(int localIndex) {
      if (this.outputLocals != null && localIndex < this.outputLocals.length) {
         ...
         if (abstractType == 0) {
            abstractType = this.outputLocals[localIndex] = 20971520 | localIndex;
         }

         return abstractType;
      } else {
         return 20971520 | localIndex;
      }
   }
   ...
   private int pop() {
      return this.outputStackTop > 0 ? this.outputStack[--this.outputStackTop] : 25165824 | -(--this.outputStackStart);
   }
   ...
   private int getInitializedType(SymbolTable symbolTable, int abstractType) {
      if (abstractType == 4194310 || (abstractType & -4194304) == 12582912 || (abstractType & -4194304) == 16777216) {
      ...
      switch (opcode) {
         ...
         case 58:
            ...
            if (arg > 0) {
               ...
               if (previousLocalType != 4194308 && previousLocalType != 4194307) {
                  if ((previousLocalType & 62914560) == 20971520 || (previousLocalType & 62914560) == 25165824) {
         ...
         case 189:
            ...
            if (arrayElementType.charAt(0) == '[') {
               this.push(symbolTable, stringConcat$2(arrayElementType));
            } else {
   ...
   // $FF: synthetic method
   private static String stringConcat$2(String var0) {
      return "[" + var0;
   }
