...
final class Constants {
   ...
   static void checkAsmExperimental(Object caller) {
      ...
      if (!isWhitelisted(internalName)) {
         checkIsPreview(callerClass.getClassLoader().getResourceAsStream(stringConcat$0(internalName)));
      }
   ...
   static boolean isWhitelisted(String internalName) {
      if (!internalName.startsWith("org/apache/xbean/asm9/")) {
         return false;
      } else {
         String member = "(Annotation|Class|Field|Method|Module|RecordComponent|Signature)";
         return internalName.contains("Test$") || Pattern.matches(stringConcat$1(member), internalName) || Pattern.matches(stringConcat$2(member), internalName);
      }
   }

   // $FF: synthetic method
   private static String stringConcat$1(String var0) {
      return "org/apache/xbean/asm9/util/Trace" + var0 + "Visitor(\\$.*)?";
   }

   // $FF: synthetic method
   private static String stringConcat$2(String var0) {
      return "org/apache/xbean/asm9/util/Check" + var0 + "Adapter(\\$.*)?";
   }
