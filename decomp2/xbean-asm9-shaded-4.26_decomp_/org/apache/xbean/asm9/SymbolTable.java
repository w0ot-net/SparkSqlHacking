...
final class SymbolTable {
      ...
      for(int itemIndex = 1; itemIndex < this.constantPoolCount; itemIndex += itemTag != 5 && itemTag != 6 ? 1 : 2) {
         ...
         switch (itemTag) {
            ...
            case 15:
               ...
               this.addConstantMethodHandle(itemIndex, classReader.readByte(itemOffset), classReader.readClass(memberRefItemOffset, charBuffer), classReader.readUTF8(nameAndTypeItemOffset, charBuffer), classReader.readUTF8(nameAndTypeItemOffset + 2, charBuffer), classReader.readByte(memberRefItemOffset - 1) == 11);
               break;
            case 17:
            case 18:
   ...
   Symbol addConstant(Object value) {
      if (value instanceof Integer) {
         ...
         throw new IllegalArgumentException(stringConcat$0(value));
      }
   }

   // $FF: synthetic method
   private static String stringConcat$0(Object var0) {
      return "value " + var0;
   }
   ...
   private void addConstantIntegerOrFloat(int index, int tag, int value) {
      this.add(new Entry(index, tag, (long)value, hash(tag, value)));
   }
   ...
   Symbol addConstantMethodHandle(int referenceKind, String owner, String name, String descriptor, boolean isInterface) {
      ...
      int hashCode = hash(15, owner, name, descriptor, data);

      for(Entry entry = this.get(hashCode); entry != null; entry = entry.next) {
         if (entry.tag == 15 && entry.hashCode == hashCode && entry.data == (long)data && entry.owner.equals(owner) && entry.name.equals(name) && entry.value.equals(descriptor)) {
            return entry;
         }
      }
      ...
      return this.put(new Entry(this.constantPoolCount++, 15, owner, name, descriptor, (long)data, hashCode));
   }
   ...
   private void addConstantMethodHandle(int index, int referenceKind, String owner, String name, String descriptor, boolean isInterface) {
      ...
      int data = getConstantMethodHandleSymbolData(referenceKind, isInterface);
      int hashCode = hash(15, owner, name, descriptor, data);
      this.add(new Entry(index, 15, owner, name, descriptor, (long)data, hashCode));
   }

   private static int getConstantMethodHandleSymbolData(int referenceKind, boolean isInterface) {
      return referenceKind > 4 && isInterface ? referenceKind << 8 : referenceKind;
   }
   ...
   int addForwardUninitializedType(String value, Label label) {
      int labelIndex = this.getOrAddLabelEntry(label).index;
      ...
      return this.addTypeInternal(new Entry(this.typeCount, 130, value, (long)labelIndex, hashCode));
   }
   ...
   int addMergedType(int typeTableIndex1, int typeTableIndex2) {
      ...
      int hashCode = hash(131, typeTableIndex1 + typeTableIndex2);
      ...
      this.put(new Entry(this.typeCount, 131, data, hashCode)).info = commonSuperTypeIndex;
   ...
   private LabelEntry getOrAddLabelEntry(Label label) {
      ...
      for(labelEntry = this.labelEntries[hashCode % this.labelEntries.length]; labelEntry != null && labelEntry.label != label; labelEntry = labelEntry.next) {
      }
      ...
      if (labelEntry != null) {
         ...
         if (this.labelCount > this.labelEntries.length * 3 / 4) {
            ...
            LabelEntry nextEntry;
            for(int i = currentCapacity - 1; i >= 0; --i) {
               for(LabelEntry currentEntry = this.labelEntries[i]; currentEntry != null; currentEntry = nextEntry) {
