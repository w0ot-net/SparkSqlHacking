...
public class Environment {
   ...
   public static String getJniLibraryName(String var0) {
      if (isUnix()) {
         String var3 = is64Bit() ? "64" : "32";
         if (!isPowerPC() && !isAarch64() && !isRiscv64()) {
            return isS390x() ? String.format("%sjni-linux-%s", var0, ARCH) : String.format("%sjni-linux%s%s", var0, var3, getLibcPostfix());
         } else {
         ...
         if (is64Bit()) {
            ...
            return String.format("%sjni-osx-%s", var0, var2);
         } else {
