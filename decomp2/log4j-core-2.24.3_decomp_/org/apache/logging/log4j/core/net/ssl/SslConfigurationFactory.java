...
public class SslConfigurationFactory {
   ...
   static SslConfiguration createSslConfiguration(final PropertiesUtil props) {
      ...
      if (Strings.isNotEmpty(location) || storeType != null) {
         ...
         try {
            trustStoreConfiguration = TrustStoreConfiguration.createKeyStoreConfiguration(Strings.trimToNull(location), passwordChars, props.getStringProperty("log4j2.trustStorePasswordEnvironmentVariable"), props.getStringProperty("log4j2.trustStorePasswordFile"), storeType, props.getStringProperty("log4j2.trustStoreKeyManagerFactoryAlgorithm"));
         } catch (Exception ex) {
      ...
      if (Strings.isNotEmpty(location) || storeType != null) {
         ...
         try {
            keyStoreConfiguration = KeyStoreConfiguration.createKeyStoreConfiguration(Strings.trimToNull(location), passwordChars, props.getStringProperty("log4j2.keyStorePasswordEnvironmentVariable"), props.getStringProperty("log4j2.keyStorePasswordFile"), storeType, props.getStringProperty("log4j2.keyStoreKeyManagerFactoryAlgorithm"));
         } catch (Exception ex) {
      ...
      if (trustStoreConfiguration == null && keyStoreConfiguration == null) {
         ...
         return SslConfiguration.createSSLConfiguration((String)null, keyStoreConfiguration, trustStoreConfiguration, isVerifyHostName);
      }
   }
