...
public class Log4jContextFactory implements LoggerContextFactory, ShutdownCallbackRegistry {
   ...
   public LoggerContext getContext(final String fqcn, final ClassLoader loader, final Object externalContext, final boolean currentContext, final ConfigurationSource source) {
      LoggerContext ctx = this.selector.getContext(fqcn, loader, currentContext, (URI)null);
      if (externalContext != null && ctx.getExternalContext() == null) {
         ctx.setExternalContext(externalContext);
      }
   ...
   public LoggerContext getContext(final String fqcn, final ClassLoader loader, final Map.Entry entry, final boolean currentContext, final URI configLocation, final String name) {
   ...
   public LoggerContext getContext(final String fqcn, final ClassLoader loader, final Object externalContext, final boolean currentContext, final List configLocations, final String name) {
      LoggerContext ctx = this.selector.getContext(fqcn, loader, currentContext, (URI)null);
      if (externalContext != null && ctx.getExternalContext() == null) {
         ctx.setExternalContext(externalContext);
      }
   ...
   static {
      SHUTDOWN_HOOK_ENABLED = PropertiesUtil.getProperties().getBooleanProperty("log4j.shutdownHookEnabled", !Constants.IS_WEB_APP);
   }
}
