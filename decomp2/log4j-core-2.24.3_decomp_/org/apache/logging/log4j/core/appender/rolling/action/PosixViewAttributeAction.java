...
public class PosixViewAttributeAction extends AbstractPathAction {
   ...
   private PosixViewAttributeAction(final String basePath, final boolean followSymbolicLinks, final int maxDepth, final PathCondition[] pathConditions, final StrSubstitutor subst, final Set filePermissions, final String fileOwner, final String fileGroup) {
   ...
   protected FileVisitor createFileVisitor(final Path basePath, final List conditions) {
   ...
   public static class Builder implements org.apache.logging.log4j.core.util.Builder {
      ...
      public PosixViewAttributeAction build() {
         if (Strings.isEmpty(this.basePath)) {
            ...
            return null;
         } else if (this.filePermissions == null && Strings.isEmpty(this.filePermissionsString) && Strings.isEmpty(this.fileOwner) && Strings.isEmpty(this.fileGroup)) {
            AbstractAction.LOGGER.error("Posix file attribute view not valid because nor permissions, user or group defined.");
            return null;
         } else if (!FileUtils.isFilePosixAttributeViewSupported()) {
      ...
      public Builder withFilePermissions(final Set filePermissions) {
         this.filePermissions = filePermissions;
         return this;
      }
