...
public class PluginBuilder implements Builder {
   ...
   public Object build() {
      ...
      try {
         ...
         LOGGER.error("Could not create plugin of type {} for element {}: {}", this.clazz, this.node.getName(), (var5 instanceof InvocationTargetException ? ((InvocationTargetException)var5).getCause() : var5).toString(), var5);
      }
      ...
      try {
         ...
         LOGGER.error("Unable to invoke factory method in {} for element {}: {}", this.clazz, this.node.getName(), (var3 instanceof InvocationTargetException ? ((InvocationTargetException)var3).getCause() : var3).toString(), var3);
         return null;
      }
   }
   ...
   private static Builder createBuilder(final Class clazz) throws InvocationTargetException, IllegalAccessException {
      for(Method method : clazz.getDeclaredMethods()) {
         if (method.isAnnotationPresent(PluginBuilderFactory.class) && Modifier.isStatic(method.getModifiers()) && TypeUtil.isAssignable(Builder.class, method.getReturnType())) {
            ReflectionUtil.makeAccessible((AccessibleObject)method);
            return (Builder)method.invoke((Object)null);
         }
      }
   ...
   private void injectFields(final Builder builder) throws IllegalAccessException {
      ...
      AccessibleObject.setAccessible((AccessibleObject[])fields.toArray(EMPTY_FIELD_ARRAY), true);
      ...
      for(Field field : fields) {
         ...
         for(Annotation a : annotations) {
            if (!(a instanceof PluginAliases)) {
               ...
               if (visitor != null) {
                  Object value = visitor.setAliases(aliases).setAnnotation(a).setConversionType(field.getType()).setStrSubstitutor(this.event == null ? this.configuration.getConfigurationStrSubstitutor() : this.configuration.getStrSubstitutor()).setMember(field).visit(this.configuration, this.node, this.event, log);
                  if (value != null) {
      ...
      String reason = validateFields(builder, fields);
   ...
   private static String validateFields(final Builder builder, final List fields) throws IllegalAccessException {
      String reason = "";
      ...
      for(int i = 0; i < annotations.length; ++i) {
         ...
         for(Annotation a : annotations[i]) {
            if (!(a instanceof PluginAliases)) {
               PluginVisitor<? extends Annotation> visitor = PluginVisitors.findVisitor(a.annotationType());
               if (visitor != null) {
                  Object value = visitor.setAliases(aliases).setAnnotation(a).setConversionType(types[i]).setStrSubstitutor(this.event == null ? this.configuration.getConfigurationStrSubstitutor() : this.configuration.getStrSubstitutor()).setMember(factory).visit(this.configuration, this.node, this.event, log);
                  if (value != null) {
                     args[i] = value;
                  }
               }
