...
final class NativeDatagramPacketArray {
   ...
   boolean addWritable(ByteBuf buf, int index, int len) {
      return this.add0(buf, index, len, 0, (InetSocketAddress)null);
   }
   ...
   private final class MyMessageProcessor implements ChannelOutboundBuffer.MessageProcessor {
      ...
      public boolean processMessage(Object msg) {
         ...
         if (msg instanceof DatagramPacket) {
            ...
            added = NativeDatagramPacketArray.this.add0(buf, buf.readerIndex(), buf.readableBytes(), 0, (InetSocketAddress)null);
         } else {
            added = false;
         }
   ...
   public final class NativeDatagramPacket {
      ...
      DatagramPacket newDatagramPacket(ByteBuf buffer, InetSocketAddress recipient) throws UnknownHostException {
         ...
         return (DatagramPacket)(this.segmentSize > 0 ? new io.netty.channel.unix.SegmentedDatagramPacket(slice, this.segmentSize, recipient, sender) : new DatagramPacket(slice, recipient, sender));
      }
   }
}
