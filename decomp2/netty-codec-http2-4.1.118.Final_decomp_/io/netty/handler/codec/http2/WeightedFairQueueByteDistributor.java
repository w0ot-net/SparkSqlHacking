...
public final class WeightedFairQueueByteDistributor implements StreamByteDistributor {
   ...
   private final IntObjectMap stateOnlyMap;
   private final PriorityQueue stateOnlyRemovalQueue;
   ...
   public void updateDependencyTree(int childStreamId, int parentStreamId, short weight, boolean exclusive) {
      ...
      if (state.activeCountForTree != 0 && state.parent != null) {
         State var10000 = state.parent;
         var10000.totalQueuedWeights += (long)(weight - state.weight);
      }
   ...
   private int distributeToChildren(int maxBytes, StreamByteDistributor.Writer writer, State state) throws Http2Exception {
      ...
      try {
         ...
         int nsent = this.distribute(nextChildState == null ? maxBytes : Math.min(maxBytes, (int)Math.min((nextChildState.pseudoTimeToWrite - childState.pseudoTimeToWrite) * (long)childState.weight / oldTotalQueuedWeights + (long)this.allocationQuantum, 2147483647L)), writer, childState);
   ...
   void notifyParentChanged(List events) {
   ...
   private static final class StateOnlyComparator implements Comparator, Serializable {
   ...
   private static final class StatePseudoTimeComparator implements Comparator, Serializable {
   ...
   private final class State implements PriorityQueueNode {
      ...
      Http2Stream stream;
      State parent;
      IntObjectMap children;
      private final PriorityQueue pseudoTimeQueue;
      final int streamId;
      int streamableBytes;
      ...
      State(int streamId) {
         this(streamId, (Http2Stream)null, 0);
      }
      ...
      void takeChild(State child, boolean exclusive, List events) {
         this.takeChild((Iterator)null, child, exclusive, events);
      }

      void takeChild(Iterator childItr, State child, boolean exclusive, List events) {
         ...
         if (this.children.remove(child.streamId) != null) {
            ...
            if (!child.children.isEmpty()) {
               ...
               do {
                  ...
                  dependency.weight = (short)((int)Math.max(1L, (long)(dependency.weight * child.weight) / totalWeight));
      ...
      private IntObjectMap removeAllChildrenExcept(State stateToRetain) {
