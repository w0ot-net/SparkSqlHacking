...
public final class CleartextHttp2ServerUpgradeHandler extends ByteToMessageDecoder {
   ...
   public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
      ctx.pipeline().addAfter(ctx.name(), (String)null, this.httpServerUpgradeHandler).addAfter(ctx.name(), (String)null, this.httpServerCodec);
   }
   ...
   protected void decode(ChannelHandlerContext ctx, ByteBuf in, List out) throws Exception {
      ...
      if (!ByteBufUtil.equals(CONNECTION_PREFACE, CONNECTION_PREFACE.readerIndex(), in, in.readerIndex(), bytesRead)) {
         ...
         ctx.pipeline().addAfter(ctx.name(), (String)null, this.http2ServerHandler);
         ctx.pipeline().remove(this);
