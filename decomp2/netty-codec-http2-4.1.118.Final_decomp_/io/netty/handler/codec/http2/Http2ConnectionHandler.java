...
public class Http2ConnectionHandler extends ByteToMessageDecoder implements Http2LifecycleManager, ChannelOutboundHandler {
   ...
   public void flush(ChannelHandlerContext ctx) {
      try {
         ...
         this.onError(ctx, true, Http2Exception.connectionError(Http2Error.INTERNAL_ERROR, cause, "Error flushing"));
      }
   ...
   public void close(ChannelHandlerContext ctx, ChannelPromise promise) throws Exception {
      if (this.decoupleCloseAndGoAway) {
         ...
         if (ctx.channel().isActive() && this.prefaceSent()) {
            ChannelFuture f = this.connection().goAwaySent() ? ctx.write(Unpooled.EMPTY_BUFFER) : this.goAway(ctx, (Http2Exception)null, ctx.newPromise());
            ctx.flush();
   ...
   public void onError(ChannelHandlerContext ctx, boolean outbound, Throwable cause) {
      ...
      if (Http2Exception.isStreamError(embedded)) {
         ...
         for(Http2Exception.StreamException streamException : (Http2Exception.CompositeStreamException)embedded) {
      ...
      if (http2Ex instanceof Http2Exception.HeaderListSizeException && ((Http2Exception.HeaderListSizeException)http2Ex).duringDecode() && this.connection().isServer()) {
         ...
         if (stream != null && !stream.isHeadersSent()) {
            try {
               ...
               this.onError(ctx, outbound, Http2Exception.connectionError(Http2Error.INTERNAL_ERROR, cause2, "Error DecodeSizeError"));
            }
         }
      }
   ...
   private abstract class BaseDecoder {
      ...
      public abstract void decode(ChannelHandlerContext var1, ByteBuf var2, List var3) throws Exception;
