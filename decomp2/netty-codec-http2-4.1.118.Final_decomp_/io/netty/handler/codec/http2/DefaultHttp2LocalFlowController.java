...
public class DefaultHttp2LocalFlowController implements Http2LocalFlowController {
   ...
   public DefaultHttp2LocalFlowController(Http2Connection connection, float windowUpdateRatio, boolean autoRefillConnectionWindow) {
      ...
      FlowState connectionState = (FlowState)(autoRefillConnectionWindow ? new AutoRefillState(connection.connectionStream(), this.initialWindowSize) : new DefaultState(connection.connectionStream(), this.initialWindowSize));
   ...
   private static void checkValidRatio(float ratio) {
      if (Double.compare((double)ratio, (double)0.0F) <= 0 || Double.compare((double)ratio, (double)1.0F) >= 0) {
