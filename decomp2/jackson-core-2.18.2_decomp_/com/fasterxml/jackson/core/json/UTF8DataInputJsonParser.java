...
public class UTF8DataInputJsonParser extends JsonParserBase {
   ...
   protected int _readBinary(Base64Variant b64variant, OutputStream out, byte[] buffer) throws IOException {
      ...
      while(true) {
         ...
         if (ch > 32) {
            ...
            var22 = var22 << 6 | bits;
            buffer[outputPtr++] = (byte)(var22 >> 16);
            buffer[outputPtr++] = (byte)(var22 >> 8);
            buffer[outputPtr++] = (byte)var22;
         }
      }
   ...
   private final JsonToken _nextTokenNotInObject(int i) throws IOException {
      if (i == 34) {
         ...
         switch (i) {
            case 43:
               if (this.isEnabled(JsonReadFeature.ALLOW_LEADING_PLUS_SIGN_FOR_NUMBERS.mappedFeature())) {
                  return this._updateToken(this._parsePosNumber());
               }

               return this._updateToken(this._handleUnexpectedValue(i));
            case 45:
               return this._updateToken(this._parseNegNumber());
            case 46:
               return this._updateToken(this._parseFloatThatStartsWithPeriod(false, false));
            case 48:
            case 49:
            case 50:
            case 51:
   ...
   protected String _parseAposName() throws IOException {
      ...
      if (ch == 39) {
         ...
         for(int[] codes = INPUT_CODES_LATIN1; ch != 39; ch = this._inputData.readUnsignedByte()) {
            if (ch != 34 && codes[ch] != 0) {
               if (ch != 92) {
                  this._throwUnquotedSpace(ch, "name");
               } else {
   ...
   protected JsonToken _handleInvalidNumberStart(int ch, boolean neg, boolean hasSign) throws IOException {
      while(true) {
         ...
         label52: {
            ...
            match = neg ? "expected digit (0-9) to follow minus sign, for valid numeric value" : "expected digit (0-9) for valid numeric value";
            this._reportUnexpectedNumberChar(ch, match);
            return null;
         }
   ...
   protected final byte[] _decodeBase64(Base64Variant b64variant) throws IOException {
      ...
      while(true) {
         ...
         bits = b64variant.decodeBase64Char(var6);
         if (bits < 0) {
            bits = this._decodeBase64Escape(b64variant, var6, 1);
         }
         ...
         if (bits < 0) {
            if (bits != -2) {
               ...
               bits = this._decodeBase64Escape(b64variant, var6, 2);
            }

            if (bits == -2) {
               var6 = this._inputData.readUnsignedByte();
               if (!b64variant.usesPaddingChar(var6) && (var6 != 92 || this._decodeBase64Escape(b64variant, var6, 3) != -2)) {
                  throw this.reportInvalidBase64Char(b64variant, var6, 3, "expected padding character '" + b64variant.getPaddingChar() + "'");
               }
         ...
         if (bits < 0) {
            if (bits != -2) {
               ...
               bits = this._decodeBase64Escape(b64variant, var6, 3);
            }
