...
public class JsonPointer implements Serializable {
   ...
   public static JsonPointer forPath(JsonStreamContext context, boolean includeRoot) {
      if (context == null) {
         ...
         int approxLength;
         for(approxLength = 0; context != null; context = context.getParent()) {
            if (context.inObject()) {
               ...
               approxLength += 6;
               next = new PointerSegment(next, (String)null, ix);
            }
         }
         ...
         if (next == null) {
            ...
            PointerSegment last;
            for(last = null; next != null; next = next.next) {
               last = next;
            ...
            JsonPointer currPtr;
            for(currPtr = EMPTY; currSegment != null; currSegment = currSegment.prev) {
   ...
   public JsonPointer appendIndex(int index) {
      if (index < 0) {
         ...
         return compile(this.toStringBuilder(8).append('/').append(index).toString());
      }
   }
   ...
   public boolean equals(Object o) {
      if (o == this) {
         ...
         return false;
      } else if (!(o instanceof JsonPointer)) {
         return false;
      } else {
   ...
   private static JsonPointer _buildPath(String fullPath, int fullPathOffset, String segment, PointerParent parent) {
      JsonPointer curr;
      for(curr = new JsonPointer(fullPath, fullPathOffset, segment, EMPTY); parent != null; parent = parent.parent) {
