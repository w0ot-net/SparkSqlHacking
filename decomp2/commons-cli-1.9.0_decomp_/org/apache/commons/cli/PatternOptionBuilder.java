...
public class PatternOptionBuilder {
   ...
   private static final Converter UNSUPPORTED = (s) -> {
   ...
   public static Options parsePattern(String pattern) {
      ...
      for(int i = 0; i < pattern.length(); ++i) {
         ...
         if (!isValueCode(ch)) {
            if (opt != ' ') {
               Option option = Option.builder(String.valueOf(opt)).hasArg(type != null).required(required).type(type).converter(converter).build();
            ...
            map.put(FILES_VALUE, (Converter)unsupported());
