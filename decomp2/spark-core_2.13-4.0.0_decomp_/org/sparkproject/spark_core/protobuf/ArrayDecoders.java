...
final class ArrayDecoders {
   ...
   static int decodePackedFixed32List(byte[] data, int position, Internal.ProtobufList list, Registers registers) throws InvalidProtocolBufferException {
      IntArrayList output = (IntArrayList)list;
      position = decodeVarint32(data, position, registers);
      int packedDataByteSize = registers.int1;
      int fieldLimit = position + packedDataByteSize;
   ...
   static int decodeExtensionOrUnknownField(int tag, byte[] data, int position, int limit, Object message, MessageLite defaultInstance, UnknownFieldSchema unknownFieldSchema, Registers registers) throws IOException {
      int number = tag >>> 3;
   ...
   static int decodeExtension(int tag, byte[] data, int position, int limit, GeneratedMessageLite.ExtendableMessage message, GeneratedMessageLite.GeneratedExtension extension, UnknownFieldSchema unknownFieldSchema, Registers registers) throws IOException {
      ...
      if (extension.descriptor.isRepeated() && extension.descriptor.isPacked()) {
         switch (extension.getLiteType()) {
            ...
            case ENUM:
               IntArrayList list = new IntArrayList();
               position = decodePackedVarint32List(data, position, list, registers);
               SchemaUtil.filterUnknownEnumList(message, fieldNumber, list, (Internal.EnumLiteMap)extension.descriptor.getEnumType(), (Object)null, unknownFieldSchema);
               extensions.setField(extension.descriptor, list);
               break;
            default:
         ...
         if (extension.getLiteType() == WireFormat.FieldType.ENUM) {
            ...
            if (enumValue == null) {
               SchemaUtil.storeUnknownEnum(message, fieldNumber, registers.int1, (Object)null, unknownFieldSchema);
               return position;
            }
