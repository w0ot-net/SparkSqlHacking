...
final class Utf8 {
   private static final Processor processor = (Processor)(Utf8.UnsafeProcessor.isAvailable() && !Android.isOnAndroidDevice() ? new UnsafeProcessor() : new SafeProcessor());
   ...
   static int encodedLength(String string) {
      ...
      int i;
      for(i = 0; i < utf16Length && string.charAt(i) < 128; ++i) {
      }
   ...
   static final class UnsafeProcessor extends Processor {
      ...
      String decodeUtf8(byte[] bytes, int index, int size) throws InvalidProtocolBufferException {
         ...
         if (s.indexOf(65533) < 0) {
            return s;
         } else if (Arrays.equals(s.getBytes(Internal.UTF_8), Arrays.copyOfRange(bytes, index, index + size))) {
            return s;
         } else {
            throw InvalidProtocolBufferException.invalidUtf8();
         }
      ...
      private static int unsafeEstimateConsecutiveAscii(byte[] bytes, long offset, final int maxChars) {
         if (maxChars < 16) {
            ...
            while(i + 8 <= maxChars && (UnsafeUtil.getLong((Object)bytes, UnsafeUtil.BYTE_ARRAY_BASE_OFFSET + offset) & -9187201950435737472L) == 0L) {
               offset += 8L;
               i += 8;
            }
