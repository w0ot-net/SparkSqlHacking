...
final class RopeByteString extends ByteString {
   ...
   public CodedInputStream newCodedInput() {
      return CodedInputStream.newInstance((Iterable)this.asReadOnlyByteBufferList(), true);
   }
   ...
   private static class Balancer {
      ...
      private ByteString balance(ByteString left, ByteString right) {
         ...
         ByteString partialString;
         ByteString newLeft;
         for(partialString = (ByteString)this.prefixesStack.pop(); !this.prefixesStack.isEmpty(); partialString = new RopeByteString(newLeft, partialString)) {
      ...
      private void insert(ByteString byteString) {
         ...
         if (!this.prefixesStack.isEmpty() && ((ByteString)this.prefixesStack.peek()).size() < binEnd) {
            ...
            ByteString newTree;
            ByteString left;
            for(newTree = (ByteString)this.prefixesStack.pop(); !this.prefixesStack.isEmpty() && ((ByteString)this.prefixesStack.peek()).size() < binStart; newTree = new RopeByteString(left, newTree)) {
               left = (ByteString)this.prefixesStack.pop();
            }

            for(var9 = new RopeByteString(newTree, byteString); !this.prefixesStack.isEmpty(); var9 = new RopeByteString(left, var9)) {
               depthBin = this.getDepthBinForLength(((ByteString)var9).size());
