...
public class AttributeLayout implements IMatcher {
   ...
   public Codec getCodec() {
      if (this.layout.indexOf(79) >= 0) {
         ...
         return Codec.BCI5;
      } else if (this.layout.indexOf(83) >= 0 && !this.layout.contains("KS") && !this.layout.contains("RS")) {
   ...
   public ClassFileEntry getValue(long value, String type, SegmentConstantPool pool) throws Pack200Exception {
      if (!this.layout.startsWith("KQ")) {
         ...
         return type.equals("Ljava/lang/String;") ? getValue("KS", value, pool) : getValue("K" + type + this.layout.substring(2), value, pool);
      }
   }
   ...
   public int numBackwardsCallables() {
      return "*".equals(this.layout) ? 1 : this.backwardsCallCount;
   }
