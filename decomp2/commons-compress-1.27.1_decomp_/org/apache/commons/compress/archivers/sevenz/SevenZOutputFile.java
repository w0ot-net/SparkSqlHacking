...
public class SevenZOutputFile implements Closeable {
   ...
   public SevenZOutputFile(File fileName) throws IOException {
      this((File)fileName, (char[])null);
   }

   public SevenZOutputFile(File fileName, char[] password) throws IOException {
      this(Files.newByteChannel(fileName.toPath(), EnumSet.of(StandardOpenOption.CREATE, StandardOpenOption.WRITE, StandardOpenOption.TRUNCATE_EXISTING)), password);
   }
   ...
   public void closeArchiveEntry() throws IOException {
      ...
      if (this.fileBytesWritten > 0L) {
         ...
         if (this.additionalCountingStreams != null) {
            ...
            Arrays.setAll(sizes, (i) -> this.additionalCountingStreams[i].getByteCount());
   ...
   public SevenZArchiveEntry createArchiveEntry(File inputFile, String entryName) {
      ...
      try {
         this.fillDates(inputFile.toPath(), entry);
   ...
   public void finish() throws IOException {
      if (this.finished) {
         ...
         bb.putInt(0);
         bb.putLong(headerPosition - 32L).putLong(4294967295L & (long)headerBytes.length).putInt((int)crc32.getValue());
         crc32.reset();
   ...
   private Iterable getContentMethods(SevenZArchiveEntry entry) {
      ...
      if (this.aes256Options != null) {
         iter = (Iterable)Stream.concat(Stream.of(new SevenZMethodConfiguration(SevenZMethod.AES256SHA256, this.aes256Options)), StreamSupport.stream(iter.spliterator(), false)).collect(Collectors.toList());
      }
   ...
   public void putArchiveEntry(ArchiveEntry archiveEntry) {
      this.putArchiveEntry((SevenZArchiveEntry)archiveEntry);
   }
