...
public class DumpArchiveInputStream extends ArchiveInputStream {
   ...
   public DumpArchiveInputStream(InputStream is, String encoding) throws ArchiveException {
      ...
      this.queue = new PriorityQueue(10, (p, q) -> p.getOriginalName() != null && q.getOriginalName() != null ? p.getOriginalName().compareTo(q.getOriginalName()) : Integer.MAX_VALUE);
   }
   ...
   public DumpArchiveEntry getNextEntry() throws IOException {
      ...
      if (!this.queue.isEmpty()) {
         ...
         entry.setName(path);
         entry.setSimpleName(((Dirent)this.names.get(entry.getIno())).getName());
   ...
   private String getPath(DumpArchiveEntry entry) throws DumpArchiveException {
      ...
      while(true) {
         ...
         dirent = (Dirent)this.names.get(i);
         visited.set(i);
   ...
   private void readBITS() throws IOException {
      ...
      if (!DumpArchiveUtil.verify(buffer)) {
         ...
         if (DumpArchiveConstants.SEGMENT_TYPE.BITS != this.active.getHeaderType()) {
            throw new InvalidFormatException();
         } else if (this.raw.skip(1024L * (long)this.active.getHeaderCount()) == -1L) {
            throw new EOFException();
         } else {
            this.readIdx = this.active.getHeaderCount();
         }
   ...
   private void readDirectoryEntry(DumpArchiveEntry entry) throws IOException {
      ...
      for(boolean first = true; first || DumpArchiveConstants.SEGMENT_TYPE.ADDR == entry.getHeaderType(); size -= 1024L) {
         ...
         if (this.blockBuffer.length < datalen) {
            this.blockBuffer = IOUtils.readRange((InputStream)this.raw, datalen);
            if (this.blockBuffer.length != datalen) {
               throw new EOFException();
            }
         ...
         for(int i = 0; i < datalen - 8 && (long)i < size - 8L; i += reclen) {
            ...
            if (!".".equals(name) && !"..".equals(name)) {
               ...
               for(Map.Entry mapEntry : this.pending.entrySet()) {
                  DumpArchiveEntry v = (DumpArchiveEntry)mapEntry.getValue();
                  String path = this.getPath(v);
                  if (path != null) {
                     v.setName(path);
                     v.setSimpleName(((Dirent)this.names.get(mapEntry.getKey())).getName());
                     this.queue.add(v);
                  }
               }

               this.queue.forEach((e) -> this.pending.remove(e.getIno()));
            }
         }
