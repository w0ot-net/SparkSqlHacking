...
public class LocateBulkRequest extends BulkRequest {
   ...
   protected String getStatement(DatastoreClass table, ObjectProvider[] ops, boolean lock) {
      ...
      if (table.getIdentityType() == IdentityType.DATASTORE) {
         ...
         int[] cols = sqlStatement.select(expr, (String)null);
      ...
      int[] nonPkFieldNums = this.cmd.getNonPKMemberPositions();
      if (nonPkFieldNums != null) {
         for(int i = 0; i < nonPkFieldNums.length; ++i) {
            ...
            if (mapping != null && mapping.includeInFetchStatement() && !(mapping instanceof PersistableMapping)) {
      ...
      for(int i = 0; i < ops.length; ++i) {
         ...
         if (table.getIdentityType() == IdentityType.DATASTORE) {
            ...
            for(int j = 0; j < pkNums.length; ++j) {
               ...
               SQLExpression expr = exprFactory.newExpression(sqlStatement, sqlStatement.getPrimaryTable(), pkMapping);
               SQLExpression val = exprFactory.newLiteralParameter(sqlStatement, pkMapping, (Object)null, "PK" + j);
   ...
   public void execute(ObjectProvider[] ops) {
      if (ops != null && ops.length != 0) {
         if (NucleusLogger.PERSISTENCE.isDebugEnabled()) {
            ...
            NucleusLogger.PERSISTENCE.debug(Localiser.msg("052223", new Object[]{str.toString(), this.table}));
         }
         ...
         String statement = this.getStatement(this.table, ops, locked);
         ...
         try {
               ...
               try {
                  for(int i = 0; i < ops.length; ++i) {
                     if (cmd.getIdentityType() == IdentityType.DATASTORE) {
                        StatementMappingIndex datastoreIdx = this.mappingDefinitions[i].getMappingForMemberPosition(-1);
                  ...
                  try {
                     ObjectProvider[] missingOps = this.processResults(rs, ops);
            ...
            String msg = Localiser.msg("052220", new Object[]{ops[0].getObjectAsPrintable(), statement, var33.getMessage()});
   ...
   private ObjectProvider[] processResults(ResultSet rs, ObjectProvider[] ops) throws SQLException {
      ...
      while(rs.next()) {
         ...
         if (this.cmd.getIdentityType() == IdentityType.DATASTORE) {
            ...
            if (this.cmd.usesSingleFieldIdentityClass()) {
               ...
               id = IdentityUtils.getApplicationIdentityForResultSetRow(ec, this.cmd, (Class)null, true, resultFM);
            }
         }
      ...
      if (!missingOps.isEmpty()) {
         return (ObjectProvider[])missingOps.toArray(new ObjectProvider[missingOps.size()]);
      } else {
         return null;
      }
   }
