...
public class DateGetMonth4Method extends AbstractSQLMethod {
   public SQLExpression getExpression(SQLExpression expr, List args) {
      if (!(expr instanceof TemporalExpression)) {
         ...
         SQLExpression mm = this.exprFactory.newLiteral(this.stmt, mapping2, "%m");
         ArrayList funcArgs = new ArrayList();
         funcArgs.add(mm);
         funcArgs.add(expr);
         NumericExpression numExpr = new NumericExpression(new NumericExpression(this.stmt, this.getMappingForClass(Integer.TYPE), "strftime", funcArgs), Expression.OP_SUB, one);
