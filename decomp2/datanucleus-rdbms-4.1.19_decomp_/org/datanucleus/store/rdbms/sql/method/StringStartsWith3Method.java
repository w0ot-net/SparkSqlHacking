      ...
      if (args != null && args.size() != 0 && args.size() <= 2) {
         ...
         if (!(substrExpr instanceof StringExpression) && !(substrExpr instanceof CharacterExpression) && !(substrExpr instanceof ParameterLiteral)) {
            ...
            if (args.size() > 1) {
               ...
               funcArgs.add(expr);
               return new BooleanExpression(new StringExpression(this.stmt, this.getMappingForClass(Integer.TYPE), "LOCATE", funcArgs), Expression.OP_EQ, one.add(numExpr));
            } else {
               funcArgs.add(substrExpr);
               funcArgs.add(expr);
               return new BooleanExpression(new StringExpression(this.stmt, this.getMappingForClass(Integer.TYPE), "LOCATE", funcArgs), Expression.OP_EQ, one);
            }
         }
      } else {
         throw new NucleusException(Localiser.msg("060003", new Object[]{"startsWith", "StringExpression", 0, "StringExpression/CharacterExpression/Parameter"}));
