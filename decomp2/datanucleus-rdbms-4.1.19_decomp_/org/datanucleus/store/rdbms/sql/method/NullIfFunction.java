...
public class NullIfFunction extends AbstractSQLMethod {
   public SQLExpression getExpression(SQLExpression expr, List args) {
      if (expr != null) {
         throw new NucleusException(Localiser.msg("060002", new Object[]{"NULLIF", expr}));
      } else {
         Class exprType = null;
         ...
         if (exprType == NumericExpression.class) {
            ...
            return new StringExpression(this.stmt, this.getMappingForClass(cls), "NULLIF", args);
         } else if (exprType == TemporalExpression.class) {
            return new TemporalExpression(this.stmt, this.getMappingForClass(cls), "NULLIF", args);
         } else {
            return new ObjectExpression(this.stmt, this.getMappingForClass(cls), "NULLIF", args);
         }
      }
   }
}
