...
public abstract class AnnotationLiteral implements Annotation, Serializable {
   ...
   private Method[] getMembers() {
      if (this.members == null) {
         this.members = (Method[])AccessController.doPrivileged(new PrivilegedAction() {
            public Method[] run() {
         ...
         if (this.members.length > 0 && !this.annotationType().isAssignableFrom(this.getClass())) {
            ...
            throw new RuntimeException(var10002 + " does not implement the annotation type with members " + this.annotationType().getName());
         }
      }
