...
public class ConsoleReader implements Closeable {
   ...
   public ConsoleReader(@Nullable String appName, InputStream in, OutputStream out, @Nullable Terminal term, @Nullable String encoding) throws IOException {
      ...
      this.expandEvents = Configuration.getBoolean("jline.expandevents", true);
   ...
   private void setBuffer(String buffer) throws IOException {
      if (!buffer.equals(this.buf.buffer.toString())) {
         ...
         for(int l2 = this.buf.buffer.length(); i < l1 && i < l2 && buffer.charAt(i) == this.buf.buffer.charAt(i); ++i) {
            ++sameIndex;
         }

         i = this.buf.cursor - sameIndex;
         if (i < 0) {
            this.moveToEnd();
            i = this.buf.buffer.length() - sameIndex;
         }
   ...
   private void clearAhead(int num, int pos) throws IOException {
      if (num != 0) {
         ...
         if (this.terminal.getStringCapability("clr_eol") != null) {
            ...
            for(int var8 = num - nb; var8 > 0; var8 -= nb) {
               int prev = cur;
               cur = cur - cur % width + width;
               this.moveCursorFromTo(prev, cur);
               nb = Math.min(var8, width);
               this.tputs("clr_eol");
            }
            ...
            int var10;
            for(var10 = pos + nb; num > 0; var10 += nb) {
               this.moveCursorFromTo(var10++, var10);
               nb = Math.min(num, width);
               this.rawPrint(' ', nb);
               num -= nb;
            }

            this.moveCursorFromTo(var10, pos);
         } else {
            this.rawPrint(' ', num);
   ...
   private int viSearch(char searchChar) throws IOException {
      ...
      if (ch != -1 && !isAborted) {
         ...
         if (idx == -1) {
            ...
            for(boolean var14 = false; !var14 && (ch = this.readCharacter()) != -1; this.flush()) {
            ...
            while(true) {
               ...
               if (o instanceof String) {
                  ...
                  for(int i = 0; i < macro.length(); ++i) {
                     this.pushBackChar.push(macro.charAt(macro.length() - 1 - i));
                  }
                        ...
                        switch (op) {
                           ...
                           case CALL_LAST_KBD_MACRO:
                              for(int i = 0; i < this.macro.length(); ++i) {
                                 this.pushBackChar.push(this.macro.charAt(this.macro.length() - 1 - i));
                              }
                           ...
                           case TRANSPOSE_CHARS:
                              ...
                              break;
                           case INSERT_COMMENT:
                              String var32 = this.insertComment(false);
                              return var32;
                           ...
                           case VI_INSERT_COMMENT:
                              String lastChar = this.insertComment(true);
                              return lastChar;
                           case VI_SEARCH:
   ...
   private String readLineSimple() throws IOException {
      if (this.skipLF) {
         ...
         if (i != 10) {
            this.buf.buffer.append((char)i);
         }
      }

      while(true) {
         int i = this.readCharacter();
         if (i == -1 && this.buf.buffer.length() == 0) {
            return null;
         }
   ...
   public void printColumns(Collection items) throws IOException {
      if (items != null && !items.isEmpty()) {
         ...
         for(CharSequence item : items) {
            int len = this.wcwidth(Ansi.stripAnsi(item.toString()), 0);
         ...
         for(CharSequence item : items) {
            ...
            int strippedItemLength = this.wcwidth(Ansi.stripAnsi(item.toString()), 0);
