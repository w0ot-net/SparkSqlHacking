...
public class PoolingHttpClientConnectionManager implements HttpClientConnectionManager, ConnPoolControl, Closeable {
            ...
            try {
               if (conn.isOpen()) {
                  ...
                  if (this.log.isDebugEnabled()) {
                     String s;
                     if (keepalive > 0L) {
                        s = "for " + (double)effectiveUnit.toMillis(keepalive) / (double)1000.0F + " seconds";
                     } else {
                        s = "indefinitely";
                     }
   ...
   public void shutdown() {
      if (this.isShutDown.compareAndSet(false, true)) {
         ...
         try {
            this.pool.enumLeased(new PoolEntryCallback() {
   ...
   static class InternalConnectionFactory implements ConnFactory {
      ...
      InternalConnectionFactory(ConfigData configData, HttpConnectionFactory connFactory) {
