...
public class LinkedHashMap extends org.datanucleus.store.types.wrappers.LinkedHashMap implements BackedSCO {
   ...
   public LinkedHashMap(ObjectProvider op, AbstractMemberMetaData mmd) {
      ...
      if (!SCOUtils.mapHasSerialisedKeysAndValues(mmd) && mmd.getPersistenceModifier() == FieldPersistenceModifier.PERSISTENT) {
         this.backingStore = (MapStore)((BackedSCOStoreManager)this.ownerOP.getStoreManager()).getBackingStoreForField(this.ownerOP.getExecutionContext().getClassLoaderResolver(), mmd, java.util.LinkedHashMap.class);
      }
