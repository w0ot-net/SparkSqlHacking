...
public class Properties extends org.datanucleus.store.types.wrappers.Properties implements BackedSCO {
   ...
   public Properties(ObjectProvider op, AbstractMemberMetaData mmd) {
      ...
      if (!SCOUtils.mapHasSerialisedKeysAndValues(mmd) && mmd.getPersistenceModifier() == FieldPersistenceModifier.PERSISTENT) {
         this.backingStore = (MapStore)((BackedSCOStoreManager)this.ownerOP.getStoreManager()).getBackingStoreForField(this.ownerOP.getExecutionContext().getClassLoaderResolver(), mmd, java.util.Map.class);
      }
