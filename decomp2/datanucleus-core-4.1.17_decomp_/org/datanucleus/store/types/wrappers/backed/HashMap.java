...
public class HashMap extends org.datanucleus.store.types.wrappers.HashMap implements BackedSCO {
   ...
   public HashMap(ObjectProvider op, AbstractMemberMetaData mmd) {
      ...
      if (!SCOUtils.mapHasSerialisedKeysAndValues(mmd) && mmd.getPersistenceModifier() == FieldPersistenceModifier.PERSISTENT) {
         this.backingStore = (MapStore)((BackedSCOStoreManager)this.ownerOP.getStoreManager()).getBackingStoreForField(this.ownerOP.getExecutionContext().getClassLoaderResolver(), mmd, java.util.HashMap.class);
      }
