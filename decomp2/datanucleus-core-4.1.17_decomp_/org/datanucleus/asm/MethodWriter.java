...
class MethodWriter extends MethodVisitor {
         ...
         if (this.compute == 0) {
            if (this.currentBlock != null) {
               if (label.position == this.currentBlock.position) {
                  Label var2 = this.currentBlock;
                  var2.status |= label.status & 16;
            ...
            if (this.previousBlock != null) {
               if (label.position == this.previousBlock.position) {
                  Label var10000 = this.previousBlock;
                  var10000.status |= label.status & 16;
   ...
   public void visitMaxs(int maxStack, int maxLocals) {
      ...
      if (this.compute == 0) {
         ...
         Handler var13 = this.firstHandler;

         for(this.handlerCount = 0; var13 != null; var13 = var13.next) {
            ++this.handlerCount;
         }
         ...
         if (this.subroutines > 0) {
            ...
            for(Label var24 = this.labels; var24 != null; var24 = var24.successor) {
               if ((var24.status & 128) != 0) {
                  for(Label L = this.labels; L != null; L = L.successor) {
                     L.status &= -2049;
                  }

                  Label subroutine = var24.successors.next.successor;
                  subroutine.visitSubroutine(var24, 0L, this.subroutines);
               }
            }
         }
   ...
   private void resizeInstructions() {
      ...
      label234:
      while(var20 < this.code.length) {
         int opcode = b[var20] & 255;
         switch (ClassWriter.TYPE[opcode]) {
            case 0:
            case 4:
            ...
            case 3:
            case 11:
               newCode.putByteArray(b, var20, 2);
               var20 += 2;
               continue;
            case 2:
            case 5:
            ...
            case 12:
            case 13:
               newCode.putByteArray(b, var20, 3);
               var20 += 3;
               continue;
            case 7:
            case 8:
               newCode.putByteArray(b, var20, 5);
               var20 += 5;
               continue;
            case 9:
               int label;
               if (opcode > 201) {
                  opcode = opcode < 218 ? opcode - 49 : opcode - 20;
                  label = var20 + readUnsignedShort(b, var20 + 1);
               } else {
                  label = var20 + readShort(b, var20 + 1);
               }

               int newOffset = getNewOffset(allIndexes, allSizes, var20, label);
               if (resize[var20]) {
                  if (opcode == 167) {
                     newCode.putByte(200);
                  } else if (opcode == 168) {
               ...
               continue;
            case 10:
               int label = var20 + readInt(b, var20 + 1);
               int newOffset = getNewOffset(allIndexes, allSizes, var20, label);
               newCode.putByte(opcode);
               newCode.putInt(newOffset);
               var20 += 5;
            ...
            case 14:
               ...
               newCode.putByte(170);
               newCode.putByteArray((byte[])null, 0, (4 - newCode.length % 4) % 4);
               int label = var20 + readInt(b, var24);
               int var25 = var24 + 4;
               int newOffset = getNewOffset(allIndexes, allSizes, var20, label);
               newCode.putInt(newOffset);
               int j = readInt(b, var25);
               int var26 = var25 + 4;
               newCode.putInt(j);
               j = readInt(b, var26) - j + 1;
               var20 = var26 + 4;
               newCode.putInt(readInt(b, var20 - 4));
            ...
            case 15:
               ...
               newCode.putByte(171);
               newCode.putByteArray((byte[])null, 0, (4 - newCode.length % 4) % 4);
               int label = var20 + readInt(b, var21);
               int var22 = var21 + 4;
               int newOffset = getNewOffset(allIndexes, allSizes, var20, label);
               newCode.putInt(newOffset);
               int j = readInt(b, var22);
               var20 = var22 + 4;
               newCode.putInt(j);
               ...
               while(true) {
                  ...
                  newCode.putInt(readInt(b, var20));
                  var20 += 4;
                  label = v + readInt(b, var20);
                  var20 += 4;
                  newOffset = getNewOffset(allIndexes, allSizes, v, label);
            ...
            case 16:
            default:
               newCode.putByteArray(b, var20, 4);
               var20 += 4;
               continue;
            case 17:
         }

         opcode = b[var20 + 1] & 255;
         if (opcode == 132) {
            newCode.putByteArray(b, var20, 6);
            var20 += 6;
         } else {
            newCode.putByteArray(b, var20, 4);
            var20 += 4;
      ...
      for(int i = 0; i < 2; ++i) {
         ...
         if (bv != null) {
            ...
            for(int var28 = 0; var28 < bv.length; var28 += 10) {
               int label = readUnsignedShort(b, var28);
               int newOffset = getNewOffset(allIndexes, allSizes, 0, label);
               writeShort(b, var28, newOffset);
               label += readUnsignedShort(b, var28 + 2);
               newOffset = getNewOffset(allIndexes, allSizes, 0, label) - newOffset;
               writeShort(b, var28 + 2, newOffset);
            }
         }
      }
      ...
      if (this.lineNumber != null) {
         ...
         for(int var29 = 0; var29 < this.lineNumber.length; var29 += 4) {
            writeShort(b, var29, getNewOffset(allIndexes, allSizes, 0, readUnsignedShort(b, var29)));
         }
      }

      for(Attribute attr = this.cattrs; attr != null; attr = attr.next) {
         Label[] labels = attr.getLabels();
         if (labels != null) {
            for(int var40 = labels.length - 1; var40 >= 0; --var40) {
               getNewOffset(allIndexes, allSizes, labels[var40]);
            }
         }
      }
