...
public class ReplaceField extends ClassMethod {
   public static ReplaceField getInstance(ClassEnhancer enhancer) {
      return new ReplaceField(enhancer, enhancer.getNamer().getReplaceFieldMethodName(), 1, (Object)null, new Class[]{Integer.TYPE}, new String[]{"index"});
   }
      ...
      if (pcSuperclassName != null) {
         if (fields.length > 0) {
            ...
            for(int i = 0; i < fields.length; ++i) {
               ...
               this.visitor.visitMethodInsn(185, this.getNamer().getStateManagerAsmClassName(), "replacing" + methodNameType + "Field", "(" + this.getNamer().getPersistableDescriptor() + "I)" + EnhanceUtils.getTypeDescriptorForEnhanceMethod(fields[i].getType()));
               if (methodNameType.equals("Object")) {
                  this.visitor.visitTypeInsn(192, fields[i].getTypeName().replace('.', '/'));
               }
