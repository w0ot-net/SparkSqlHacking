...
public class EnhancerPropertySetterAdapter extends MethodVisitor {
   ...
   public EnhancerPropertySetterAdapter(MethodVisitor mv, ClassEnhancer enhancer, String methodName, String methodDesc, AbstractMemberMetaData mmd, ClassVisitor cv) {
      super(327680, mv);
      ...
      this.visitor = cv.visitMethod(access, enhancer.getNamer().getSetMethodPrefixMethodName() + mmd.getName(), methodDesc, (String)null, (String[])null);
   }

   public void visitEnd() {
      this.visitor.visitEnd();
      if (DataNucleusEnhancer.LOGGER.isDebugEnabled()) {
         String msg = ClassMethod.getMethodAdditionMessage(this.enhancer.getNamer().getSetMethodPrefixMethodName() + this.mmd.getName(), (Object)null, new Object[]{this.mmd.getType()}, new String[]{"val"});
         DataNucleusEnhancer.LOGGER.debug(Localiser.msg("005019", msg));
      }

      if (!this.mmd.isAbstract()) {
         generateSetXXXMethod(this.mv, this.mmd, this.enhancer.getASMClassName(), this.enhancer.getClassDescriptor(), this.enhancer.getNamer());
      }
   ...
   public static void generateSetXXXMethod(MethodVisitor mv, AbstractMemberMetaData mmd, String asmClassName, String asmClassDesc, EnhancementNamer namer) {
      ...
      if ((mmd.getPersistenceFlags() & 8) == 8) {
         ...
         mv.visitLocalVariable(argNames[1], fieldTypeDesc, (String)null, startLabel, endLabel, 1);
         mv.visitMaxs(5, 2);
         ...
         if (!this.enhancer.getASMClassName().equals(owner)) {
            ...
            if (propName != null) {
               ...
               if (callingOverriddenSuperclassMethod) {
                  String redirectMethodName = this.enhancer.getNamer().getSetMethodPrefixMethodName() + propName;
