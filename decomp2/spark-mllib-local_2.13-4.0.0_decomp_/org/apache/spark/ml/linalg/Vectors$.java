...
public final class Vectors$ {
   ...
   private static final DenseVector empty;
   private static final int MAX_HASH_NNZ;

   static {
      empty = new DenseVector(.MODULE$.emptyDoubleArray());
      MAX_HASH_NNZ = 128;
   }

   public Vector dense(final double firstValue, final double... otherValues) {
      return this.dense(firstValue, (Seq)scala.runtime.ScalaRunTime..MODULE$.wrapDoubleArray(otherValues));
   }
   ...
   public Vector normalize(final Vector vector, final double p) {
      double n = this.norm(vector, p);
      scala.Predef..MODULE$.require(n > (double)0, () -> "Can not normalize zero-length vectors.");
      BLAS$.MODULE$.scal((double)1.0F / n, vector);
      return vector;
   }
