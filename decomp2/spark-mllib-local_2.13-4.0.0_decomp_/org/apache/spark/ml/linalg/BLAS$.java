...
public final class BLAS$ implements Serializable {
   ...
   private static final int nativeL1Threshold = 256;
   ...
   public void dspmv(final int n, final double alpha, final DenseVector A, final DenseVector x, final double beta, final DenseVector y) {
      this.javaBLAS().dspmv("U", n, alpha, A.values(), x.values(), 1, beta, y.values(), 1);
   }
   ...
   public void spr(final double alpha, final Vector v, final double[] U) {
         ...
         if (!var12.isEmpty()) {
            ...
            for(double av = (double)0.0F; j < nnz; prevCol = col) {
               ...
               for(int var26 = 0; var26 <= j; ++var26) {
                  int var23 = colStartIdx + indices[var26];
                  U[var23] += av * values[var26];
               }
      ...
      throw new IllegalArgumentException("spr doesn't support vector type " + v.getClass() + ".");
   }
   ...
   public void gemm(final double alpha, final Matrix A, final DenseMatrix B, final double beta, final double[] CValues) {
      .MODULE$.require(A.numRows() * B.numCols() <= CValues.length, () -> {
         int var10000 = A.numRows();
         return "the length of CValues must be no less than " + var10000 + " X " + B.numCols();
      });
      if (alpha != (double)0.0F || beta != (double)1.0F) {
         if (alpha == (double)0.0F) {
            int n = A.numRows() * B.numCols();
            this.getBLAS(n).dscal(n, beta, CValues, 1);
         } else if (A instanceof SparseMatrix) {
            SparseMatrix var11 = (SparseMatrix)A;
            this.gemmImpl(alpha, var11, B, beta, CValues);
            BoxedUnit var13 = BoxedUnit.UNIT;
         } else if (A instanceof DenseMatrix) {
            DenseMatrix var12 = (DenseMatrix)A;
            this.gemmImpl(alpha, var12, B, beta, CValues);
            BoxedUnit var10000 = BoxedUnit.UNIT;
         } else {
            throw new IllegalArgumentException("gemm doesn't support matrix type " + A.getClass() + ".");
         }
   ...
   private void gemm(final double alpha, final DenseMatrix A, final DenseMatrix B, final double beta, final DenseMatrix C) {
      ...
      this.gemmImpl(alpha, A, B, beta, C.values());
   }

   private void gemmImpl(final double alpha, final DenseMatrix A, final DenseMatrix B, final double beta, final double[] CValues) {
      ...
      this.nativeBLAS().dgemm(tAstr, tBstr, A.numRows(), B.numCols(), A.numCols(), alpha, A.values(), lda, B.values(), ldb, beta, CValues, A.numRows());
   }
   ...
   private void gemmImpl(final double alpha, final SparseMatrix A, final DenseMatrix B, final double beta, final double[] CValues) {
      int mA = A.numRows();
      int nB = B.numCols();
      int kA = A.numCols();
      int kB = B.numRows();
            ...
            while(colCounterForB < nB) {
               ...
               for(int Cstart = colCounterForB * mA; colCounterForA < kA; ++colCounterForA) {
                  ...
                  for(double Bval = Bvals[Bstart + colCounterForA] * alpha; i < indEnd; ++i) {
                     int var42 = Cstart + ArowIndices[i];
                     Cvals[var42] += Avals[i] * Bval;
                  }
               }
            ...
            while(colCounterForB < nB) {
               ...
               for(int Cstart = colCounterForB * mA; colCounterForA < kA; ++colCounterForA) {
                  ...
                  for(double Bval = Bvals[colCounterForB + nB * colCounterForA] * alpha; i < indEnd; ++i) {
                     int var49 = Cstart + ArowIndices[i];
                     Cvals[var49] += Avals[i] * Bval;
                  }
               }
   ...
   public void gemv(final double alpha, final Matrix A, final double[] x, final double beta, final double[] y) {
      .MODULE$.require(A.numCols() <= x.length, () -> {
         int var10000 = A.numCols();
         return "The columns of A don't match the number of elements of x. A: " + var10000 + ", x: " + x.length;
      });
      .MODULE$.require(A.numRows() <= y.length, () -> {
         int var10000 = A.numRows();
         return "The rows of A don't match the number of elements of y. A: " + var10000 + ", y:" + y.length;
      });
      if (alpha != (double)0.0F || beta != (double)1.0F) {
         if (alpha == (double)0.0F) {
            this.getBLAS(A.numRows()).dscal(A.numRows(), beta, y, 1);
         } else if (A instanceof SparseMatrix) {
            SparseMatrix var10 = (SparseMatrix)A;
            this.gemvImpl(alpha, var10, x, beta, y);
            BoxedUnit var12 = BoxedUnit.UNIT;
         } else if (A instanceof DenseMatrix) {
            DenseMatrix var11 = (DenseMatrix)A;
            this.gemvImpl(alpha, var11, x, beta, y);
            BoxedUnit var10000 = BoxedUnit.UNIT;
         } else {
   ...
   private void gemv(final double alpha, final DenseMatrix A, final DenseVector x, final double beta, final DenseVector y) {
      this.gemvImpl(alpha, A, x.values(), beta, y.values());
   }

   private void gemvImpl(final double alpha, final DenseMatrix A, final double[] xValues, final double beta, final double[] yValues) {
      String tStrA = A.isTransposed() ? "T" : "N";
      int mA = !A.isTransposed() ? A.numRows() : A.numCols();
      int nA = !A.isTransposed() ? A.numCols() : A.numRows();
      this.nativeBLAS().dgemv(tStrA, mA, nA, alpha, A.values(), mA, xValues, 1, beta, yValues, 1);
   }
   ...
   private void gemvImpl(final double alpha, final SparseMatrix A, final double[] xValues, final double beta, final double[] yValues) {
      int mA = A.numRows();
      int nA = A.numCols();
      double[] Avals = A.values();
      int[] Arows = !A.isTransposed() ? A.rowIndices() : A.colPtrs();
      ...
      if (A.isTransposed()) {
         ...
         if (beta != (double)1.0F) {
            this.getBLAS(mA).dscal(mA, beta, yValues, 1);
         }
