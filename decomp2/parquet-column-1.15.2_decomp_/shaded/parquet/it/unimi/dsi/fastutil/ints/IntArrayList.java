...
public class IntArrayList extends AbstractIntList implements RandomAccess, Cloneable, Serializable {
   ...
   public static IntArrayList toListWithExpectedSize(IntStream stream, int expectedSize) {
      return expectedSize <= 10 ? toList(stream) : (IntArrayList)stream.collect(new IntCollections.SizeDecreasingSupplier(expectedSize, (size) -> size <= 10 ? new IntArrayList() : new IntArrayList(size)), IntArrayList::add, IntList::addAll);
   }
