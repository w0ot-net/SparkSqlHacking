...
public class IntOpenHashSet extends AbstractIntSet implements Serializable, Cloneable, Hash {
   ...
   public static IntOpenHashSet toSetWithExpectedSize(IntStream stream, int expectedSize) {
      return expectedSize <= 16 ? toSet(stream) : (IntOpenHashSet)stream.collect(new IntCollections.SizeDecreasingSupplier(expectedSize, (size) -> size <= 16 ? new IntOpenHashSet() : new IntOpenHashSet(size)), IntOpenHashSet::add, IntOpenHashSet::addAll);
   }
   ...
   protected void rehash(int newN) {
      ...
      for(int j = this.realSize(); j-- != 0; newKey[pos] = key[i]) {
         --i;
         if (key[i] != 0 && newKey[pos = HashCommon.mix(key[i]) & mask] != 0) {
      ...
      public SetSpliterator trySplit() {
         if (this.pos >= this.max - 1) {
            ...
            if (retLen <= 1) {
               ...
               int retPos = this.pos;
               SetSpliterator split = IntOpenHashSet.this.new SetSpliterator(retPos, myNewPos, this.mustReturnNull, true);
               this.pos = myNewPos;
               this.mustReturnNull = false;
               this.hasSplit = true;
               return split;
