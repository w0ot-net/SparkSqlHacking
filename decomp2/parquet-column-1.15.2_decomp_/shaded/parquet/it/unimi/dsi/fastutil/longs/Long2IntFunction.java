...
@FunctionalInterface
public interface Long2IntFunction extends shaded.parquet.it.unimi.dsi.fastutil.Function, LongToIntFunction {
   ...
   default Long2ObjectFunction andThenObject(Int2ObjectFunction after) {
      return (k) -> after.get(this.get(k));
   }

   default Object2IntFunction composeObject(Object2LongFunction before) {
      return (k) -> this.get(before.getLong(k));
   }

   default Long2ReferenceFunction andThenReference(Int2ReferenceFunction after) {
      return (k) -> after.get(this.get(k));
   }

   default Reference2IntFunction composeReference(Reference2LongFunction before) {
      return (k) -> this.get(before.getLong(k));
   }
}
