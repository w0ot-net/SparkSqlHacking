...
public interface Double2IntMap extends Double2IntFunction, Map {
   int size();
   ...
   default int computeIfAbsentNullable(double key, DoubleFunction mappingFunction) {
      Objects.requireNonNull(mappingFunction);
      int v = this.get(key);
      int drv = this.defaultReturnValue();
      if (v == drv && !this.containsKey(key)) {
   ...
   default int computeIfPresent(double key, BiFunction remappingFunction) {
   ...
   default int compute(double key, BiFunction remappingFunction) {
      Objects.requireNonNull(remappingFunction);
      int oldValue = this.get(key);
      int drv = this.defaultReturnValue();
   ...
   default int merge(double key, int value, BiFunction remappingFunction) {
      Objects.requireNonNull(remappingFunction);
      int oldValue = this.get(key);
      int drv = this.defaultReturnValue();
      int newValue;
   ...
   default int mergeInt(double key, int value, shaded.parquet.it.unimi.dsi.fastutil.ints.IntBinaryOperator remappingFunction) {
   ...
   /** @deprecated */
   @Deprecated
   default Integer computeIfAbsent(Double key, Function mappingFunction) {
      return (Integer)super.computeIfAbsent(key, mappingFunction);
   }

   /** @deprecated */
   @Deprecated
   default Integer computeIfPresent(Double key, BiFunction remappingFunction) {
      return (Integer)super.computeIfPresent(key, remappingFunction);
   }

   /** @deprecated */
   @Deprecated
   default Integer compute(Double key, BiFunction remappingFunction) {
      return (Integer)super.compute(key, remappingFunction);
   }

   /** @deprecated */
   @Deprecated
   default Integer merge(Double key, Integer value, BiFunction remappingFunction) {
