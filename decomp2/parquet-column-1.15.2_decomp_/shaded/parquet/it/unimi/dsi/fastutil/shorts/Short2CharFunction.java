...
@FunctionalInterface
public interface Short2CharFunction extends shaded.parquet.it.unimi.dsi.fastutil.Function, IntUnaryOperator {
   /** @deprecated */
   @Deprecated
   ...
   default Short2ObjectFunction andThenObject(Char2ObjectFunction after) {
      return (k) -> after.get(this.get(k));
   }

   default Object2CharFunction composeObject(Object2ShortFunction before) {
      return (k) -> this.get(before.getShort(k));
   }

   default Short2ReferenceFunction andThenReference(Char2ReferenceFunction after) {
      return (k) -> after.get(this.get(k));
   }

   default Reference2CharFunction composeReference(Reference2ShortFunction before) {
      return (k) -> this.get(before.getShort(k));
   }
}
