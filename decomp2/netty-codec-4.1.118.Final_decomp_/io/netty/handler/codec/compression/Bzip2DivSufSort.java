...
final class Bzip2DivSufSort {
   ...
   private void ssFixdown(int td, int pa, int sa, int i, int size) {
      ...
      int j;
      int k;
      for(int c = T[td + SA[pa + v]] & 255; (j = 2 * i + 1) < size; i = k) {
   ...
   private static int ssLog(int n) {
      return (n & '\uff00') != 0 ? 8 + LOG_2_TABLE[n >> 8 & 255] : LOG_2_TABLE[n & 255];
   }
      ...
      while(true) {
         while(last - first > 8) {
            ...
            if (limit < 0) {
               ...
               int v;
               for(v = T[depth + SA[pa + SA[first]]] & 255; a < last; ++a) {
                  if ((x = T[Td + SA[pa + SA[a]]] & 255) != v) {
                     if (1 < a - first) {
                        break;
                     }
   ...
   private void trFixdown(int isa, int isaD, int isaN, int sa, int i, int size) {
      ...
      int j;
      int k;
      for(int c = this.trGetC(isa, isaD, isaN, v); (j = 2 * i + 1) < size; i = k) {
         k = j++;
   ...
   private static int trLog(int n) {
      return (n & -65536) != 0 ? ((n & -16777216) != 0 ? 24 + LOG_2_TABLE[n >> 24 & 255] : LOG_2_TABLE[n >> 16 & 271]) : ((n & '\uff00') != 0 ? 8 + LOG_2_TABLE[n >> 8 & 255] : LOG_2_TABLE[n & 255]);
   }
         ...
         if (n < isaD - isa) {
            ...
            do {
               ...
               if ((var13 = SA[first]) < 0) {
                  ...
                  for(int i = first; i < last; SA[isa + SA[i]] = i++) {
                  }
      ...
      while(true) {
         if (limit < 0) {
            if (limit == -1) {
               ...
               int b = result.last;
               if (first >= a && b >= last) {
                  for(int c = first; c < last; SA[isa + SA[c]] = c++) {
                  }
   ...
   private int sortTypeBstar(int[] bucketA, int[] bucketB) {
      ...
      m = n - m;
      if (m == 0) {
         for(int var30 = 0; var30 < n; SA[var30] = var30++) {
         }
         ...
         int c1;
         for(int var25 = m - 2; 0 <= var25; SA[--bucketB[BUCKET_BSTAR(c0, c1)]] = var25--) {
            int t = SA[PAb + var25];
            c0 = T[t] & 255;
            c1 = T[t + 1] & 255;
         }
         ...
         while(0 <= i) {
            ...
            if (0 <= i) {
               ...
               for(SA[SA[ISAb + var34]] = i--; 0 <= i && (T[i] & 255) <= (T[i + 1] & 255); --i) {
               }
            }
         }
