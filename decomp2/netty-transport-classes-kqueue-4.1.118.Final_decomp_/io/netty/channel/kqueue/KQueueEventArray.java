...
final class KQueueEventArray {
   ...
   void evSet(AbstractKQueueChannel ch, short filter, short flags, int fflags) {
      ...
      evSet((long)getKEventOffset(this.size++) + this.memoryAddress, ch.socket.intValue(), filter, flags, fflags);
   }
   ...
   private short getShort(int index, int offset) {
      return PlatformDependent.hasUnsafe() ? PlatformDependent.getShort(this.getKEventOffsetAddress(index) + (long)offset) : this.memory.getShort(getKEventOffset(index) + offset);
   }
   ...
   int fd(int index) {
      return PlatformDependent.hasUnsafe() ? PlatformDependent.getInt(this.getKEventOffsetAddress(index) + (long)KQUEUE_IDENT_OFFSET) : this.memory.getInt(getKEventOffset(index) + KQUEUE_IDENT_OFFSET);
   }

   long data(int index) {
      return PlatformDependent.hasUnsafe() ? PlatformDependent.getLong(this.getKEventOffsetAddress(index) + (long)KQUEUE_DATA_OFFSET) : this.memory.getLong(getKEventOffset(index) + KQUEUE_DATA_OFFSET);
   }
