...
class LevelDBTypeInfo {
   ...
   class Index {
      ...
      byte[] keyPrefix(byte[] prefix) {
         ...
         return this.parent != null ? LevelDBTypeInfo.this.buildKey(false, prefix, this.name) : LevelDBTypeInfo.this.buildKey(this.name);
      }

      byte[] start(byte[] prefix, Object value) {
         this.checkParent(prefix);
         return this.parent != null ? LevelDBTypeInfo.this.buildKey(false, prefix, this.name, this.toKey(value)) : LevelDBTypeInfo.this.buildKey(this.name, this.toKey(value));
      }

      byte[] end(byte[] prefix) {
         this.checkParent(prefix);
         return this.parent != null ? LevelDBTypeInfo.this.buildKey(false, prefix, this.name, LevelDBTypeInfo.END_MARKER) : LevelDBTypeInfo.this.buildKey(this.name, LevelDBTypeInfo.END_MARKER);
      }

      byte[] end(byte[] prefix, Object value) {
         this.checkParent(prefix);
         return this.parent != null ? LevelDBTypeInfo.this.buildKey(false, prefix, this.name, this.toKey(value), LevelDBTypeInfo.END_MARKER) : LevelDBTypeInfo.this.buildKey(this.name, this.toKey(value), LevelDBTypeInfo.END_MARKER);
      }
      ...
      byte[] entityKey(byte[] prefix, Object entity) throws Exception {
         ...
         if (!this.isNatural) {
            entityKey = LevelDBTypeInfo.this.buildKey(false, entityKey, this.toKey(LevelDBTypeInfo.this.naturalIndex().getValue(entity)));
         }
      ...
      private void updateCount(WriteBatch batch, byte[] key, long delta) {
         ...
         if (updated > 0L) {
            batch.put(key, LevelDBTypeInfo.this.db.serializer.serialize(updated));
         } else {
            batch.delete(key);
         }
      ...
      private void addOrRemove(WriteBatch batch, Object entity, Object existing, byte[] data, byte[] naturalKey, byte[] prefix) throws Exception {
         ...
         byte[] entityKey = this.start(prefix, indexValue);
         if (!this.isNatural) {
            entityKey = LevelDBTypeInfo.this.buildKey(false, entityKey, naturalKey);
         }
      ...
      long getCount(byte[] key) {
         ...
         return data != null ? LevelDBTypeInfo.this.db.serializer.deserializeLong(data) : 0L;
      }
      ...
      byte[] toKey(Object value, byte prefix) {
         ...
         if (value instanceof String str) {
            ...
            result = new byte[]{prefix, bool ? LevelDBTypeInfo.TRUE : LevelDBTypeInfo.FALSE};
            ...
            result = LevelDBTypeInfo.this.buildKey(false, components);
         } else {
            int bytes;
            ...
            for(int i = 0; i < key.length - 2; ++i) {
               ...
               key[key.length - i - 1] = LevelDBTypeInfo.HEX_BYTES[masked];
            }
