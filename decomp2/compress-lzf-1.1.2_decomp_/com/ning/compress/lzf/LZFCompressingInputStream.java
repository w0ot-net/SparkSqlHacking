      ...
      if (this._bufferPosition < this._bufferLength) {
         ...
         if (count < 0) {
            ...
            if (this._encoder == null) {
               ...
               this._encoder = ChunkEncoderFactory.optimalNonAllocatingInstance(bufferLen, this._recycler);
            }

            if (this._encodedBytes == null) {
               int bufferLen = chunkLength + (chunkLength + 31 >> 5) + 7;
               this._encodedBytes = this._recycler.allocEncodingBuffer(bufferLen);
            }

            int encodeEnd = this._encoder.tryCompress(this._inputBuffer, 0, chunkLength, this._encodedBytes, 7);
