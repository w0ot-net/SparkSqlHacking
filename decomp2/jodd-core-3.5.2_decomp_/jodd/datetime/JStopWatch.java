...
public class JStopWatch {
   ...
   protected long lap(long lap) {
      if (!this.running) {
         ...
         long lapSpanTime = lap - this.startTime;
         long lapTime;
         if (this.laps == null) {
            lapTime = lapSpanTime;
            this.laps = new ArrayList();
         } else {
            long[] previous = (long[])this.laps.get(this.laps.size() - 1);
   ...
   public int totalLaps() {
      return this.laps == null ? 0 : this.laps.size();
   }
   ...
   public long[] getLapTimes(int index) {
      if (this.laps == null) {
         ...
         return index > 0 && index <= this.laps.size() ? (long[])this.laps.get(index - 1) : null;
      }
   }
   ...
   public String toString() {
      ...
      if (this.laps != null) {
         ...
         for(int i = 0; i < this.laps.size(); ++i) {
            long[] longs = (long[])this.laps.get(i);
