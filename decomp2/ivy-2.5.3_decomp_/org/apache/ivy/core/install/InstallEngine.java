...
public class InstallEngine {
   ...
   public ResolveReport install(ModuleRevisionId mrid, String from, String to, InstallOptions options) throws IOException {
      ...
      } else if (toResolver == null) {
         throw new IllegalArgumentException("unknown resolver " + to + ". Available resolvers are: " + this.settings.getResolverNames());
      } else {
         PatternMatcher matcher = this.settings.getMatcher(options.getMatcherName());
         if (matcher == null) {
            throw new IllegalArgumentException("unknown matcher " + options.getMatcherName() + ". Available matchers are: " + this.settings.getMatcherNames());
         } else {
            Message.info(":: installing " + mrid + " ::");
            DependencyResolver oldDictator = this.resolveEngine.getDictatorResolver();
            ...
            try {
               ...
               String resolveId = ResolveOptions.getDefaultResolveId((ModuleDescriptor)md);
               ...
               for(String dc : options.getConfs()) {
                  ...
                  if (MatcherHelper.isExact(matcher, mrid)) {
                     ...
                     for(ModuleRevisionId imrid : this.searchEngine.listModules(fromResolver, mrid, matcher)) {
               ...
               IvyNode[] dependencies = this.resolveEngine.getDependencies(md, resolveOptions, report);
               report.setDependencies(Arrays.asList(dependencies), options.getArtifactFilter());
               Message.info(":: downloading artifacts to cache ::");
               this.resolveEngine.downloadArtifacts(report, options.getArtifactFilter(), new DownloadOptions());
               ...
               this.resolveEngine.setDictatorResolver(oldDictator);
