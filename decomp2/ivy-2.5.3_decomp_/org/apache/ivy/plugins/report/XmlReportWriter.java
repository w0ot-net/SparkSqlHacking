...
public class XmlReportWriter {
   ...
   public void output(ConfigurationResolveReport report, String[] confs, OutputStream stream) {
      ...
      try {
         encodedOutStream = new OutputStreamWriter(stream, "UTF-8");
      ...
      out.println(this.extraToString(mrid.getQualifiedExtraAttributes(), "\t\t"));
      out.println("\t\tconf=\"" + XMLHelper.escape(report.getConfiguration()) + "\"");
      out.println("\t\tconfs=\"" + XMLHelper.escape(StringUtils.joinArray(confs, ", ")) + "\"");
      ...
      for(ModuleId mid : report.getModuleIds()) {
         ...
         for(IvyNode dep : report.getNodes(mid)) {
            this.outputRevision(report, out, dependencies, dep);
         }
   ...
   private void outputRevision(ConfigurationResolveReport report, PrintWriter out, List dependencies, IvyNode dep) {
      ...
      this.outputMetadataArtifact(out, dep);
      this.outputEvictionInformation(report, out, dep);
      this.outputCallers(report, out, dep);
      this.outputArtifacts(report, out, dep);
      out.println("\t\t\t</revision>");
   }

   private String extraToString(Map extraAttributes, String prefix) {
      StringBuilder sb = new StringBuilder();

      for(Map.Entry entry : extraAttributes.entrySet()) {
         if (sb.length() > 0 && !" ".equals(prefix)) {
            sb.append(System.lineSeparator());
         }

         sb.append(prefix);
         sb.append(ExtendableItemHelper.encodeAttribute((String)entry.getKey(), "extra-"));
         sb.append("=\"");
         sb.append(XMLHelper.escape((String)entry.getValue()));
         sb.append("\"");
      }
   ...
   private void outputArtifacts(ConfigurationResolveReport report, PrintWriter out, IvyNode dep) {
      ...
      for(ArtifactDownloadReport adr : report.getDownloadReports(dep.getResolvedId())) {
         ...
         out.print(this.extraToString(adr.getArtifact().getQualifiedExtraAttributes(), " "));
