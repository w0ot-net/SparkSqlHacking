...
public final class XmlModuleDescriptorWriter {
   ...
   public static void write(ModuleDescriptor md, String licenseHeader, File output) throws IOException {
      ...
      try {
         ...
         for(Map.Entry ns : namespaces.entrySet()) {
            xmlNamespace.append(" xmlns:").append((String)ns.getKey()).append("=\"").append((String)ns.getValue()).append("\"");
         }
   ...
   protected static void printDependency(ModuleDescriptor md, DependencyDescriptor dep, PrintWriter out) {
      ...
      out.print(" conf=\"" + sb + "\"");
      printExtraAttributes((ExtendableItem)dep, out, " ");
   ...
   private static void printAllMediators(ModuleDescriptor md, PrintWriter out) {
      ...
      for(Map.Entry mediatorRule : mediators.entrySet()) {
         ...
         if (mediator instanceof OverrideDependencyDescriptorMediator) {
            ...
            out.print(String.format("\t\t<override org=\"%s\" module=\"%s\" matcher=\"%s\"", XMLHelper.escape((String)matcher.getAttributes().get("organisation")), XMLHelper.escape((String)matcher.getAttributes().get("module")), XMLHelper.escape(matcher.getPatternMatcher().getName())));
   ...
   private static void printDependencyArtefacts(ModuleDescriptor md, PrintWriter out, DependencyArtifactDescriptor[] depArtifacts) {
      if (depArtifacts.length > 0) {
         for(DependencyArtifactDescriptor depArtifact : depArtifacts) {
            ...
            printExtraAttributes((ExtendableItem)depArtifact, out, " ");
            out.println("/>");
         }
      }
   ...
   private static void printExtraAttributes(Map extra, PrintWriter out, String prefix) {
      if (extra != null) {
         ...
         for(Map.Entry entry : extra.entrySet()) {
            out.print(String.format("%s%s=\"%s\"", delim, entry.getKey(), XMLHelper.escape((String)entry.getValue())));
            delim = " ";
         }
   ...
   private static void printPublications(ModuleDescriptor md, PrintWriter out) {
      ...
      for(Artifact artifact : md.getAllArtifacts()) {
         out.print(String.format("\t\t<artifact name=\"%s\" type=\"%s\" ext=\"%s\" conf=\"%s\"", XMLHelper.escape(artifact.getName()), XMLHelper.escape(artifact.getType()), XMLHelper.escape(artifact.getExt()), XMLHelper.escape(getConfs(md, artifact))));
         printExtraAttributes((ExtendableItem)artifact, out, " ");
         out.println("/>");
      }
   ...
   protected static void printConfiguration(Configuration conf, PrintWriter out) {
      ...
      printExtraAttributes((ExtendableItem)conf, out, " ");
      out.println("/>");
   }
   ...
   private static void printInfoTag(ModuleDescriptor md, PrintWriter out) {
      ...
      if (!md.getExtraAttributes().isEmpty()) {
         printExtraAttributes((ExtendableItem)md, out, "\t\t");
         out.println();
      }
   ...
   private static void printExtraInfoElement(PrintWriter out, ExtraInfoHolder extraInfo, int indent) {
      ...
      for(Map.Entry entry : extraInfo.getAttributes().entrySet()) {
