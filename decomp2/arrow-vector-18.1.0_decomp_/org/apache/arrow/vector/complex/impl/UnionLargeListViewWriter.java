...
public class UnionLargeListViewWriter extends AbstractFieldWriter {
   protected LargeListViewVector vector;
   ...
   private static final long SIZE_WIDTH = 8L;

   public UnionLargeListViewWriter(LargeListViewVector vector) {
      this(vector, NullableStructWriterFactory.getNullableStructWriterFactoryInstance());
   }

   public UnionLargeListViewWriter(LargeListViewVector vector, NullableStructWriterFactory nullableStructWriterFactory) {
      this.inStruct = false;
   ...
   public UnionLargeListViewWriter(LargeListViewVector vector, AbstractFieldWriter parent) {
      this(vector);
   }
   ...
   public void startList() {
      ...
      this.writer.setPosition(this.vector.getOffsetBuffer().getInt((long)this.idx() * 8L));
   ...
   public void endList() {
      ...
      for(int i = 0; i < this.idx(); ++i) {
         sizeUptoIdx += this.vector.getSizeBuffer().getInt((long)i * 8L);
      }

      this.vector.getSizeBuffer().setInt((long)this.idx() * 8L, this.writer.idx() - sizeUptoIdx);
   ...
   public void startListView() {
      this.vector.startNewValue(this.idx());
      this.writer.setPosition(LargeMemoryUtil.checkedCastToInt((long)this.vector.getOffsetBuffer().getInt((long)this.idx() * 8L)));
      this.listStarted = true;
   }
