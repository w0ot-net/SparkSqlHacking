...
public abstract class BaseRepeatedValueViewVector extends BaseValueVector implements RepeatedValueVector, BaseListVector {
   ...
   protected BaseRepeatedValueViewVector(String name, BufferAllocator allocator, FieldVector vector, CallBack callBack) {
      ...
      this.sizeAllocationSizeInBytes = 15880L;
   ...
   public void setInitialCapacity(int numRecords) {
      this.offsetAllocationSizeInBytes = (long)(numRecords * 4);
      this.sizeAllocationSizeInBytes = (long)(numRecords * 4);
   ...
   protected int getSizeBufferValueCapacity() {
      return LargeMemoryUtil.capAtMaxInt(this.sizeBuffer.capacity() / 4L);
   }

   public int getBufferSize() {
      return this.valueCount == 0 ? 0 : this.valueCount * 4 + this.valueCount * 4 + this.vector.getBufferSize();
   }
   ...
   public int getBufferSizeFor(int valueCount) {
      if (valueCount == 0) {
         ...
         for(int i = 0; i < valueCount; ++i) {
            innerVectorValueCount += this.sizeBuffer.getInt((long)(i * 4));
         }

         return valueCount * 4 + valueCount * 4 + this.vector.getBufferSizeFor(innerVectorValueCount);
      }
   }
   ...
   protected int getMaxViewEndChildVector() {
      ...
      for(int i = 0; i < this.valueCount; ++i) {
         ...
         int currentSize = this.sizeBuffer.getInt((long)(i * 4));
