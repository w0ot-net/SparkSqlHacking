...
public class LargeListViewVector extends BaseLargeRepeatedValueViewVector implements PromotableVector, ValueIterableVector {
   ...
   public static LargeListViewVector empty(String name, BufferAllocator allocator) {
      return new LargeListViewVector(name, allocator, FieldType.nullable(ArrowType.LargeListView.INSTANCE), (CallBack)null);
   }

   public LargeListViewVector(String name, BufferAllocator allocator, FieldType fieldType, CallBack callBack) {
   ...
   private void setReaderAndWriterIndex() {
      ...
      if (this.valueCount == 0) {
         ...
         this.offsetBuffer.writerIndex((long)this.valueCount * 8L);
         this.sizeBuffer.writerIndex((long)this.valueCount * 8L);
      }
   ...
   public void copyFromSafe(int inIndex, int outIndex, ValueVector from) {
      throw new UnsupportedOperationException("LargeListViewVector does not support copyFromSafe operation yet.");
   }

   public void copyFrom(int inIndex, int outIndex, ValueVector from) {
      throw new UnsupportedOperationException("LargeListViewVector does not support copyFrom operation yet.");
   }
   ...
   public int hashCode(int index, ArrowBufHasher hasher) {
      if (this.isSet(index) == 0) {
         ...
         int hash = 0;
         int start = this.offsetBuffer.getInt((long)index * 8L);
         int end = this.sizeBuffer.getInt((long)index * 8L);

         for(int i = start; i < end; ++i) {
            hash = ByteFunctionHelpers.combineHash(hash, this.vector.hashCode(LargeMemoryUtil.checkedCastToInt((long)i), hasher));
         }
   ...
   protected FieldReader getReaderImpl() {
      throw new UnsupportedOperationException("LargeListViewVector does not support getReaderImpl operation yet.");
   }

   public UnionListReader getReader() {
      throw new UnsupportedOperationException("LargeListViewVector does not support getReader operation yet.");
   }
   ...
   public int getBufferSize() {
      if (this.valueCount == 0) {
         ...
         int offsetBufferSize = this.valueCount * 8;
         int sizeBufferSize = this.valueCount * 8;
   ...
   public List getObject(int index) {
      if (this.isSet(index) == 0) {
         ...
         int start = this.offsetBuffer.getInt((long)(index * 8));
         int end = start + this.sizeBuffer.getInt((long)(index * 8));
         ValueVector vv = this.getDataVector();

         for(int i = start; i < end; ++i) {
            vals.add(vv.getObject(LargeMemoryUtil.checkedCastToInt((long)i)));
         }
   ...
   public boolean isEmpty(int index) {
      if (this.isNull(index)) {
         ...
         return this.sizeBuffer.getInt((long)(index * 8)) == 0;
      }
   }
   ...
   public void setNull(int index) {
      ...
      this.offsetBuffer.setInt((long)(index * 8), 0);
      this.sizeBuffer.setInt((long)(index * 8), 0);
   ...
   public int startNewValue(int index) {
      ...
      if (index > 0) {
         ...
         this.offsetBuffer.setInt((long)(index * 8), prevOffset);
      }

      BitVectorHelper.setBit(this.validityBuffer, (long)index);
      return this.offsetBuffer.getInt((long)(index * 8));
   }
   ...
   public void setOffset(int index, int value) {
      this.validateInvariants(value, this.sizeBuffer.getInt((long)(index * 8)));
      this.offsetBuffer.setInt((long)(index * 8), value);
   }

   public void setSize(int index, int value) {
      this.validateInvariants(this.offsetBuffer.getInt((long)(index * 8)), value);
      this.sizeBuffer.setInt((long)(index * 8), value);
   }
   ...
   public void setValueCount(int valueCount) {
      ...
      long childValueCount = valueCount == 0 ? 0L : (long)this.getMaxViewEndChildVector();
      Preconditions.checkArgument(childValueCount <= 2147483647L && childValueCount >= 0L, "LargeListViewVector doesn't yet support 64-bit allocations: %s", childValueCount);
   ...
   public void validate() {
      for(int i = 0; i < this.valueCount; ++i) {
         int offset = this.offsetBuffer.getInt((long)i * 8L);
         int size = this.sizeBuffer.getInt((long)i * 8L);
   ...
   public void endValue(int index, int size) {
      this.sizeBuffer.setInt((long)index * 8L, size);
   }
   ...
   private class TransferImpl implements TransferPair {
      ...
      public TransferImpl(String name, BufferAllocator allocator, CallBack callBack) {
         this(new LargeListViewVector(name, allocator, LargeListViewVector.this.field.getFieldType(), callBack));
      }
      ...
      public TransferImpl(LargeListViewVector to) {
         this.to = to;
         to.addOrGetVector(LargeListViewVector.this.vector.getField().getFieldType());
         if (to.getDataVector() instanceof ZeroVector) {
            to.addOrGetVector(LargeListViewVector.this.vector.getField().getFieldType());
         }

         this.dataTransferPair = LargeListViewVector.this.getDataVector().makeTransferPair(to.getDataVector());
      }

      public void transfer() {
         this.to.clear();
         this.dataTransferPair.transfer();
         this.to.validityBuffer = BaseValueVector.transferBuffer(LargeListViewVector.this.validityBuffer, this.to.allocator);
         this.to.offsetBuffer = BaseValueVector.transferBuffer(LargeListViewVector.this.offsetBuffer, this.to.allocator);
         this.to.sizeBuffer = BaseValueVector.transferBuffer(LargeListViewVector.this.sizeBuffer, this.to.allocator);
         if (LargeListViewVector.this.valueCount > 0) {
            this.to.setValueCount(LargeListViewVector.this.valueCount);
         }

         LargeListViewVector.this.clear();
      }

      public void splitAndTransfer(int startIndex, int length) {
         Preconditions.checkArgument(startIndex >= 0 && length >= 0 && startIndex + length <= LargeListViewVector.this.valueCount, "Invalid parameters startIndex: %s, length: %s for valueCount: %s", startIndex, length, LargeListViewVector.this.valueCount);
         this.to.clear();
         if (length > 0) {
            this.to.offsetBuffer = this.to.allocateBuffers((long)length * 8L);
            this.to.sizeBuffer = this.to.allocateBuffers((long)length * 8L);
            int maxOffsetAndSizeSum = Integer.MIN_VALUE;
            int minOffsetValue = Integer.MAX_VALUE;

            for(int i = 0; i < length; ++i) {
               int offsetValue = LargeListViewVector.this.offsetBuffer.getInt((long)(startIndex + i) * 8L);
               int sizeValue = LargeListViewVector.this.sizeBuffer.getInt((long)(startIndex + i) * 8L);
               this.to.sizeBuffer.setInt((long)i * 8L, sizeValue);
            ...
            for(int i = 0; i < length; ++i) {
               ...
               this.to.offsetBuffer.setInt((long)i * 8L, relativeOffset);
            }
      ...
      private void splitAndTransferValidityBuffer(int startIndex, int length, LargeListViewVector target) {
         int firstByteSource = BitVectorHelper.byteIndex(startIndex);
         int lastByteSource = BitVectorHelper.byteIndex(LargeListViewVector.this.valueCount - 1);
         int byteSizeTarget = LargeListViewVector.getValidityBufferSizeFromCount(length);
         ...
         if (length > 0) {
            if (offset == 0) {
               ...
               target.validityBuffer = LargeListViewVector.this.validityBuffer.slice((long)firstByteSource, (long)byteSizeTarget);
               target.validityBuffer.getReferenceManager().retain(1);
            } else {
               target.allocateValidityBuffer((long)byteSizeTarget);

               for(int i = 0; i < byteSizeTarget - 1; ++i) {
                  byte b1 = BitVectorHelper.getBitsFromCurrentByte(LargeListViewVector.this.validityBuffer, firstByteSource + i, offset);
                  byte b2 = BitVectorHelper.getBitsFromNextByte(LargeListViewVector.this.validityBuffer, firstByteSource + i + 1, offset);
                  target.validityBuffer.setByte((long)i, b1 + b2);
               }

               if (firstByteSource + byteSizeTarget - 1 < lastByteSource) {
                  byte b1 = BitVectorHelper.getBitsFromCurrentByte(LargeListViewVector.this.validityBuffer, firstByteSource + byteSizeTarget - 1, offset);
                  byte b2 = BitVectorHelper.getBitsFromNextByte(LargeListViewVector.this.validityBuffer, firstByteSource + byteSizeTarget, offset);
