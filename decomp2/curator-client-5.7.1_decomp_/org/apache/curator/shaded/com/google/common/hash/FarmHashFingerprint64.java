...
final class FarmHashFingerprint64 extends AbstractNonStreamingHashFunction {
   ...
   private static long hashLength0to16(byte[] bytes, int offset, int length) {
      if (length >= 8) {
         ...
         long a = (long)LittleEndianByteArray.load32(bytes, offset) & 4294967295L;
   ...
   private static long hashLength65Plus(byte[] bytes, int offset, int length) {
      ...
      do {
         ...
         long var21 = Long.rotateRight(y + v[1] + LittleEndianByteArray.load64(bytes, offset + 48), 42) * -5435081209227447693L;
         x ^= w[1];
      ...
      tmp = -5435081209227447693L + ((tmp & 255L) << 1);
      w[0] += (long)(length - 1 & 63);
      v[0] += w[0];
      w[0] += v[0];
      x = Long.rotateRight(x + y + v[0] + LittleEndianByteArray.load64(bytes, last64offset + 8), 37) * tmp;
      y = Long.rotateRight(y + v[1] + LittleEndianByteArray.load64(bytes, last64offset + 48), 42) * tmp;
      x ^= w[1] * 9L;
      y += v[0] * 9L + LittleEndianByteArray.load64(bytes, last64offset + 40);
      z = Long.rotateRight(z + w[0], 33) * tmp;
      weakHashLength32WithSeeds(bytes, last64offset, v[1] * tmp, x + w[0], v);
      weakHashLength32WithSeeds(bytes, last64offset + 32, z + w[1], y + LittleEndianByteArray.load64(bytes, last64offset + 16), w);
      return hashLength16(hashLength16(v[0], w[0], tmp) + shiftMix(y) * -4348849565147123417L + x, hashLength16(v[1], w[1], tmp) + z, tmp);
   }
}
