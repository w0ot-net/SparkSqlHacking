...
class OpenSslKeyMaterialProvider {
   ...
   private static void validateSupported(PrivateKey key, String password) throws SSLException {
      if (key != null) {
         ...
         try {
            pkeyBio = ReferenceCountedOpenSslContext.toBIO(UnpooledByteBufAllocator.DEFAULT, (PrivateKey)key);
   ...
   private static void validateSupported(X509Certificate[] certificates) throws SSLException {
      if (certificates != null && certificates.length != 0) {
         ...
         try {
            ...
            chainBio = ReferenceCountedOpenSslContext.toBIO(UnpooledByteBufAllocator.DEFAULT, (PemEncoded)encoded.retain());
