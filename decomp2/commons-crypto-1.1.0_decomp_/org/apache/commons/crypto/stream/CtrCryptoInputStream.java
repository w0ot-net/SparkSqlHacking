...
public class CtrCryptoInputStream extends CryptoInputStream {
   ...
   public CtrCryptoInputStream(Properties properties, InputStream inputStream, byte[] key, byte[] iv, long streamOffset) throws IOException {
      this(inputStream, Utils.getCipherInstance("AES/CTR/NoPadding", properties), CryptoInputStream.getBufferSize(properties), key, iv, streamOffset);
   }
   ...
   public CtrCryptoInputStream(Properties properties, ReadableByteChannel in, byte[] key, byte[] iv, long streamOffset) throws IOException {
      this(in, Utils.getCipherInstance("AES/CTR/NoPadding", properties), CryptoInputStream.getBufferSize(properties), key, iv, streamOffset);
   }
   ...
   protected CtrCryptoInputStream(InputStream inputStream, CryptoCipher cipher, int bufferSize, byte[] key, byte[] iv, long streamOffset) throws IOException {
      this((Input)(new StreamInput(inputStream, bufferSize)), cipher, bufferSize, key, iv, streamOffset);
   }
   ...
   protected CtrCryptoInputStream(ReadableByteChannel channel, CryptoCipher cipher, int bufferSize, byte[] key, byte[] iv, long streamOffset) throws IOException {
      this((Input)(new ChannelInput(channel)), cipher, bufferSize, key, iv, streamOffset);
   }
   ...
   protected CtrCryptoInputStream(Input input, CryptoCipher cipher, int bufferSize, byte[] key, byte[] iv, long streamOffset) throws IOException {
      super((Input)input, cipher, bufferSize, new SecretKeySpec(key, "AES"), new IvParameterSpec(iv));
      this.streamOffset = 0L;
      this.cipherReset = false;
      this.initIV = (byte[])(([B)iv).clone();
      this.iv = (byte[])(([B)iv).clone();
   ...
   public long skip(long n) throws IOException {
      ...
      if (n == 0L) {
         return 0L;
      } else if (n <= (long)this.outBuffer.remaining()) {
         int pos = this.outBuffer.position() + (int)n;
         this.outBuffer.position(pos);
         return n;
      } else {
         n -= (long)this.outBuffer.remaining();
         ...
         skipped += (long)this.outBuffer.remaining();
   ...
   protected byte getPadding(long position) {
      return (byte)((int)(position % (long)this.cipher.getBlockSize()));
   }
   ...
   static void calculateIV(byte[] initIV, long counter, byte[] IV) {
      ...
      for(int sum = 0; i-- > 0; IV[i] = (byte)sum) {
         sum = (initIV[i] & 255) + (sum >>> 8);
         if (j++ < 8) {
            sum += (byte)((int)counter) & 255;
            counter >>>= 8;
         }
      }
