...
import shaded.parquet.com.fasterxml.jackson.core.util.InternalJacksonUtil;
...
public class UTF8StreamJsonParser extends JsonParserBase {
      ...
      if (i == 34) {
         ...
         switch (i) {
            case 43:
               ...
               return this._updateToken(this._parseSignedNumber(false));
            case 45:
   ...
   protected String _parseAposName() throws IOException {
      ...
      if (ch == 39) {
         ...
         for(int[] codes = INPUT_CODES_LATIN1; ch != 39; ch = this._inputBuffer[this._inputPtr++] & 255) {
            if (codes[ch] != 0 && ch != 34) {
               if (ch != 92) {
                  this._throwUnquotedSpace(ch, "name");
               } else {
   ...
   protected JsonToken _handleUnexpectedValue(int c) throws IOException {
      switch (c) {
         case 43:
            ...
            return this._handleInvalidNumberStart(this._inputBuffer[this._inputPtr++] & 255, false, true);
         case 73:
   ...
   protected JsonToken _handleApos() throws IOException {
      ...
      while(true) {
         ...
         int max = this._inputEnd;
         int max2 = InternalJacksonUtil.addOverflowSafe(this._inputPtr, outBuf.length - outPtr);
         if (max2 < max) {
            max = max2;
         }

         while(this._inputPtr < max) {
            c = inputBuffer[this._inputPtr++] & 255;
            if (c == 39) {
               this._textBuffer.setCurrentLength(outPtr);
   ...
   protected JsonLocation _currentLocationMinusOne() {
      ...
      return new JsonLocation(this._contentReference(), this._currInputProcessed + (long)prevInputPtr, -1L, this._currInputRow, col);
   }
