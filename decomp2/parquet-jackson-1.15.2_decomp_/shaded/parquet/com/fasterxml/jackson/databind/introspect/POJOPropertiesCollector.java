...
public class POJOPropertiesCollector {
   ...
   protected void _addCreators(Map props) {
      ...
      if (this._isRecordType) {
         defaultCreator = JDK14Util.findCanonicalRecordConstructor(this._config, this._classDef, constructors);
      } else {
         defaultCreator = this._annotationIntrospector.findDefaultCreator(this._config, this._classDef, constructors, factories);
      }

      this._removeDisabledCreators(constructors);
      this._removeDisabledCreators(factories);
      this._removeNonFactoryStaticMethods(factories, defaultCreator);
      ...
      if (!creators.hasPropertiesBased()) {
         this._addCreatorsWithAnnotatedNames(creators, constructors, defaultCreator);
      }

      if (defaultCreator != null && (constructors.remove(defaultCreator) || factories.remove(defaultCreator))) {
         if (this._isDelegatingConstructor(defaultCreator)) {
            if (!creators.hasDelegating()) {
               creators.addExplicitDelegating(defaultCreator);
            }
         } else if (!creators.hasPropertiesBased()) {
            creators.setPropertiesBased(this._config, defaultCreator, "Primary");
         }
      }
   ...
   private void _removeDisabledCreators(List ctors) {
      ...
      while(it.hasNext()) {
         if (((PotentialCreator)it.next()).creatorMode() == JsonCreator.Mode.DISABLED) {
            it.remove();
         }
      }
   ...
   private void _removeNonFactoryStaticMethods(List ctors, PotentialCreator defaultCreator) {
      Class<?> rawType = this._type.getRawClass();
      Iterator<PotentialCreator> it = ctors.iterator();

      while(it.hasNext()) {
         PotentialCreator ctor = (PotentialCreator)it.next();
         if (!ctor.isAnnotated() && defaultCreator != ctor) {
   ...
   private void _addCreatorsWithAnnotatedNames(PotentialCreators collector, List ctors, PotentialCreator defaultCtor) {
      List<PotentialCreator> found = this._findCreatorsWithAnnotatedNames(ctors);
      if (defaultCtor != null && found.contains(defaultCtor)) {
         collector.setPropertiesBased(this._config, defaultCtor, "implicit");
      } else {
