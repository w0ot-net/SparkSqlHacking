...
class Base64Codec extends BaseNCodec {
   ...
   Base64Codec(int lineLength, byte[] lineSeparator) {
      this(lineLength, lineSeparator, false);
   }

   Base64Codec(int lineLength, byte[] lineSeparator, boolean urlSafe) {
      this(lineLength, lineSeparator, urlSafe, DECODING_POLICY_DEFAULT);
   }

   Base64Codec(int lineLength, byte[] lineSeparator, boolean urlSafe, CodecPolicy decodingPolicy) {
      super(3, 4, lineLength, BaseNCodec.length(lineSeparator), (byte)61, decodingPolicy);
      this.decodeTable = DECODE_TABLE;
      if (lineSeparator != null) {
         if (this.containsAlphabetOrPad(lineSeparator)) {
            String sep = Strings.utf8(lineSeparator);
            throw new IllegalArgumentException("lineSeparator must not contain base64 characters: [" + sep + "]");
         }

         if (lineLength > 0) {
            this.encodeSize = 4 + lineSeparator.length;
            this.lineSeparator = (byte[])(([B)lineSeparator).clone();
         } else {
            this.encodeSize = 4;
